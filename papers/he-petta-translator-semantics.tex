...
\paragraph{Method.}
Each \HE{} surface is run on upstream Rust \HE{} (the sole oracle of \HE{}
...
cluster in the families below, and per-surface probes corroborate and add finer
cases (such as the \code{if}-arity row).  The machine-readable record is the
surface registry \code{petta-he-profile/tests/he\_spec\_surfaces.tsv} and the
...
Typed-error surface & \HE{} emits structured \code{BadArgType}/\code{BadType};
...
surfaces (for example arithmetic, \code{eval}, \code{chain},
...
precisely the surfaces where the two dialects genuinely disagree.  \PeTTa{}
\code{--he} is
