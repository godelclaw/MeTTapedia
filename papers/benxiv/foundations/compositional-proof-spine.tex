...
in the surrounding \texttt{ClosedWebAtGoodWord.Instance}.  The remaining
geometric statement is therefore narrower than whole-annulus coverage but
still substantive: refactor that closed-web parameter to the collar or
three-Cell word actually selected by the source, and identify its cumulative
terminal projection with the Seed semantics consumed at the local target.  No
further equivalence between vertex-component and edge-component semantics is
owed.  The filtered finite accepting set has also not yet been emitted as an
