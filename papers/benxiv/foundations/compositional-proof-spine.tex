...
\paragraph{Remaining supply step.}
The complete cuts now exist with a uniform target-class width.
To discharge the global geometric obligation, large vertex count must
still produce sufficiently many strictly nested cuts with cycles on both
shores, and those cuts must feed the exact-support node constructor and
state bound.  That extraction, fixed-mesh exclusion, the raw
decomposition consumer and the generating-base audit remain open.
The fixed ledger remains $57/100$.
