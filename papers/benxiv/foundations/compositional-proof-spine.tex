...
cuts ordered \emph{edges}.  At a cubic middle vertex one can locally move
the noose off the vertex by crossing at most two incident edges, but
preservation of nesting, seam order, and the rotation system under all such
moves is still a construction, not a citation.
...
are no longer D3 gaps.  What remains is topological: the matching must reverse
the boundary orientation, the composite face orbits must satisfy the
spherical Euler equality, and the result must land in whichever
loopless/simple representation the minimality theorem consumes.  The current
matching type is only a bare equivalence, so it cannot yet express the first
...
Theorem~\ref{thm:implemented-seam-composite-fields}.  What remains is its
orientation-aware spherical face calculation and, for a simple-graph
carrier, the finite loop/parallel-edge check; neither may be left as an
implicit property of the drawing.
...
sphere-cut adapters; the orientation-reversed seam witness, spherical
face-orbit calculation, and representation closure still needed for D3; and
the route-native finite base.  The mesh
