...
branch-decomposition supplier must construct the nested literal shores,
their roots and width bounds, and the strict cubic star between repeated
states.  No ambient graph data are reread after the state has been formed.
...
Thus the remaining cut-side work is M1 rather than M2: produce compatible
nested connected shores, with roots and a strict-material witness, from the
chosen corridor or sphere-cut decomposition.  Their ordered first-return data
and exact supports are computed fields of the already checked finite state;
the supplier does not assume in advance that two such fields are equal.
