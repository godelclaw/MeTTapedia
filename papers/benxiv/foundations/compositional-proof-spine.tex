...
Let an exact cyclic edge cut have connected induced vertex sides in a finite
cubic graph, and let its cut size be at most (b).  Assign to the chosen
side every edge with at least one endpoint there.  This is a connected shore
...
\begin{proof}
Connectivity of the two induced vertex sides transports to connectivity of
...
both separator components are short.  A long carrier interval is compressed
by the restricted LIFO sweep; its outcomes are deeper laminar nesting or a
materially spaced repeated state inside that same interval.  A long ambient
return is likewise compressed by the attachment sweep: its outcomes are an
external separator exit, a deep same-turn chord nest, or a materially spaced
repeated attachment state.  What remains is to consume these structured
depth/repetition/exit alternatives, choose compatible interfaces coherently
across neighbouring mesh sites, saturate them to connected complementary
shores, and prove that the cumulative zero-count profile factors through the
resulting ordered interfaces.  The corollary is therefore a graph-level local
alternative at one alternating site; it proves neither a coherent mesh-level
family nor the splice.
