...
long-ambient-return, long-carrier-interval, and repeated-state horns uniformly,
...
both separator components are short.  A long carrier interval remains on the
one-dimensional LIFO carrier and can be attacked by a restricted sweep.  A
long ambient return instead requires a new sweep of the attachments along
that path; planarity alone does not make its interface bounded.  Neither case
is mistaken for a bounded interface.  One still has to compress these two
long-return horns, choose interfaces coherently
