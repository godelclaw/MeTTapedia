...
\begin{corollary}[localization along the sparse deletion path]
...
union of the nine deletion footprints.  Then $|P|\leq45$.  The two
receipt-bearing completed matchings agree on every ambient edge outside
$P$, and their symmetric-difference graphs with the fixed residual-defect
minimizer also agree outside $P$.  Consequently, every walk in the first
symmetric-difference graph whose edges avoid $P$ is a walk between the same
endpoints in the second graph; in particular its endpoints belong to one
connected component there.  If one endpoint of that transferred walk lies
in the distinguished alternating carrier recorded by the second site's
two-sector receipt, then the transferred component is the second site's
...
operation, so equality outside $P$ is preserved.  Finally transfer the
supplied walk edge by edge to the second graph.  The transferred walk is a
reachability witness, which is exactly equality of its endpoint connected
components.
...
This is a localization statement, not an identification of distinguished
components without an anchor.  The remaining geometric obligation is now
the exact existential statement that a canonical return component at one
selected site has an endpoint in the distinguished carrier at the other
site, reached by a source arc outside $P$.  No such anchor is inferred merely
