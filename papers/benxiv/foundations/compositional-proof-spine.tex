...
This closes the former representation-level realization adapter.  The
remaining statement is geometric coverage by the serial construction: the
terminal prefix produced by the literal one-Cell transition must cover the
whole annulus (or be proved to have the same cumulative terminal projection),
with the correct realizable initial state.  No further equivalence between
vertex-component and edge-component semantics is owed.  The filtered finite
accepting set has also not yet been emitted as an optimized executable
certificate.
