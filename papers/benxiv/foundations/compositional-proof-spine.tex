...
and minimum face length five are machine-checked.  Only the standard external
...
\begin{proposition}[one hexagonal cell has no smaller exact connected replacement]
\label{prop:hex-cell-no-smaller-exact-replacement}
\Sprose
No connected cubic six-port tangle with fewer than six internal vertices has
the same ordered boundary support as \(H_6\).
\end{proposition}

\begin{proof}
Let such a tangle have \(n\) internal vertices and \(m\) internal edges.
Counting incidences, including the six ports, gives
\[
                            3n=2m+6.
\]
The internal graph is connected, so \(m\geq n-1\).  Hence \(n\geq4\), while
the incidence equation makes \(n\) even.  Under \(n<6\) this forces
\(n=4\) and \(m=3\); the internal graph is therefore a tree.

Root that tree.  A proper colouring has \(3!=6\) choices for the three
labelled incidences at the root.  At each of the other three vertices, the
parent-edge colour is already fixed, and the remaining two labelled
incidences can receive the other two colours in at most two orders.  Thus the
tangle has at most
\[
                            6\cdot2^3=48
\]
proper colourings, and consequently realizes at most \(48\) boundary words.
Since \(48<63\), its support cannot equal that of \(H_6\).
\end{proof}

This is a committed local obstruction, not wall exclusion.  It rules out an
exact support-preserving reduction which removes one isolated hexagonal cell
and keeps the same six ports.  It does not rule out replacing a larger patch,
using a target-aware quotient weaker than full support equality, or performing
a two-dimensional move whose size decrease occurs elsewhere.  Those are the
remaining source-faithful possibilities.
...
Thus Proposition~\ref{prop:hex-cell-no-smaller-exact-replacement} does not
kill target-aware one-cell reduction; it shows precisely why such a reduction
cannot be justified by exact support preservation.  The remaining constructive
problem is to exploit the fixed exterior support while preserving the
topological class and decreasing size.
...
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24SphericalReductiveAssembly.sphericalFourColorStatement_of_supply_and_base}
...
most $B(k,w)$ vertices is Tait colourable.  Then every finite spherical
map is vertex-four-colourable, component by component.
\end{theorem}
...
counterexample and constructs the decomposition supply consumed by
Theorem~\ref{thm:spherical-reductive-assembly}.  The finite-base hypothesis
is its second premise.  That theorem gives the spherical Four-Colour
statement; the separate classical representation theorem from an abstract
planar graph to a spherical rotation presentation gives the customary
abstract-planar formulation.
\end{proof}
