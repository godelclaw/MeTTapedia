...
darts, and a matching of equal seam states which reverses their computed
first-return orders.  A tight noose is one sufficient way to manufacture
that data, but it is not part of the algebraic seam theorem.
...
for the splice are saturated, their boundary first-return orders carry a
specified orientation-reversing bijection, and the annular slab between
...
The stipulated equality of ordered seam states identifies the two computed
first-return orders in opposite directions, so
Lemma~\ref{lem:canonical-hub-orientation} supplies the required orientation
reversal.  Hence
...
Corollary~\ref{cor:planar-bond-canonical-cap}, together with the explicit
opposite first-return equation carried by the ordered seam state.
...
link is narrower than a transverse-noose construction: the chosen sphere-cut
decomposition must yield nested connected literal shores and a seam-state
matching which reverses their computed first-return orders.  The cap face
count, cap sphericity, and one-hub property are no longer open.
...
the production of compatible nested connected shores, with opposite ordered
first-return data, from the supplied vertex-based sphere cuts.
