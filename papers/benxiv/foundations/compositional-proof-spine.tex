...
\Sverified{Mettapedia.GraphTheory.Embedding.exists_exactFaceCut_of_closedTrail}
...
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24ResidualReturnSectorNoncrossing.ExactFaceCut.label_eq_otherChord_thirdDarts}
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24ResidualReturnSectorNoncrossing.OrderedReturnChord.interval_long_of_crosses}
\Sprose
...
\begin{proof}
Use the connected orientable cellular surface associated with the spherical
rotation system.  Its Euler characteristic is two, so this surface is a
sphere, and
the embedded simple cycle \(C_e\) is a Jordan curve.  It bounds two open
discs, precisely the two face shores encoded by \(F_e\) and its complement.

For a return chord with endpoints \(a,c\), choose a simple path \(P\) in its
common-residual component.  No edge of \(P\) is an edge of \(C_e\).  Moreover,
no internal vertex of \(P\) lies on \(C_e\): every cycle vertex has degree one
in the common residual graph, whereas an internal vertex of a simple path
uses two incident path edges.  Hence the interior of \(P\) is a connected
subset of the complement of the Jordan curve and lies wholly in one of its
two open discs.  The oriented third dart at either endpoint identifies that
disc, so it is exactly the value \(\chi_e(P)\).  This also gives the geometric
meaning of the already-proved equality of the two endpoint labels.

Now suppose two distinct chords have cyclically alternating endpoints
\(a,b,c,d\), and choose their simple return paths \(P:a\leadsto c\) and
\(Q:b\leadsto d\).  If their labels were equal, the interiors of both paths
would be properly embedded arcs in the same closed disc bounded by \(C_e\).
The Jordan arc theorem for a disc says that the proper arc \(P\) separates
the disc into two components whose boundary portions are the two complementary
\(a\)-to-\(c\) arcs of \(C_e\).  Alternation puts \(b\) and \(d\) on different
such boundary portions.  Therefore every arc from \(b\) to \(d\) in that disc
meets \(P\), so \(Q\) meets \(P\).  But distinct return chords lie in distinct
components of the degree--\(1/2\) common residual graph; the preceding
corollary proves their witnessing paths have disjoint vertex supports.  This
contradiction proves that alternating chords have opposite labels, and hence
that each fixed-label family is noncrossing.
\end{proof}
...
simultaneous choice of the \(\tau_e\).  The local formation receipts identify
