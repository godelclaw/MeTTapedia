...
D2b remains open: the source-facing cyclic encoding/round trip,
integrated admissible-prefix pruning, and the exact raw/support/Nerode
closure certificates of Proposition~\ref{prop:exact-closure-gate} still
require discharge. Constructing a physical object for every valid code
does not by itself prove coverage of every required source witness.
...
What is \emph{not} established here is that pruning is lossless. Visibility
is already known to be inherited by prefixes of a connected object. The
planar half is not: it needs monotonicity of the Euler defect under the
literal vertex attachment, equivalently
\[
 \Delta F\le 1+|P|-2d
\]
in the notation of \S\ref{sec:physical-vertex-attachment}, and that
inequality is open.
...
missing step is therefore not a new combinatorial lemma but a presentation
bridge: identifying the vertex, face, component and edge counts of a
...
Section~\ref{sec:vertex-code-lossless-pruning} below. Source coverage still
needs the rotation-faithful run-level round trip. No closure count, threshold
...
not source coverage or the optimized closure certificate. The latter still
...
This is the single-letter half of source coverage. It does not yet give the
run-level round trip: that one sweep of a source map emits a well-typed run
whose realization is isomorphic to the source, with interior darts, the edge
involution, persistent-port permutations and named boundary coordinates all
corresponding. Nor does it supply a source class for which cyclicity holds at
every swept vertex. Both remain open, and the earned ledger is unchanged.
