...
strict-material accounting.  What remains is to package the connected branch
decomposition as a rooted strictly nested edge-shore tree and prove the
vertex accounting.  The majority shores, literal nodes, and exact ordered
Count states are then computed by
Theorem~\ref{thm:connected-edge-shore-majority},
Lemma~\ref{lem:connected-shore-literal-node}, and
Theorem~\ref{thm:literal-majority-shore-tree-descent}.

\begin{proposition}[wall-exclusion reduction, relative form]
\label{prop:target-forced-wall}
\Sconditional{planar wall theorem; connected branch-decomposition tree supply; route-native finite base}
...
most \(w\);
\item every width-\(w\) instance in the class has a rooted binary connected
edge-shore tree satisfying the nodewise middle bound, proper nesting, and
\(|V|\le2|V(\mathcal T)|\) required by
Theorem~\ref{thm:literal-majority-shore-tree-descent}.
\end{enumerate}
...
branch-decomposition supplier must construct the nested connected edge-shore
tree, its middle bounds, proper nesting, and vertex accounting.  The roots,
literal widths, two boundary ports, exact states, and strict cubic star
between repeated states are now derived.  No ambient graph data are reread
after the state has been formed.
...
\Sconditional{Tait bridge; fixed-wall exclusion for least counterexamples; connected branch-decomposition tree supply; route-native finite base}
...
of the normal-form class carry the connected edge-shore tree required by
Theorem~\ref{thm:literal-majority-shore-tree-descent}.  If the resulting
...
Thus the remaining cut-side work is M1 rather than M2: produce the rooted
strictly nested connected edge-shore tree, middle bounds, and vertex
accounting from the chosen corridor or branch decomposition.  Roots, ports,
strict material, ordered first-return data, and exact supports are computed
by the checked consumer; the supplier does not assume in advance that two
such fields are equal.
...
exclusion plus a connected branch-decomposition tree supply.  Both chains
...
connected branch-decomposition tree supplier; and the route-native finite
