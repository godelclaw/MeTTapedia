...
\[
                  \ell_o<\ell_i<r_i<r_o.
\]
Assume that the displayed inner carrier interval has more than one edge.
...
The outer closure is automatically nondegenerate.  Both its physical return
and its complementary carrier interval are nonempty.  If both had exactly
one edge, simplicity of the ambient graph would make them the same edge;
this is impossible because a residual return avoids every carrier edge.
