...
and the mesh obstruction are machine-checked.  The connected
branch-decomposition theorem is a named external conditional; wall exclusion
and the exact finite-state supply are not proved \\
...
\begin{lemma}[a boundary-separated source rail segment is an exact shore]
\label{lem:source-rail-segment-exact-shore}
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24FramedCorridorRailChainComponent.exists_alignedBoundary_component_exactBoundary}
...
are disjoint, and that neither rail visits the centre face of either endpoint
interface.  Close the two rails by the transverse two-edge paths at the
...
This lemma is not yet Proposition~\ref{prop:target-forced-cut-chain}.  It
constructs a saturated vertex-side boundary for one three-interface segment.
The remaining source adapter must compose rails over arbitrary length, prove
the required noncollision uniformly, convert the successive bounded cycles
to connected complementary \emph{edge} shores, choose those shores
consistently so that they are strictly nested, and then provide the roots and
uniform width bounds consumed by
