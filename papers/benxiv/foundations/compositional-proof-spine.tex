...
obstruction & open; the signed curvature identity alone does not imply a
clean hexagonal corridor, but the bounded-face/long-face dichotomy below
reduces the geometry to two explicit transfer constructions \\
...
Thus the marked face containing the special edges of the $A\ne\varnothing$
system, the cap faces, and any fixed collar can be put into $S$ at an explicit
multiplicative cost.  The source sentence that the defect budget ``localizes''
the marked edges is unnecessary in this branch; bounded-degree dual
connectivity gives the required avoidance directly.
...
The bounded-memory assertion in that last sentence is not merely heuristic.
...
Hence the receipt-enlargement process terminates structurally in the
bounded-face branch: retaining a fixed radius-two axis window and the
radius-one side faces already contains every face that can cross the next
cut.  No prefix-length-dependent field is legitimate.  What remains for this
branch is to instantiate the existing rooted Count factorization at the
$B$-bounded cell alphabet and prove its physical splice, rather than inventing
successively larger receipts.
...
of the following source-preserving constructions.
\begin{enumerate}
...
Theorem~\ref{thm:dual-diameter-dichotomy} proves that these two constructions
are exhaustive.  This is a sharper and more plausible research front than
Proposition~\ref{prop:uniform-bulk-hypothesis}: the high-negative-curvature
branch is not discarded, but converted into either bounded nonhexagonal
letters or a long boundary on which the same finite-interface idea can act.

The finite-state part of the long-boundary branch is also elementary once the
planar connectivity is written as noncrossing pairings.
...
For the route, the two Kempe families give the matching stacks and capped
face progress supplies additional finite labels.  In the shallow case an
equal sweep state is the length splice interface.  In the deep case the
nested arcs provide layer boundaries; applying the same pigeonhole argument
to finite layer profiles gives the depth splice.  What is still load-bearing
is geometric realization: the chosen cuts/layers must be simple ordered
transversals, and deleting between equal states must preserve the target
count and the map class.  Lemma~\ref{lem:noncrossing-length-depth} proves the
classification, not that physical splice.
...
bookkeeping; either Proposition~\ref{prop:uniform-bulk-hypothesis} or both
branches of Proposition~\ref{prop:global-corridor-alternative}; the exact
