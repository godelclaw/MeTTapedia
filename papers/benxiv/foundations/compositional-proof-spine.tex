...
the relevant refined counts to the pentagon frontier & the local square
fibre and additive profile identity are checked; the graph-level Seed-count
adapter and the separate $\mathrm{WC}^{*}$ migration theorem remain open \\
...
$T_4$ & Square migration between the two reduction sides for full
$\mathrm{WC}^{*}$ reachability & open; the proposed universal shortcut
$A\ne\varnothing$ is refuted in Proposition~\ref{prop:a-nonempty-refuted} \\
...
\Sconditional{Tait bridge; Trail Completability; ladder transport; Seed Lemma; square migration; global corridor alternative; physical splice; finite base}
...
migration, factor-completion lifting, and the final
