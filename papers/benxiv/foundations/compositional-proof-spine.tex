...
literal checked consumer are machine-checked.  Width-preserving
connectedization is isolated as the named external theorem it actually is;
wall exclusion and the arbitrary-length nested exact finite-state supply are
not proved \\
...
doi:10.1007/BF01215352).  A more direct supplier is available here.
Fomin, Fraigniaud and Thilikos (The price of connectedness in expansions,
Technical Report LSI--04--28--R, 2004,
\url{https://hdl.handle.net/2117/98590}, Theorem~1) prove exactly that,
given a width-$w$ branch decomposition of a \(2\)-edge-connected graph,
their algorithm \emph{Make-it-Connected} returns a connected branch
decomposition of width at most $w$.  Here ``connected'' means that both sets
of edge leaves at every tree cut induce connected subgraphs; there is no
hidden factor in the width.  The finite simple-graph specialization of this
statement is the explicit proposition
\texttt{WidthPreservingConnectedization}.  It is not an axiom and is not
claimed kernel-checked here: callers must supply the named external theorem.
...
\Sconditional{planar wall theorem; fixed-wall exclusion; width-preserving connectedization; route-native finite base}
...
\item the Fomin--Fraigniaud--Thilikos width-preserving connectedization
theorem is available in the finite simple-graph specialization above.
\end{enumerate}
...
width-$w$ tree and Hypothesis~3 connectedizes it without increasing the
width.  The checked standard-to-consumer adapter supplies the complete
connected edge-leaf decomposition.  Theorem~\ref{thm:rooted-connected-branch-decomposition}
...
\begin{proposition}[unrestricted target-aware pairing reduction fails]
\label{prop:hexagon-target-aware-pairing-obstruction}
\Srefuted{Mettapedia.GraphTheory.FourColor.GoertzelV24HexagonPairingTargetAwareBoundary.no_unrestricted_targetAware_planarPairing_reduction}
...
caps.  It is false that
\[
  \forall E\subseteq U,\quad E\cap P=\varnothing
  \Longrightarrow
  \exists i\in\{0,\ldots,4\},\quad E\cap R_i=\varnothing.
\]
Thus no rule which chooses one of the five caps after seeing an arbitrary
zero exterior support can replace the hexagon while preserving zero Count.
\end{proposition}
...
Consequently \(R_i\nsubseteq P\) for every \(i\).
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24HexagonPairingTargetAwareBoundary.no_planarPairingSupport_subset}

Now take the maximal abstract zero exterior \(E=U\setminus P\).  It is
disjoint from \(P\), while \(R_i\nsubseteq P\) says exactly that \(E\) meets
every \(R_i\).  Hence no cap works against this one exterior.  Lean records
both the explicit complement adversary and the displayed quantified
negation.
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24HexagonPairingTargetAwareBoundary.complement_hexagonSupport_meets_every_planarPairing}
\end{proof}

The word ``unrestricted'' is load-bearing.  We have not shown that
\(U\setminus P\), or any equally strong hitting support, is realized by a
planar cubic exterior.  The same obstruction applies to a proposed physical
exterior class if it contains \(U\setminus P\):
...
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24SphericalReductiveAssembly.combinatorialFourColorStatement_of_raw_connectedization_and_base}
...
most $w$.  Suppose also the width-preserving connectedization proposition
of Fomin--Fraigniaud--Thilikos, and suppose every graph-backed bridgeless
spherical cubic map with at most $B$ vertices is Tait colourable.  Then every
finite combinatorially planar simple graph is vertex-four-colourable.
\end{corollary}

\begin{proof}
A least counterexample is connected and bridge-free, hence
$2$-edge-connected.  Apply the supplied connectedization theorem to the raw
width-$w$ tree, then apply the checked rooted adapter to obtain the exact
decomposition supply of Theorem~\ref{thm:spherical-reductive-assembly} with
$k=w$.  That theorem and the bounded base give the conclusion.  Lean checks
this composition while leaving both genuine mathematical inputs---the raw
width bound and the external connectedization theorem---visible.
\end{proof}

\begin{corollary}[wall-exclusion assembly]
\label{cor:wall-exclusion-assembly}
\Sconditional{abstract-planar representation theorem; fixed-wall exclusion for least counterexamples; bounded-width decomposition; width-preserving connectedization; route-native finite base}
...
width at most $w$.  Assume the Fomin--Fraigniaud--Thilikos connectedization
theorem and that the resulting route-native finite base contains no
counterexample.  Then every planar graph is four-colourable.
\end{corollary}
...
Corollary~\ref{cor:factored-bounded-width-assembly}.  The connectedization
theorem converts it to the literal decomposition supply, and the finite-base
hypothesis is the remaining premise.  That corollary gives the spherical Four-Colour
...
exclusion plus a raw bounded-width decomposition and the named
width-preserving connectedization theorem.  Both chains
...
bounded-width decomposition supplier and width-preserving connectedization;
and the route-native finite
