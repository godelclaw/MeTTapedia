...
This witness has boundary colour multiplicities $(3,2,0)$, not the good-word
type $(3,1,1)$.  It therefore refutes the universal statement applied to the
source's census specimens, but does not refute a new theorem explicitly
restricted to good-word closed webs.  That distinction must remain visible
in every downstream use.
...
The surviving source-facing engine is innermost peeling.  For a minimum-span
chord the prohibited nested case is absent, and the checked theorem supplies
uniform drainage,
...
After peeling that disk, the next proof obligation is not universal sector
alternation.  It is a quantitative dichotomy: either one peeled band has a
long gap yielding a shallow pumpable corridor, or many bounded gaps expose a
fixed-width physical interface whose profile must repeat.  The observed
depth-three census bound is evidence for this re-aim, not a general theorem.
No finite-state pumping step may use it until that uniform interface theorem
is constructed.
