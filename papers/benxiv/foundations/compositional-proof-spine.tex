...
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24ResidualReturnPairing.exists_exchangeRigid_with_facialFormation_and_returnPairing_at_every_globalMeshStep}
...
third-edge excursions when they return to the alternating cycle.  No
noncrossing order or formula for the number of resulting components is
asserted here.
\end{corollary}
...
asserted unchanged residual path.
\end{proof}
...
They do not yet put those returns in a noncrossing cyclic order, synchronize
the pairings at neighbouring sites, or compute the resulting change in the
number of odd residual components.
