...
What remains geometric is only noncrossing: in a disk embedding, distinct
two-ended bichromatic components must induce a noncrossing matching in the
cyclic port order.  `PortTangle` deliberately forgets the rotation and disk,
so that conclusion cannot follow at this carrier level.  More importantly,
even the full noncrossing bridge would not rescue the reduction, because the
