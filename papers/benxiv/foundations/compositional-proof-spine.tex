...
from \(C\) to \(C'\).  The last assertion is the contrapositive.
\end{proof}
...
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24AdjacentPairCommonCoreKempeBoundary.firstOrbitReachesSecondPair_or_exists_liftedColoring}
Let \(D_{uv}\) be the graph obtained by deleting an adjacent pair
\((u,v)\), and let \(H=D_{uv}-\{x,y\}\) be its common core with the deletion
of another adjacent pair \((x,y)\).  Let \(C\) be an edge colouring of
\(D_{uv}\), and suppose that an edge colouring \(C^*\) of \(H\) is
Kempe-reachable from \(C|_H\).  Then one of the following holds.
\begin{enumerate}
...
\item There is a colouring \(\widetilde C\) in the Kempe closure of \(C\)
      such that \(\widetilde C|_H=C^*\).
\end{enumerate}
\end{lemma}
...
orbit with the required final restriction.  Finally, an edge of \(D_{uv}\)
...
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24AdjacentPairCommonCoreDisagreementResidue.branching_or_reachesSecondPair_or_strictRepair}
Let \(C_{uv}\) and \(C_{xy}\) colour the two adjacent-pair deletion graphs,
...
\item At an intermediate representative in the Kempe orbit of \(C_{uv}\),
      a common-core Kempe component reaches an edge incident with \(x\) or
      \(y\).
\item There is a colouring \(\widetilde C_{uv}\ne C_{uv}\) in the Kempe
      closure of \(C_{uv}\) whose restriction to \(H\) equals
      \(C_{xy}|_H\).
\end{enumerate}
\end{corollary}

\begin{proof}
If \(C_{xy}|_H\) is not Kempe-reachable from \(C_{uv}|_H\), apply
...
exclusion: repairing one pair can change the repaired colouring's
restrictions at other mesh sites.  The remaining two-dimensional task is
therefore to show that repeated repairs either terminate in a coherent
atlas, which is impossible by
Lemma~\ref{lem:global-common-core-colouring-atlas}, or force branching or
boundary-reaching witnesses with enough geometric coherence to support a
strict replacement.  No monotonicity of the naive number of disagreeing
pairs is assumed.
