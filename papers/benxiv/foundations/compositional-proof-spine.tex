...
Substituting this literal state into
Theorem~\ref{thm:cardinality-phased-shore-descent} removes the last abstract
seam-type parameter.  For cuts of width at most \(k\), the unphased state
count is
...
and the cardinality phase gives the explicit bound
...
This exact specialization is machine-checked as
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24NormalizedShoreDescent.vertexCount_le_of_cardPhasedNormalizedShore}.
Its former application-specific replacement premise is discharged by
Theorem~\ref{thm:majority-shore-physical-replacement}.  The supplier must now
produce exactly that theorem's literal hypotheses: nested connected shores,
two genuine boundary ports, equal normalized states, and a cubic star wholly
inside the intervening edge slab.

This theorem closes the finite counting once the supply exists.  In
particular, the Lean declaration does \emph{not} prove that a branch decomposition gives
ordered port tangles, that the local perturbations of its nooses are mutually
compatible with the retained vertex shores, or that the resulting ordered
ports realize the same exact Count state.  Once those literal data are
present, however, both semantic preservation and the spherical bridgeless
cubic replacement are verified by
