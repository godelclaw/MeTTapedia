...
In the cyclically five-edge-connected normal form, two distinct faces share
at most one edge.  Consequently the bounded-face branch is a genuine chain
of cells drawn from a finite alphabet depending only on $B$: each central
face has at most $B$ sides, consecutive cells share one edge, and the
geodesic has no nonconsecutive face adjacency.  The existing hexagonal Cell
is the $B=6$, all-central-faces-hexagonal subalphabet, not the only possible
finite alphabet.
