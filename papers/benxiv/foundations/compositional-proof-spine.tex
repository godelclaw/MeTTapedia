...
\Sopen
For each relevant corridor type, produce finite arrays $I$ of
manuscript-exact two-pair initial profiles and $E$ of directed profile edges
such that
\[
...
and
\[
 (q,q')\in E\quad\Longleftrightarrow\quad
 (q,q')\text{ is produced by a compatible positive prefix/Cell witness}.
\]
Prove both biconditionals in the kernel, replay breadth-first reachability from
$I$ through $E$, and compare the resulting state-set fingerprint and count
with an independently implemented generator.  Then, and only then, the
replayed array length is the exact reachable source-profile count for that
corridor type.
\end{proposition}
