...
objects.  The external attachment and the two repeated-state outcomes remain
explicit; in particular, equality of the present unlabelled stack shapes is
not asserted to be a colour-preserving splice interface.
...
Thus a sufficiently long ambient return yields an external attachment, a
strictly nested physical chord family, or a materially spaced raw-state
repeat whose two actual cut boundaries share one graph-independent bound.
...
This is the correct input boundary for a second, semantic compression by
Count profiles.  It is not itself that compression: corresponding open-wire
slots have not yet been proved to carry equal Tait-colouring behaviour.
\end{proof}
...
that same interval.  A long ambient
...
attachment/repetition alternatives, obtain a uniform bound for the realized
deep-return states, choose compatible interfaces coherently across
neighbouring mesh sites, and prove that the cumulative zero-count profile
factors through the resulting ordered interfaces.  The corollary is therefore
a graph-level local alternative at one alternating site; it proves neither a
coherent mesh-level family nor the splice.
