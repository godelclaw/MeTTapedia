...
majority vertex sides are nonempty, and their middle set has size at most
both \(k\) and \(w\).
\end{enumerate}
Set
\[
...
The theorem above still receives the resulting nonemptiness facts as node
fields; only the elementary propagation of the two-edge cardinalities through
the rooted leaf tree remains prose.  Lemma~\ref{lem:connected-shore-literal-node}
then supplies the roots, literal width, and two-port fields.
...
\Sconditional{planar wall theorem; connected branch-decomposition supply; route-native finite base}
...
most \(w\);
\item every width-\(w\) instance in the class has a complete rooted
edge-leaf decomposition whose two nondegenerate subtrees satisfy the
nodewise connected-shore, majority-nonemptiness, and middle-set hypotheses
of Theorem~\ref{thm:rooted-connected-branch-decomposition}, with \(k=w\).
\end{enumerate}
...
bound and Hypothesis~3 supplies the complete connected edge-leaf
...
\Sconditional{Tait bridge; fixed-wall exclusion for least counterexamples; connected branch-decomposition supply; route-native finite base}
...
of the normal-form class carry the complete connected edge-leaf decomposition
required by Theorem~\ref{thm:rooted-connected-branch-decomposition}.  If the resulting
...
exclusion plus a connected branch-decomposition supply.  Both chains
...
connected branch-decomposition supplier; and the route-native finite
