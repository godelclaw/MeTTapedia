...
\begin{corollary}[strictly nested residual returns give a bounded cyclic cut]
...
\Sverified{Mettapedia.GraphTheory.FourColor.Compositional.ResidualReturnSweepCyclicCut.hasBoundedCyclicReturnCut_of_nested_openArcs}
\Sverified{Mettapedia.GraphTheory.FourColor.Compositional.ResidualReturnSweepCyclicCut.hasBoundedCyclicReturnCut_of_deep_return_shore}
\Sverified{Mettapedia.GraphTheory.FourColor.Compositional.ResidualReturnSweepCyclicCut.hasBoundedCyclicReturnCut_or_spaced_eq_rawState}
Fix a requested spacing $s\geq0$.  If a proper alternating site has $n$
cyclic positions and
\[
                              n>8(s+1),
\]
then either the site has a bounded cyclic return cut, or there are positions
$i<j$ with $j-i\geq s+1$ carrying the same phased two-shore sweep state.
Here a bounded cyclic return cut is obtained from two strictly nested physical
returns on one facial shore; its crossed-edge set has cardinality at most
three times the number of vertices in the inner return separator.
\end{corollary}
...
therefore supplies the bounded cyclic edge cut.  Finally,
$2(s+1)(1+1)^2=8(s+1)$.
\end{proof}
...
\Sverified{Mettapedia.GraphTheory.FourColor.Compositional.MeshResidualReturnSweep.GeometricSweepReceipt.ofTwoSector}
...
requested spacing.  Moreover, at each internal row junction at least one of
the immediately incoming and outgoing arms carries this alternative.
\end{corollary}
...
The conclusion is pointwise.  It does not choose the cut horn or the repeated
state horn uniformly, nor does it assert that choices made at adjacent
junctions define nested or disjoint shores.
\end{proof}
...
case the nested records now produce a literal bounded cyclic edge cut, rather
than merely candidate layer boundaries.  One still has to choose such cuts
coherently across neighbouring mesh sites and prove that the cumulative
zero-count profile factors through the resulting ordered interfaces.  The
corollary is therefore a graph-level local dichotomy at one alternating site;
it proves neither a coherent mesh-level family nor the splice.
