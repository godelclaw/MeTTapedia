...
$D_2$ & Literal one-cell Count factors through an exact finite labelled
transition and executable reachable closure & the raw bounded-interface Count
law and support determinization are proved; the current Cell--3 Lean theorem
still requires a special closed-web witness and global (U), so
colouring-independent geometric fields, target-specific acceptance,
executable initials, closure, and minimization remain open \\
...
The graph-level restriction/gluing bijection (C1) is still the prose seam.
Conditional on that one equivalence, its cardinal convolution is
machine-checked.
...
\Sconditional{Tait bridge; ladder transport; target-forced cut chain; exact labelled Count closure; simple-map seam adapter; route-native finite base}
...
Assume the literal Count of each physical Cell factors exactly through the
labelled finite system of Proposition~\ref{prop:exact-closure-gate}.  If the
implementation represents maps by simple graphs, also prove that the common
...
closure after removing its false global face-uniqueness hypothesis, the
rotation-system seam constructor and simple-map adapter needed to implement
the proved ordinary physical splice, and the route-native base.  The conditional finite
transition organizes the closure premise; it cannot yet be instantiated on
the source annulus.  The target-specific square seam is no longer one of
these premises: capped colourability rules out a target-free reduction side
before any migration is needed.  In particular, neither the signed curvature
identity nor the bounded-face geodesic theorem currently proves
