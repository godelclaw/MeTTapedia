...
The generic finite-tree descent and the mesh obstruction are machine-checked,
but the sphere-cut instantiation, physical splice, and wall exclusion are not \\
...
multigraph adapter, and strict vertex accounting are machine-checked.  The
exact rotation-system composition, orientation-reversed seam match,
sphericity, and simple-map closure remain open \\
...
The following is the precise statement currently proved about bounded
branchwidth.  It is a theorem about a supplied sphere-cut decomposition, not
yet a theorem constructing that supply from a plane map.

\begin{theorem}[typed sphere-cut descent]
\label{thm:typed-sphere-cut-descent}
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24SphereCutDescent.vertexCount_le_of_supply'}
Fix \(w\).  Suppose an instance \(X\) is supplied with a rooted binary
decomposition whose nodes have states
\[
  (j,s,S),\qquad 0\le j\le 2w,\quad
...
\[
 ...
 q=\sum_{j=0}^{2w}|\mathrm{SeamType}_j|\,2^{3^j}.
\]
\end{theorem}
...
This theorem closes the finite counting once the supply exists.  Its second
hypothesis still contains the geometric content of D3.  In particular, the
Lean declaration does \emph{not} prove that a branch decomposition gives
...
ports realize the same exact Count state.  The spherical bridgeless cubic
packaging of any two sides satisfying those interface conditions is now
verified by Corollary~\ref{cor:implemented-seam-composite-structure}.
...
doi:10.1007/BF01215352).  Neither
theorem is formalized in this development.  Moreover, a sphere cut meets
the graph in its middle \emph{vertices}, whereas the present Count functor
cuts ordered \emph{edges}.  Lemma~\ref{lem:cubic-noose-edge-perturbation}
proves the local conversion and the factor \(2\): a tight noose through
\(w\) cubic middle vertices becomes a tight transverse noose with at most
\(2w\) crossings.  What the cited sphere-cut theorem does not by itself
package in the present types is a compatible choice of retained vertex
shores under those local perturbations and the resulting ordered
\texttt{OpenTangleData}.  Lemma~\ref{lem:cubic-sphere-cut-material} and the
depth phase above settle strict-material accounting.  The carrier and
boundary-order construction remains mathematics to prove, not a citation.

\begin{proposition}[wall-exclusion reduction, relative form]
\label{prop:target-forced-wall}
\Sconditional{planar wall theorem; sphere-cut supply; exact D3 replacement; route-native finite base}
...
most \(w\);
\item every width-\(w\) instance in the class has a sphere-cut supply of the
precise kind consumed by Theorem~\ref{thm:typed-sphere-cut-descent}; and
\item the supply's equal-state replacement is the D3 physical splice and
stays inside the normal-form class.
\end{enumerate}
...
bound, Hypothesis~3 constructs the decomposition, and Hypothesis~4 supplies
its strict replacement.  Theorem~\ref{thm:typed-sphere-cut-descent} gives the
effective vertex bound.  An empty finite base rules out \(X\).
\end{proof}
...
which is face-simplicity.  Finally, the two boundary orientations induced on
the common boundary of the two discs are opposite.  Reading the same crossed
edge from the other side therefore conjugates one cyclic order to the
inverse of the other, which is equation~(25).
\end{proof}

The adjective ``tight'' is load-bearing.  A simple closed curve can enter the
same face several times, in which case one capped face can pass through the
hub more than once and \texttt{HubFacesDistinct} need not hold.  Standard
sphere-cut decompositions of connected bridgeless plane graphs use tight
nooses (each face is visited at most once).  The remaining sphere-cut adapter
is therefore not Jordan separation itself; it is the explicit, nested local
perturbation from those vertex-meeting tight nooses to the edge-crossing
tight nooses consumed here.
...
avoid vertices, are tight, and each cross exactly \(k\ge2\) edges.  Suppose their ordered
...
\begin{proof}
Retain the disc side of the inner noose and the complementary disc side of
the outer noose.  Lemma~\ref{lem:tight-noose-cap-data} supplies spherical,
face-simple hub closures and opposite boundary rotations.  Hence
...
The bare matching type cannot manufacture those witnesses; they come from
the tight noose by Lemma~\ref{lem:tight-noose-cap-data}.
...
multigraph which is not Tait-colourable.  Repeatedly suppress a parallel pair
by the digon splice of Lemma~\ref{lem:digon-splice}.  The process terminates
in a strictly smaller connected bridgeless cubic spherical simple graph
which is still not Tait-colourable.
\end{corollary}
...
loop.  A Tait colouring of the suppressed graph lifts: colour the two
external replacement paths by the colour of the new edge and colour the
parallel pair by the other two colours.  Consequently uncolourability is
preserved contrapositively.  Each suppression removes two vertices, so
...
link is the construction of the tight transverse cut data from the chosen
sphere-cut decomposition, not a missing face-count theorem.

\begin{theorem}[direct Count-reductive assembly]
\label{thm:direct-count-reductive-assembly}
\Sconditional{Tait bridge; ladder transport; target-forced tight cut chain; physical D3 supply; route-native finite base}
...
\Sconditional{Tait bridge; fixed-wall exclusion for least counterexamples; sphere-cut supply; D3 replacement; route-native finite base}
...
of the normal-form class carry the exact sphere-cut supply of
Theorem~\ref{thm:typed-sphere-cut-descent}, and that equal supplied states
admit the D3 physical replacement inside that class.  If the resulting
...
literal composite.  Lemma~\ref{lem:tight-noose-cap-data}
derives their cap hypotheses from a tight transverse noose.  The literal
...
the production of compatible tight transverse nooses from the supplied
vertex-based sphere cuts.
...
exclusion plus an exact sphere-cut supply.  Both chains use D3 and the same
finite base.
...
sphere-cut adapters; the compatible tight vertex-noose to edge-noose
perturbation; and the route-native finite base.  D3's face count, Euler
arithmetic, and loopless-multigraph normalization are no longer open.  The mesh
