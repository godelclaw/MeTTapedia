...
What remains is the transport itself: the face identification, which is the
edge product of an involution's edge list composed with the rotation; the
edge count, which is twice the edge-list length; and the two counting
bijections above. Until that transport is written, the carrier is finite and
exactly interpreted but is not yet known to retain every required source
prefix. No closure count, threshold or generating base is claimed, and the
earned ledger remains $57/100$.
...
This supplies $F$ for the direct Count system of
Proposition~\ref{prop:exact-closure-gate}. It does not supply $I$, $E$, the
reachability replays or any count, and it is not the Lift/Seed accepting
array, which remains the cap-composed predicate.
