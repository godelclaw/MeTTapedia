...
projected, and its finite letter semantics has been separated from the global
closed-web colouring.  This does not yet prove the full theorem above.  The
prefix-colour function in the transition API still has the ambient edge set
as its domain, even though the proof reads only selected coordinates; the
realizable initial image is separately prefix-local but still sits under the
surrounding closed-web parameter.  The source collar or three-Cell word must
therefore be made into a formation directly, the ambient-domain prefix API
must be restricted or proved representation-invariant, and the resulting
realized transitions and initials must be connected soundly and completely to
the cumulative Count semantics.  Global opened-annulus face uniqueness is no
longer an obligation of either rolling transition.
...
formation.  What remains is narrower than whole-annulus coverage but still
substantive: construct that formation directly from the radius-$1$ collar or
three-Cell word actually selected by the source, perform the same domain
restriction for the transition's ambient prefix-colour coordinate, and
identify the cumulative terminal projection with the Seed semantics consumed
at the local target.  No further equivalence between vertex-component and
edge-component semantics is owed.  The filtered finite accepting set has also
not yet been emitted as an optimized executable certificate.
