...
Lemma~\ref{lem:five-cycle-class-support-semantics}.  The exact five-cut's
single cyclic boundary order and opposite shore orientation are now checked by
Lemma~\ref{lem:exact-five-cut-cyclic-boundary}.  Literal physical Kempe
components are now known to pair the five boundary ports noncrossingly by
Lemma~\ref{lem:exact-five-cut-physical-noncrossing}.  The remaining physical
work is to derive the two displayed class-closure implications from those
noncrossing components and smaller-side surgery, prove that minimality supplies
a primary class on each shore, and instantiate the existing exact-support splice.  No finite
boundary-word certificate is used or licensed by these lemmas.
