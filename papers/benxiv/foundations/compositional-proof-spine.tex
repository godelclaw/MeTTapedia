...
which the proved monotonicity of the edge-presentation defect applies. Only
the vertex identification needs a genuinely new invariant, namely that the
stored rotation is one cycle per vertex on a constructed run.

Until that bridge exists, the carrier above is finite and exactly interpreted
but is not yet known to retain every required source prefix. No closure
count, threshold or generating base is claimed, and the earned ledger remains
$57/100$.
