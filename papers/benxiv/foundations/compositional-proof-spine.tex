...
the relevant refined counts to the pentagon frontier & substantial parts
proved; the square branch still consumes $A\ne\varnothing$ and special-edge
transport \\
...
$T_4$ & $A\ne\varnothing$: some colouring makes two marked cofacial,
nonadjacent edges equal-coloured & open; reduced to the same corridor engine,
special-edge transport, and a finite base \\
...
$T_8$ & Route-native finite bases for the Seed and $A\ne\varnothing$ systems
& open; the earlier GWCO base cannot be inherited \\
\bottomrule
\end{tabular}
\end{center}
...
We isolate the exact ordinary-mathematics theorem used by both the Seed Lemma
and $A\ne\varnothing$.
...
\Sopen
For each fixed boundary type used by the Seed and $A\ne\varnothing$ systems,
...
Thus, within the bounded-face branch, the marked face containing the special
edges of the $A\ne\varnothing$ system, the cap faces, and any fixed collar can
be put into $S$ at an explicit multiplicative cost.  The source sentence that
the defect budget ``localizes'' the marked edges is unnecessary in this
branch; bounded-degree dual connectivity gives the required avoidance
directly.
...
this cannot happen in a least Seed or $A\ne\varnothing$ obstruction, or
...
\Sopen
For each fixed boundary type and for each of the two target systems (Seed and
$A\ne\varnothing$), there are computable constants \(b,m,V_0\) with the
...
\item the designated holes, special edges, and frozen boundary lie outside
the slabs;
...
\Sconditional{Tait bridge; Trail Completability; ladder transport; Seed Lemma; $A\ne\varnothing$; global corridor alternative; physical splice; finite bases}
...
assembly; all four ladder transports including special-edge
bookkeeping; Proposition~\ref{prop:target-forced-cut-chain} (for example via
...
corridor type; and empty route-native finite bases for the Seed and
$A\ne\varnothing$ systems.  Then every planar graph is four-colourable.
\end{theorem}
...
physical splice instantiate Theorem~\ref{thm:finite-profile-reductive-pumping} for the
Seed and $A\ne\varnothing$ counts.  Their empty finite bases therefore prove
both counts positive.  The source's Lift, word-graph, and propagation
...
the two route-native bases, the Seed and
$A\ne\varnothing$ instantiations, factor-completion lifting, and the final
