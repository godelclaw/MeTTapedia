...
not yet prove wall exclusion: bounded edge-disagreement is now verified, but
the remaining adapter must dispose of the branch in which the minimizer uses
one or both selected central edges.  In the proper-component branch it must
still compare the distinguished components, rather than merely their ambient
alternating graphs, and then use the mesh geometry to force a strict
compositional descent.
\end{proof}
