...
$a$ or $c$.  If the retained endpoints of $b$ and $d$ are distinct, the two
components cannot be distinct.  Thus two nondegenerate physical component
trails cannot join alternating pairs of boundary ports.
...
The $b$--$d$ component trail is nonempty because its retained endpoints are
distinct, and it is edge-disjoint from $Q$.  At either endpoint,
cubic cyclic rotation identifies the boundary-side face label with the label
beside the first or last trail dart; along the trail the label cannot change,
because none of its edges lies in $Q$.  Hence its endpoint faces have equal
labels, contradicting the preceding paragraph.  Distinct bichromatic
components are vertex-disjoint and therefore edge-disjoint, while
...
nondegenerate noncrossing conclusion.  A component whose two boundary ports
are based at one retained vertex is not covered by this separator argument;
showing directly that such a pair is boundary-local is the remaining small
degenerate case before promoting the conclusion to an unrestricted matching
statement.
\end{proof}
