...
replacement now preserves zero Count on that same physical composite.  What
remains is to derive equal seam supports from equal decomposition states and
to formalize the already-written parallel-seam normalization \\
...
supply the two disc-side hypotheses for the replacement piece; no further
Count semantics or hidden orientation choice remains.
...
Its sole application-specific premise is now transparent: equal normalized
states at two nested majority shores, together with the strict slab vertex,
must produce a smaller instance in the same target class.
...
\Sconditional{planar wall theorem; connected branch-decomposition supply; exact Count replacement; route-native finite base}
...
states consumed by Theorem~\ref{thm:typed-sphere-cut-descent}; and
\item equality of two supplied states gives exact Count replacement, and
the resulting majority-shore splice stays inside the normal-form class after
the permitted normalization.
\end{enumerate}
...
shores, and Hypothesis~4 supplies exact strict replacement.  The structural
part of that splice is the checked implication following
...
the structural constructor, and perform the stated normalization if the seam
creates a parallel pair.  The cap face count, cap sphericity,
...
\Sconditional{Tait bridge; fixed-wall exclusion for least counterexamples; connected branch-decomposition supply; exact Count replacement; route-native finite base}
...
admit exact Count replacement inside that class.  If the resulting
...
connected branch-decomposition state adapter; exact Count replacement; and
the route-native finite base.  A compatible tight vertex-noose to edge-noose
