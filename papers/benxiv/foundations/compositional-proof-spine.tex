...
\subsection{Load-bearing dependency audit after retirement of the closed-web branch}

This spine no longer assigns a scalar completion percentage.  The former
scalar score prevented file counts from masquerading as proof progress, but
its all-or-nothing arithmetic erased useful intermediate state changes and
eventually distorted scheduling.  It was an audit convention, not mathematics
from the source and not a proposition consumed by Lean.

The named nodes are retained only as stable addresses for theorem interfaces.
Their current dependency status is:

\begin{center}
\small
\begin{tabular}{@{}p{0.12\textwidth}p{0.62\textwidth}p{0.14\textwidth}@{}}
\toprule
Node & Obligation & Status \\
\midrule
$D_0$ & spherical Tait bridge and least-counterexample normal form & verified \\
$D_{1a}$ & generic mesh, connected decomposition, cuts, and state supply & verified \\
$D_{1b}$ & target-forced compositional cut chain with computable constants & open alternative \\
$D_{1c}$ & bounded-face geodesic and long-face perimeter transfers & open alternative \\
$D_{1d}$ & planar wall reduction and fixed ordered-mesh exclusion & open, direct \\
$D_{2a}$ & raw Count semantics, finite profiles, composition, and pumping & verified \\
$D_{2b}$ & executable optimized closure certificate & provisional tool path \\
$D_3$ & physical splice, class closure, strict decrease, and normalization & verified \\
$D_{4a}$ & route-native finite-base reflection interface & verified \\
$D_{4b}$ & concrete trace, coverage, adequacy, and empty finite-base audit & open, direct \\
$D_5$ & headline assembly from its discharged premises & verified conditional \\
\bottomrule
\end{tabular}
\end{center}

The $D_{4b}$ premise has been weakened at its joint with $D_{1d}$.  It was
stated as colourability of every graph-backed bridgeless spherical cubic map
below the checked vertex threshold, with no width hypothesis.  That threshold
is a tower, so the premise ranged over maps of arbitrarily large interface
width, while the route's instrument is an interface closure at width $w$.  At
the point of use the minimal counterexample already carries the supply's
rooted connected decomposition, so handing that decomposition to the base
costs nothing.  The restricted premise retains every hypothesis of the old one
and adds the decomposition, hence is weaker, and the earlier headlines are
recovered from it.
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24DecomposableFiniteBase.taitBaseVerifiedOnDecomposable_of_taitBaseVerifiedAt}
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24DecomposableFiniteBase.noGraphBackedVertexMinimalTaitCounterexample_of_supply_and_decomposableBase}
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24DecomposableFiniteBase.combinatorialFourColorStatement_of_raw_and_decomposableBase}
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24DecomposableFiniteBase.combinatorialFourColorStatement_of_raw_and_base'}

The restriction is not vacuous.  The width parameter bounds the branch middle
set, and exact branchwidth of small bridgeless planar cubic maps already
reaches four at eight vertices, so for each fixed $w$ there are maps far below
the vertex threshold that the base is no longer asked about.

The consequence for the reflection joint is the operative one.  The earlier
reflection extracted its finite state from the bare map, so an instantiation
had to manufacture an interface from an arbitrary map below the threshold —
the one step a fixed-width closure cannot take.  The decomposition-aware
reflection takes the decomposition as an input to its trace, so a width-$w$
sweep along that decomposition is a legal instantiation.
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24DecomposableFiniteBase.DecomposableBaseReflection.taitBaseVerifiedOnDecomposable_of_filter_eq_empty}
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24DecomposableFiniteBase.DecomposableBaseReflection.combinatorialFourColorStatement_of_raw_and_filter_audit}

Neither coverage, adequacy, nor emptiness is supplied here; $D_{4b}$ remains
open, with a strictly smaller obligation.

Sharpening that obligation immediately refutes one candidate instrument for
it.  The width supply delivers a branch decomposition, which is a tree of edge
bipartitions.  The physical finite-state closure instead processes vertices in
a line, and its width is the number of edges leaving a prefix of that order.
Writing $\mathrm{BW}$ for the minimum over branch decompositions of the
largest middle-vertex set and $\mathrm{LP}$ for the minimum over vertex
orderings of the largest leaving-edge set, always $\mathrm{LP}\ge\mathrm{BW}$.
The two are not equivalent parameters.

\begin{proposition}[a bounded branch decomposition does not bound the linear
interface]
\label{prop:tree-vs-linear-width-refuted}
\Srefuted{exhaustive tube-tree branch/linear width gate, tools/fourcolor/v24\_tree\_vs\_linear\_width\_gate.py}
There is a family of simple, cubic, planar, bridgeless maps carrying explicit
branch decompositions of width four at every size, whose exact linear-sweep
width is unbounded.
\end{proposition}

The witness is a tube tree: hexagon nodes joined in a tree by two-edge tubes.
Every member is simple, cubic, planar and bridgeless, hence two-edge
connected, so it lies in the class the base premise quantifies over and the
checked width-preserving connectedization applies to it.  Only
\texttt{WidthAtMost} is asked of the raw supply, so the explicit decomposition
below is exactly what $D_{1d}$ would hand over.

\begin{center}
\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
family & vertices & $\mathrm{BW}\le$ & $\mathrm{LP}$ & proved $\mathrm{LP}>$\\
\midrule
prism $C_4\times K_2$ & 8 & --- & 5 & 4\\
prism $C_{10}\times K_2$ & 20 & --- & 5 & 4\\
tube tree, depth 1 & 24 & 4 & 6 & 5\\
tube tree, depth 2 & 60 & 4 & 6 & 5\\
tube tree, depth 3 & 132 & 4 & 8 & 7\\
tube tree, depth 6 & 1140 & 4 & --- & ---\\
\bottomrule
\end{tabular}
\end{center}

The branch width is certified by exhibiting one decomposition and measuring
it; the linear width is exact, since a prefix search that exhausts at $k$
proves $\mathrm{LP}>k$.  The prism control keeps both parameters bounded, so
the measurement separates families rather than reporting growth everywhere.

The consequence is that a fixed-width linear closure cannot be the instrument
for $D_{4b}$.  Already at twenty-four vertices a map the supply delivers at
width four forces a linear interface of width six, and the gap grows with the
tree depth while the supplied width does not move.  The corrected obligation
is a closure over the branch decomposition itself: states are interfaces of at
most $w$ middle vertices, and the transition is the binary fork that merges
two child interfaces, not the attachment of one vertex to a frontier.  Pumping
survives the change, since a path in the decomposition tree that revisits a
state is spliceable exactly as a repeated corridor profile is.

The measured width-three and width-four closures are therefore calibration of
the vertex-attachment operation, not partial progress toward the base audit,
and the width-five execution frontier is not the obstacle it appeared to be.

The fork presentation has now been measured on actual maps, and it is small.
\Sprose{exhaustive interface-behaviour census over duals of planar triangulations, tools/fourcolor/v24\_interface\_behaviour\_gate.py}
What a piece presents to the rest of a map is its inside-degree at each middle
vertex, its colour support, its connectivity pairing, and whether it hides a
component meeting no middle vertex, all taken up to relabelling the middle
set.  Counting those over connected pieces of simple cubic planar bridgeless
maps gives
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
interface width & 2 & 3 & 4 & 5\\
\midrule
behaviours realized & 2 & 6 & $\approx 28$ & $\approx 187$\\
\bottomrule
\end{tabular}
\end{center}

Widths two and three saturate exactly and immediately: no map beyond the
smallest contributes a new behaviour.  Widths four and five are still
sampling-limited rather than unbounded.  No realized piece has empty colour
support at any width, which is the emptiness audit on the realized image.  The
search is validated against exhaustive enumeration on every map small enough
to enumerate, so the counts are not an artefact of the search order.

Two limits belong with those numbers.  Finiteness of this profile is not what
was measured and is not in question: the support is a subset of at most $3^{w}$
tuples, so the profile ranges over an a-priori finite set.  And the profile
omits capped face-progress, so the counts are lower bounds on the full profile
count.  What the census does establish is scale, and the scale is the point:
the refuted linear presentation needed $3{,}696$ letters at frontier width
five, where the fork presentation realizes under two hundred behaviours.

The realized image is not the closure.  A closure must also reach states that
no single map realizes, and ruling those out is what the structural
coordinates are for.  Two obligations remain before this becomes a base audit:
a fork transition system in Lean whose coordinates are proved to describe the
literal constructed object, and the closure over it rather than the census of
realized pieces.

For the current direct headline, the two load-bearing open inputs are fixed
ordered-mesh exclusion ($D_{1d}$) and the route-native finite base ($D_{4b}$).
The $D_{1b}$/$D_{1c}$ chain is an alternative global construction, while
$D_{2b}$ is only one possible implementation path toward $D_{4b}$.  A theorem
outside those dependency paths is retained only if it is reusable generic
mathematics or a compact refutation that prevents a known false attack from
being repeated.

Scheduling is qualitative and fail-closed.  Each attack starts with a bounded,
falsifiable control.  A step counts as a state change only when it proves a
load-bearing premise, refutes a satisfiable proposed lemma, removes a false
dependency, or produces a replayable certificate consumed by one of the open
premises.  After one bounded attempt without such a change, work rotates to an
independent obligation; after two, the exact wreckage is committed and that
attack is frozen until genuinely new information appears.  Conditional
wrappers, import-only wiring, elapsed effort, and additional local lemmas do
not count as progress.

Consequently the route remains a conditional proof rather than a proof of the
Four-Color Theorem.  Its status is the dependency table above, not a percentage.
...
a different source-faithful global construction.

\subsection{Dependency-directed scheduling and a genuinely absolute control}
\label{sec:absolute-topology-gate}

The node names below are addresses, not units of progress.  In the existing
direct specialization, fixed-mesh exclusion and the
...
optional when the conservative finite state bound is retained. A complete
closure certificate would still require the physical alphabet, labelled steps,
...
load-bearing premise is discharged here.
...
not added as a premise supplier to the headline.
...
fixed-mesh exclusion, or D1b/D1c.  The incidence and single-cycle frontier steps are now supplied below.
...
open.  No catalogue or reducibility computation is used.
...
discharged here.  This is a proved intermediate statement, not discharge of
...
implication is recorded without treating it as discharge of those premises.
...
source-facing global supply remain open.
...
under ordered-mesh exclusion for the closed-map Count system.
...
the full D1b/D1c obligations remain open.
...
obligations and the generating base remain open.
...
D1b/D1c obligations remain open.
...
No wall exclusion is asserted here.
...
D1b/D1c supply, and the generating base remain open.
...
fixed ordered-mesh exclusion. The exchange statements concern ordinary Tait colourings; preservation of
...
for its construction. No obligation row is discharged.
...
refutation checkpoint on the global reduction attack.
...
load-bearing premise changes status.
...
obligation has been discharged by this refinement: The stronger supplier in Section~\ref{sec:protected-linked-band} repairs
...
counting argument merely by adjusting size and width.
...
global reduction or generating-base obligation.
...
construction discharges no load-bearing premise; the total stays
open.
...
reduction still needs to respect the actual reclosure equations.
...
reclosure constraints. No load-bearing premise is fully discharged:
the total remains open.
...
edges on these full interfaces.
...
Those target-facing conditions remain to be proved.
...
load-bearing premise is discharged,.
...
second sustained wide-case attack without that implication, so the
proof plan is due for review.
...
No global reduction or finite base is proved here.
...
\subsection{Composite endpoints: temporary expansion cannot repair a fixed two-vertex footprint}
\label{sec:four-port-composite-endpoint}

The preceding obstruction can be strengthened without specifying the
intermediate rewrite sequence. Keep a finite cubic exterior $T$ with four
unrestricted ports. Assume that $T$ has a proper Tait colouring but its
original two-vertex closure $T\circ I$ has none. Then every proper exterior
colouring has equal colours on ports $01$ and on ports $23$.
\Sverified{Mettapedia.GraphTheory.FourColor.FourPortCompositeEndpoint.forced_pairs}
More strongly, some proper exterior colouring makes \emph{all four} port
colours equal.
\Sverified{Mettapedia.GraphTheory.FourColor.FourPortCompositeEndpoint.exists_monochromatic_coloring}

\paragraph{The physical switch argument.}
Write the two forced pair colours as $a,a,b,b$. If $a=b$, nothing is
needed. Otherwise switch the $(a,b)$ component starting at port $0$.
It must also contain port $1$: otherwise the switched colouring violates
the forced $01$ equality. It cannot contain port $2$, because a
bichromatic component reaching a boundary has exactly two boundary ends,
already $0$ and $1$. The switch therefore makes ports $0$ and $2$ equal
to $b$; the forced pair equalities make all four ports equal. This uses
the existing physical mate theorem, not an assumed planar separator.

The monochromatic conclusion is a known fact about isochromatic
four-poles, not a new claim of mathematical novelty; see Karab\'{a}\v{s},
M\'{a}\v{c}ajov\'{a}, Nedela and \v{S}koviera,
\emph{Cubic graphs with colouring defect 3}, Section~2 and the proof of
Claim~4 in Theorem~5.3,
\href{https://arxiv.org/abs/2308.13639}{arXiv:2308.13639}.
Here it is proved against the port-tangle colouring semantics and used
to construct literal endpoints.

\paragraph{Every pairing endpoint is colourable.}
The monochromatic proper colouring first proves that the four port
vertices are distinct. For any fixed-point-free involution on the four
ports, move those darts into the interior carrier and pair them by that
involution. This constructs a closed, loop-free cubic port tangle with
the same exterior vertices. Parallel edges are permitted. The same
colouring extends to every such pairing, since all four port colours agree.
\Sverified{Mettapedia.GraphTheory.FourColor.FourPortCompositeEndpoint.wireClosure_cubic}
\Sverified{Mettapedia.GraphTheory.FourColor.FourPortCompositeEndpoint.every_wire_endpoint_colorable}
The endpoint has two fewer vertices than the original closure.
\Sverified{Mettapedia.GraphTheory.FourColor.FourPortCompositeEndpoint.endpoint_vertex_count}
A reused two-digon exterior supplies a kernel-checked nonvacuous instance.
\Sverified{Mettapedia.GraphTheory.FourColor.FourPortCountRewireControl.nonvacuous_endpoint_obstruction}

The executable gate independently checks all $45$ adjacent-pair deletions
of the Petersen graph and the five-flower snark, hence $135$ pairing
endpoints. Both controls are nonplanar, bridgeless and uncolourable;
their deletion exteriors are colourable. Two independent complete
enumerators agree, and a monochromatic exterior colouring is explicitly
extended at each endpoint. The source and receipt are
\texttt{tools/fourcolor/v24\_composite\_endpoint\_gate.py} and
\texttt{results/fourcolor/v24\_composite\_endpoint\_gate.json}.

\paragraph{Exact restriction, not a global impossibility claim.}
Whatever happens during a composite rewrite, if its final graph keeps
this exterior unchanged and merely pairs these four ports, it cannot be
a smaller zero-Count graph under the stated hypotheses. This is an
endpoint result, not a formalization of an arbitrary rewrite language.
It does not exclude larger-footprint moves, exterior changes, or a
different proved descent mechanism. Exterior colourability remains an
explicit hypothesis; no supplier from the target counterexample class,
planarity of the endpoint, or global reduction is proved here. The
formal carriers are in universe zero.

\subsection{A finite semantic image is not an effective generating-base certificate}
\label{sec:finite-image-base-audit}

One attempted shortcut to the source's categorical presentation is to
enumerate physical objects of unbounded size, retain one representative of
each finite profile, and use the resulting representatives as a generating
base. The missing step in this shortcut is an effective
certificate that \emph{all} realizable profiles have been found. A finite
codomain alone does not supply that certificate, even if observation of each
enumerated object is computable.

\paragraph{A generic obstruction with two observations.}
For a partial-recursive program code $c$, let $b_c(k)$ be true exactly when
mathlib's bounded evaluator, with bound $k$ and input zero, returns an
output. This observation is primitive recursive. The evaluator parameter
is a bound in its operational definition, not elapsed running time.
Write
\[
 S_c(k)=\{b_c(j):0\le j\le k\},\qquad
 \mathrm{Complete}(c,k)\iff
       \forall j\; b_c(j)\in S_c(k).
\]
Each $S_c(k)$ has at most two elements. For every $c$ there exists a finite
$k$ with $\mathrm{Complete}(c,k)$: if $c$ halts, choose a bound witnessing
its output; otherwise the sole observation is false.
\Sverified{Mettapedia.GraphTheory.FourColor.FiniteImageBaseAudit.exists_complete_cutoff}
Nevertheless there is no total computable function $B$ on program codes
such that $\mathrm{Complete}(c,B(c))$ for all $c$.
\Sverified{Mettapedia.GraphTheory.FourColor.FiniteImageBaseAudit.no_computable_complete_cutoff}
Otherwise bounded evaluation at $B(c)$ would decide whether $c$ halts.
This proof reuses mathlib's halting theorem; for the computability
formalization see Carneiro, \emph{Formalizing Computability Theory via
Partial Recursive Functions},
\href{https://doi.org/10.4230/LIPIcs.ITP.2019.12}{ITP 2019, paper 12}.

The explicit constant-output-$8$ control has
$S_c(1)=S_c(7)=\{\mathrm{false}\}$ but $b_c(8)=\mathrm{true}$.
Thus even a run of unchanged observed images is not a closure certificate.
\Sverified{Mettapedia.GraphTheory.FourColor.FiniteImageBaseAudit.plateau_control}
There is also no function $f:\mathrm{Bool}\to\mathrm{Bool}$ satisfying
$b_c(k+1)=f(b_c(k))$ for this control: the same false observation has
different successors at different stages.
\Sverified{Mettapedia.GraphTheory.FourColor.FiniteImageBaseAudit.plateau_has_no_observation_transition}
There are two \emph{observations}, not two internal machine states.

\paragraph{What a genuine finite presentation adds.}
This does not obstruct closure computation for an explicitly given finite
generator algebra with computable operations on a finite state carrier.
Such an algebra supplies the successor information missing from the
countermodel. A proof that every relevant physical object is generated
then connects this closure to the graph class. The existing exact closure
interface already separates alphabet completeness, transition soundness
and completeness, initial-state coverage, and reachability; its state-array
exactness theorem is available.
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24FiniteClosureInterfaces.ExactReachableClosureCertificate.state_entry_iff_reachable}
None of those physical obligations is constructed by the generic audit.

\paragraph{The base consumer does not require exactness.}
Exact image exhaustion is not a prerequisite of the existing spherical
base reflection. Its finite reachable set may \emph{overapproximate} the
traces of realizable bounded maps. It suffices to cover every actual trace
and to verify that the finite set contains no state satisfying the necessary
counterexample condition.
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24SphericalRouteNativeBaseReflection.TaitBaseReflection.taitBaseVerifiedAt_of_filter_eq_empty}
Consequently the exact-closure optimization and the finite-base obligation
must not be conflated. A sound, computable overapproximation with an empty
bad-state filter would discharge the base without solving exact realizability.
The construction of that overapproximation, its coverage proof, and its
successful audit remain open; the theorem here supplies none of them.
Likewise, once an explicit vertex bound and a complete finite encoding of
the bounded maps are supplied, exhausting that finite domain terminates.
The obstruction does not make the resulting finite base undecidable or
prevent its direct computation; it concerns extracting a cutoff from
unbounded enumeration using only finite observations.

The precise failed shortcut is therefore
\emph{finite profile image $\Rightarrow$ computably certified exhaustion},
not the source's stronger generating-presentation strategy. No particular
mesh, corridor, or smoothing geometry is mandated by this observation.
Nor is this a reduction of a fixed Four-Color base problem to halting, a
claim that that problem is undecidable, or a lower bound on human proof
length. The base and global descent remain open.

\subsection{Constructed bounded incidence traces for the finite base}
\label{sec:bounded-cubic-colour-trace}

The finite-base reflection requires an actual trace of every bounded map,
not merely a finite set with an assumed coverage field. There is now a
concrete, deliberately uncompressed incidence trace. For $n$ vertices and
$m$ edges, a code is a function
\[
       C:\operatorname{Fin}(n)\times\operatorname{Fin}(3)
          \longrightarrow\operatorname{Fin}(m).
\]
The three entries at a vertex name its three incident edges. Structural
validity requires distinct entries at each vertex and exactly two distinct
incident vertices for each edge; parallel edges are permitted. The
executable Boolean test $\operatorname{Col}(C)$ enumerates assignments of
the three nonzero Klein-group colours to the numbered edges and tests
pairwise distinctness at every vertex. It does not assume colourability.

\paragraph{Coverage and adequacy are constructed.}
For every cubic rotation system $M$, number its vertices and edges and
number the three incident edges at each vertex. This constructs
$C_M$, with no presentation-supply hypothesis. The no-self-loop field of
the rotation system makes dart-degree and edge-degree coincide, and the
two-dart edge fibres supply the two distinct endpoints. The resulting code
is structurally valid.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.encode_valid}
Colourings transfer in both directions along the edge enumeration:
\[
        \operatorname{Col}(C_M)=\mathrm{true}
          \quad\Longleftrightarrow\quad M\text{ is Tait colourable}.
\]
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.hasColouring_encode_iff}
The numbering is chosen at proof level; evaluation on a supplied numerical
code is computable. This is exact colourability semantics, not a claim that
the code preserves embedding or exact colouring multiplicities.

If $|V(M)|\le B$, double-counting darts gives
$|E(M)|\le\lfloor3B/2\rfloor$. Thus all such traces belong to the explicit
finite carrier
\[
 \coprod_{0\le n\le B}\;\coprod_{0\le m\le\lfloor3B/2\rfloor}
       \operatorname{Fin}(m)^{\operatorname{Fin}(n)\times\operatorname{Fin}(3)}.
\]
Filtering this carrier by structural validity gives the computable
\texttt{structuralEnvelope}; membership of every actual cubic-map trace
is proved, rather than supplied as a premise.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.trace_mem_structuralEnvelope}
The false Boolean value is equivalent to noncolourability of that same map.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.trace_bad_iff}

\paragraph{Why this is not yet the spherical base.}
The trace carries no chosen rotation; structural validity does not check
connectedness, bridge-freeness or spherical admissibility. The entire
structural envelope therefore cannot be used as a successful spherical
bridgeless audit. A six-vertex code, consisting of two doubled-edge
triangles joined at their remaining degree-two vertices, is structurally
valid and has no colouring. Its obstruction is checked in the kernel, and
for every $B\ge6$ it witnesses a bad state in the structural envelope.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.Controls.structuralEnvelope_contains_bad}
For calibration, the tetrahedral code is structurally valid and colourable.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.Controls.tetrahedron_hasColouring}
The executable gate compares the trace evaluator with independent
edge-colouring solvers on $K_4$, $K_{3,3}$, Petersen and the bridged control,
and checks $48$ vertex/edge/slot reindexings. These are encoding controls,
not a base census or a catalogue of reducible configurations.

\paragraph{Connectivity and single-edge deletion are now reflected.}
For a numerical code, reconstruct the primal graph using the two endpoints
of each edge index. After deleting an edge, omit only that index: parallel
edges with other indices remain. The resulting graph is proved isomorphic
to the original rotation-system primal graph, both before and after each
single-edge deletion. Consequently the executable connectivity and
bridge-freeness tests are equivalent to those properties of the original
map.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.connected_encode_iff}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.bridgeFree_encode_iff}
The refined \texttt{topologicalEnvelope} checks structural validity,
connectivity and bridge-freeness. Every bounded connected bridgeless cubic
map has its actual trace in this filter; no topological coverage premise
is introduced.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.trace_mem_topologicalEnvelope}
The bridged control is excluded for every $B\ge6$.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.Controls.bridged_not_in_topologicalEnvelope}
Kernel checks also distinguish a connected parallel triple from two
disjoint parallel triples: both are bridge-free, only the former is
connected. The independent topology gate covers five controls and $60$
reindexings. Petersen passes these topology checks, so neither the filter
nor these controls establish planarity or colourability.

\paragraph{The supplied rotation and spherical admissibility.}
The incidence trace is now augmented by the edge-flip and vertex-rotation
permutations on $\operatorname{Fin}(n)\times\operatorname{Fin}(3)$.
The actual source darts are bijective with these positions: a dart is
located by its vertex and incident-edge index. Conjugating $\alpha$ and
$\rho$ along this bijection constructs the encoded rotation; no embedding
search or preferred geometric pattern is required. Facial cycles are
the cycles of $\rho\alpha$, with singleton cycles retained. The executable
cycle count equals the source map's quotient-face count.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.encodeRotation_faceCount}

The admissibility check includes incidence validity, edge-flip compatibility,
absence of self-loops, one cyclic rotation per vertex, primal connectivity,
bridge-freeness and the Euler equation $n-m+F=2$. On an actual cubic
rotation map, acceptance is equivalent to inhabiting the existing structural
class \texttt{BridgelessSphericalCubicMapData}.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.encodeRotation_admissible_iff}
In particular, every bounded map in the target class has its actual code in
the executable \texttt{sphericalEnvelope}. Coverage is constructed for every
symbolic bound, including arbitrary finite source universes.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.rotationTrace_mem_sphericalEnvelope}

\paragraph{The concrete base consumer.}
The generic spherical base reflection is now instantiated with this finite
code space, its admissibility filter and its actual colouring test.
Writing
\[
 \operatorname{Bad}_B
   =\{C\in\operatorname{sphericalEnvelope}(B):
                    \operatorname{Col}(C)=\mathrm{false}\},
\]
the remaining input to that consumer is precisely the finite audit
$\operatorname{Bad}_B=\varnothing$, equivalently
\texttt{baseAudit B = true}. It implies the existing
\texttt{TaitBaseVerifiedAt B}; trace coverage and colourability adequacy are
no longer extra hypotheses of this numerical consumer.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.taitBaseVerifiedAt_of_baseAudit}
This is not a converse asserting that every raw accepted code has been
realized by a graph-backed simple map. A sound covering set suffices for
the base consumer.

\paragraph{Controls and the size boundary.}
On the same two-vertex, three-parallel-edge incidence graph, opposite vertex
rotations have three face cycles and pass the spherical check, whereas
equal vertex rotations have one face cycle and are rejected.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.Controls.parallelSphere_admissible}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.Controls.parallelTorus_not_admissible}
The pre-registered rotation gate checks $1108$ orientation assignments
across four control graphs and $48$ dart reindexings. These are
representation checks, not a census of the target base.

The deliberately uncompressed raw carrier has
\[
 \sum_{n=0}^{B}\sum_{m=0}^{\lfloor3B/2\rfloor}
               m^{3n}\bigl((3n)!\bigr)^2
\]
elements before validation. At $B=2$ this is already $411610900$, proved by
the cardinality formula and kernel arithmetic without enumerating the
carrier.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.raw_code_count_two}
Only the vacuous raw full audits at bounds zero and one have been directly evaluated;
there is no connected loopless cubic map in either domain.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.baseAudit_one}
This raw evaluator supplies no audit at a useful reduction threshold,
efficient generation, optimized interface closure, or global reduction. The raw carrier count
measures this representation, not an intrinsic lower bound on the cost of
the route. It motivates generation of valid data and removal of redundant
labellings before a substantial computation.

\paragraph{Eliminating redundant edge-flip and vertex permutations.}
For valid incidence data, every dart has exactly one other occurrence of
its edge at a different vertex. This constructs its edge-flip partner by
finite search. At each cubic vertex, a cyclic rotation is a fixed-point-free
permutation of three positions, hence one of two orientations. A compact
code therefore stores only the incidence function and one Boolean per
vertex. Its decoder reconstructs every valid raw rotation code exactly,
including the given embedding; this is not a quotient that identifies
spherical and nonspherical rotations.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.decode_compress}

The resulting audit is equivalent to the raw audit for every symbolic
bound, not merely for the tested examples.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.compactAudit_iff_baseAudit}
Its unfiltered carrier has size
\[
 \sum_{n=0}^{B}\sum_{m=0}^{\lfloor3B/2\rfloor} m^{3n}2^n,
\]
which removes the two factorial factors. At bound two it has $3252$
elements rather than $411610900$.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.compact_code_count_two}
The complete compact audit at this bound succeeds by kernel decision,
and the equivalence transfers the result to the raw audit without
enumerating its carrier.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.compactAudit_two}
The existing base consumer is reached through this equivalence.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundedCubicColourTrace.taitBaseVerifiedAt_of_compactAudit}

The pre-registered independent gate finds $145$ structurally valid codes
at this bound, including the empty rotation, and $72$ accepted spherical
codes, all colourable. These accepted codes represent the two-vertex
parallel-edge graph. Thus the code audit has nonempty mathematical
content, but the graph-backed \emph{simple} cubic target class at this
bound is still empty. No useful reduction threshold or substantive
simple-map base audit is established. Incidence labelling remains highly
redundant, and the global descent is independently open.

\subsection{Scalar normalization does not make exact tangle counts finite}
\label{sec:projective-tangle-count}

The source's digon and pendant two-cut laws multiply all relevant counts
by a common factor. This suggests an algebraic alternative to a finite
exact-Count image: retain relative multiplicities but identify matrices
that differ by an overall positive scalar. That alternative still has an
infinite image, already on a single physical serial-composition family
with two ports at each end.

The generating rung has two cubic vertices joined by an interior edge,
with an input and output port at each vertex. For monochromatic input
and output pairs with nonzero colours $c,d$, there is exactly one extension
if $c\ne d$, and none otherwise. The file constructs the literal tangle
and identifies its colouring cardinality with the executable extension
count, rather than postulating a transfer matrix.
\Sverified{Mettapedia.GraphTheory.FourColor.ProjectiveTangleCount.count_rung}
Serial gluing preserves cubicity, and the existing physical gluing
bijection supplies the exact count convolution.
\Sverified{Mettapedia.GraphTheory.FourColor.ProjectiveTangleCount.series_cubic}
The constructed \texttt{chain n} consists of $n+1$ such rungs; its actual
colouring row from a fixed monochromatic input is the iterated transfer
row.
\Sverified{Mettapedia.GraphTheory.FourColor.ProjectiveTangleCount.chain_row}

For $2r$ rungs, this row is
\[
 D+a_r U,\qquad a_0=0,\qquad a_{r+1}=4a_r+1,
\]
where $D$ singles out the input colour on the output pair, and $U$
indicates all three monochromatic nonzero output words. Thus its
distinguished and other monochromatic entries are $a_r+1$ and $a_r$.
\Sverified{Mettapedia.GraphTheory.FourColor.ProjectiveTangleCount.row_even}
If positive integer multiples of two such rows are equal, subtracting
these two entries first equates the multipliers; strict growth of $a_r$
then equates the lengths. This also rules out equality after positive
rational normalization by clearing denominators.
\Sverified{Mettapedia.GraphTheory.FourColor.ProjectiveTangleCount.projective_row_injective}
Consequently no finite code can identify these physical chains while
retaining the row up to positive rescaling.
\Srefuted{Mettapedia.GraphTheory.FourColor.ProjectiveTangleCount.no_finite_projective_chain_code}

This is not an obstruction to Boolean support: every positive even-length
row has the same support, namely the three monochromatic nonzero words.
\Sverified{Mettapedia.GraphTheory.FourColor.ProjectiveTangleCount.even_row_support}
It refutes the proposed scalar-normalized \emph{exact-count} repair, not
the already proved zero-Count support laws, a target-restricted invariant,
or the global compositional route. No claim is made that these colourable
controls occur in a least counterexample.

The pre-registered independent gate enumerates all colourings of the
literal ladders of lengths one through eight. At even lengths it finds
the pairs $(2,1),(6,5),(22,21),(86,85)$, and checks colour equivariance and
constant support. The rectangular drawings have an explicit four-terminal
boundary completion; its rotation cycles pass Euler characteristic two
and a facial terminal-cycle check in all eight controls. The kernel
statement records the actual cubic tangles and their serial compositions,
not an additional rotation-system sphericity theorem. This separation of
the count proof and the geometric controls is intentional. No fixed
load-bearing premise is discharged: no load-bearing premise is closed.

\subsection{Positive count resolution stops at the pentagon frontier}
\label{sec:pentagon-positive-resolution}

Infinite relative counts do not by themselves prevent a finite algebraic
presentation. A different candidate, suggested by the source's square
identity, is a positive sum of counts of smaller physical pieces. Such an
identity would preserve zero Count after contraction with any exterior.
The triangle and square admit this kind of resolution; the pentagon
already obstructs a uniform continuation of that particular mechanism.

Consider pieces with five labelled terminals and fewer than five cubic
vertices, encoded by perfect pairings of all vertex slots and terminals.
The internal vertex count must be odd, hence is one or three. The gate
includes all $105+135135=135240$ pairings at those sizes, without assuming
planarity, connectedness, simplicity, or a favourable boundary order. It
finds $59460$ colourable labelled pieces, and none has support contained
in that of the five-cycle. Vertex-free circle components are omitted from
the endpoint encoding; they only multiply counts by three and do not
alter this support test.

There is a small description of all the nonzero profiles found by the
gate. Up to boundary permutation and a scalar factor, they are the count
of a cubic Y with a separate wire, or the count of a three-vertex cubic
path. These give respectively ten and fifteen distinct profiles. With
boundary word $w$, the first requires a proper triple at ports $0,1,2$
and equal nonzero colours at ports $3,4$. The second counts colours $a,b$
making the triples
\[
       (w_0,w_1,a),\quad(a,w_2,b),\quad(b,w_3,w_4)
\]
proper. The gate verifies the full profiles, not only their supports:
the only scalar multipliers required are $1,2,6$.

The obstruction for these two types has a compact kernel certificate.
Write the nonzero colours as $r,b,p$. The five words
\[
 (b,r,p,r,r),\quad(b,r,r,p,r),\quad(r,b,r,p,r),\quad
 (r,b,r,r,p),\quad(r,r,b,r,p)
\]
all have zero pentagon extension count.
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonPositiveResolution.badWord_count_zero}
Every boundary permutation of each of the two smaller types accepts at
least one of those words.
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonPositiveResolution.every_shape_accepts_badWord}
Thus none of these smaller supports is contained in the pentagon support.
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonPositiveResolution.no_small_support_subset}
In any proposed nonnegative integer combination, evaluate at a rejected
word accepted by a chosen summand. Positivity forces that summand's
coefficient to vanish. All coefficients must therefore vanish, whereas
the pentagon has an extension at $(r,r,r,b,p)$. Consequently there is no
such positive count identity, with any fixed weights on the two types
and all their boundary permutations.
\Srefuted{Mettapedia.GraphTheory.FourColor.PentagonPositiveResolution.no_positive_resolution}

The trust boundary is explicit: the kernel theorem covers the displayed
count definitions and every boundary permutation. The classification of
\emph{all} smaller endpoint pieces into their scalar multiples is the
separate exhaustive gate, not an additional Lean structural theorem. The
gate is independently calibrated by the triangle and square identities,
cycle-edge versus boundary-recurrence enumeration, and edge-colour versus
vertex-colour enumeration on controls.

This is not a proof that a generating base or a global positive reduction
is impossible. It excludes a uniform, context-independent positive
resolution of this one face by smaller endpoint pieces. A larger piece,
an exterior-dependent relation, or retaining such pieces among generators
changes the question and still requires a genuine global descent and
coverage proof. No configuration catalogue or target-class exclusion is
constructed. The analogous shortcut for a long face also fails at its first local gate.
Delete two adjacent vertices of an $n$-cycle and pair the four loose ends in
each of the three possible ways.  For every $5\leq n\leq10$, exact boundary
enumeration finds a word accepted by each smoothing and rejected by the
original cycle.  Hence no nonnegative exact combination of those three local
smoothings can equal the facial-cycle count tensor.  This refutes only that
adjacent two-vertex schema, not arbitrary smaller tangles or reductions that
depend on the exterior.  The executable receipt and independent controls are
in \texttt{v24\_long\_face\_local\_resolution\_gate.py}.

\subsection{The five-port context tensor retains one positive obstruction}
\label{sec:pentagon-context-tensor}

Failure of a context-independent positive resolution leaves a different
possibility: use constraints on the exterior's count vector. The next gate
tests the signed $\delta\epsilon$ span with its cyclic boundary sign,
coordinatewise nonnegativity, and abstract noncrossing Kempe closure.
It does not identify this relaxation with actual plane exteriors.

Five-port admissible words have colour multiplicities $(3,1,1)$.
Modulo colour relabelling, their ten coordinates are indexed by the two
singleton positions in the order
\[
  01,02,03,04,12,13,14,23,24,34.
\]
For each pair $ij$, the signed elementary tensor is
$\delta(w_i,w_j)\epsilon(w_a,w_b,w_c)$, with $a<b<c$ the other
positions. Correct its sign using any compatible adjacent wire and the
complementary Y; the gate checks independence of that choice on all sixty
admissible words. The displayed span has rank six. Its coordinate
vectors satisfy
\[
\begin{split}
 -f_0+f_1-f_2+f_3&=0,\qquad -f_0+f_4-f_5+f_6=0,\\
 -f_1+f_4-f_7+f_8&=0,\qquad -f_2+f_5-f_7+f_9=0.
\end{split}
\]
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonContextTensor.tensor_relations}
Vanishing on the pentagon coordinates $0,3,4,7,9$ forces
\[
             f=t(0,1,1,0,0,1,1,0,1,0).
\]
Nonnegativity says only $t\geq0$, not $t=0$.
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonContextTensor.positive_ray_of_relations}
An explicit integer combination of three elementary tensors realizes
the vector with $t=1$. On every genuine three-colour boundary word it
is one exactly when the word is admissible and rejected by the pentagon,
and zero otherwise.
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonContextTensor.adversary_value}
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonContextTensor.adversary_nonzero}

There is a more directly physical description. Write $B$ for this vector,
$P$ for the pentagon count, $Y_r$ for the five cyclic Y-with-wire counts,
and $T_r$ for the five cyclic three-vertex tree counts. Pointwise,
\[
              B=\sum_{r=0}^4Y_r-2P,
       \qquad 3B=\sum_{r=0}^4T_r-P.
\]
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonContextTensor.signed_physical_identity}
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonContextTensor.signed_connected_identity}
Thus even connected plane controls span the obstruction: disconnected
wires are not its cause. No homogeneous linear functional vanishing on
those six connected count profiles can exclude $B$.
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonContextTensor.no_homogeneous_separator}

The independent gate also checks all ninety word/colour-pair cases of
abstract Kempe closure: for each rejected word and colour pair there is
a noncrossing matching of the active ports for which switching any union
of pairs stays in the thirty-word rejected support. This closure check,
the ranks, and the gauge-choice audit are program checks, not additional
Lean theorems. The signed identities, all-word values, and ray implication
are kernel-checked. The gate checks all $3^5$ words, calibrates fifteen
cyclic plane controls, and tests colour equivariance and boundary order.

This isolates a loss of information, not a counterexample to the route.
The vector is not claimed to be an actual plane exterior. Its existence
rules out using just the stated linear identities, positivity and
abstract closure to force a pentagon extension. A useful further argument
must add a genuine restriction on simultaneous physical realization,
an additional inequality, or a different global compositional reduction.
No particular mesh geometry, catalogue, or new execution trust follows.
The generating-base and global-descent obligations remain open.

\subsection{Joint physical matchings do not force immediate repair}
\label{sec:pentagon-kempe-plateau}

The preceding tensor gate leaves simultaneous physical realization open.
A more specific attempted strengthening is false: even when the three
bichromatic boundary matchings come from one proper colouring of one plane
exterior, a single switch need not make the boundary extend across the
pentagon. This is a generic reconfiguration control, not a new
configuration-exclusion certificate.

Open the first facial pentagon of the fixed thirty-vertex spherical control
used by the smoothing-provenance gate. The resulting connected exterior
has twenty-five cubic vertices, five degree-one terminals, and forty edges.
The program checks the rotation and spherical Euler count, the facial port
order, and connectivity with and without the terminals. Two independent
proper-colouring searches agree on all $354$ colourings. They form one
Kempe orbit, accepting all sixty admissible boundary words, thirty of which
extend across the pentagon. Thus this is \emph{not} a physical realization
of the rejected-only support.

A displayed proper colouring has word $(0,0,2,0,1)$ and boundary matchings
\[
 (01):\{04,13\},\qquad (02):\{03,12\},\qquad (12):\{24\}.
\]
Here the parenthesized pair denotes colours and the braces list port pairs.
Lean checks properness against the explicit graph incidence.
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonKempePlateau.states_proper}
Five explicit bichromatic paths force the displayed matching constraints
for every union of components of the respective colour pair.
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonKempePlateau.matches_of_region}
Only the $32$ boundary-selection masks then need checking: none gives a
positive pentagon extension count. No enumeration over forty-edge subsets
is needed for this conclusion.
\Srefuted{Mettapedia.GraphTheory.FourColor.PentagonKempePlateau.no_immediate_repair}

Three explicit switches do reach an extendable colouring. The kernel checks
that each selected edge set is a union of bichromatic components, verifies
the three switch equalities, and computes extension counts $0,0,0,1$ along
the four states.
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonKempePlateau.escape_regions}
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonKempePlateau.escape_switches}
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonKempePlateau.intermediate_counts}
\Sverified{Mettapedia.GraphTheory.FourColor.PentagonKempePlateau.final_count}
The program additionally verifies that these are single-component switches
and that three is the shortest length; these and the spherical embedding
are separate program checks, not additional Lean theorems. Distances to
extension have histogram $96,150,84,24$ at distances $0,1,2,3$.

There is also a concrete failure of the proposed state compression for
\emph{reconfiguration}. Let $\pi(c)$ retain a colouring's boundary word
and all three boundary matchings. Among $144$ profile fibres, eighteen
contain colourings with different shortest repair distances. An explicit
pair in the same fibre has distances three and two, respectively, and
different sets of one-switch successor profiles. This mismatch also holds
when a step may switch an arbitrary union of components of one colour pair.
The program preserves both colourings and witnesses for both transition
conventions. An existential quotient relation can still be defined, but
the observed equality of profiles does not let one lift every abstract
transition from every representative.
Even adding the lengths of all bichromatic cycles missing the boundary
leaves six mixed-distance fibres among $192$ refined profiles in this
control; the receipt includes an explicit pair. Thus merely remembering
those forgotten cycles does not repair this state compression either.

Consequently, joint realization at one colouring does not replace a theorem
about compatibility \emph{across} colourings. A profile sufficient for
gluing must not silently be assumed sufficient for Kempe reconfiguration.
This does not refute the existing gluing semantics, arbitrary eventual
repair, or a different well-founded measure. It removes an unnecessarily
strong immediate-repair step from this candidate attack on global descent;
it supplies neither global coverage nor the generating base.

\subsection{Constructed vertex letters and complete Count sweeps}
\label{sec:constructed-vertex-count}

Instead of compressing Kempe reconfiguration, the following construction
returns to exact composition. For an arbitrary side $A$ of a finite
loopless rotation system and a vertex $v\notin A$, adjoining $v$ gives one
vertex star and the edges crossing both cuts as persistent wires.
Every dart at $v$ receives its colour either from the opposite dart on
the old boundary or from its own dart on the new boundary. No embedding
of a supplied slab, connected shore, or width estimate is assumed.

For boundary words $x,y$, let $\operatorname{Test}_v(x,y)$ require nonzero
boundary colours, distinct colours on the new vertex's incident darts,
and equal colours on each persistent wire with its original ambient
identity. These equations construct an actual layer colouring, and every
such colouring satisfies them. Fixed boundary words determine the layer
colouring uniquely. Thus the literal matrix is
\[
 M_{A,v}(x,y)=[\operatorname{Test}_v(x,y)],\qquad
 C_{A\cup\{v\}}(y)=\sum_x C_A(x)[\operatorname{Test}_v(x,y)].
\]
\Sverified{Mettapedia.GraphTheory.FourColor.TubeSlab.VertexTransfer.slabRel_iff}
\Sverified{Mettapedia.GraphTheory.FourColor.TubeSlab.VertexTransfer.slabCount_eq}
\Sverified{Mettapedia.GraphTheory.FourColor.TubeSlab.VertexTransfer.sideCount_step}

For a cubic system the three dart positions are constructed from cubicity.
After explicit input and output enumeration, the letter consists of three
port indices and a partial map from output indices to the corresponding
persistent input indices. The star indices are distinct and two wire
outputs cannot copy the same input edge.
\Sverified{Mettapedia.GraphTheory.FourColor.TubeSlab.VertexTransfer.starEquiv}
\Sverified{Mettapedia.GraphTheory.FourColor.TubeSlab.VertexTransfer.encode_star_injective}
\Sverified{Mettapedia.GraphTheory.FourColor.TubeSlab.VertexTransfer.encode_wire_injective}
For arities $l,r$, the unrestricted raw code type has
$(l+r)^3(l+1)^r$ members. This is a syntax bound, not a claim that all
those codes are well-formed or planar in the chosen boundary order.
The test of an extracted code is equivalent to the literal boundary
equations, and computes the literal Count entry.
\Sverified{Mettapedia.GraphTheory.FourColor.TubeSlab.VertexTransfer.card_code}
\Sverified{Mettapedia.GraphTheory.FourColor.TubeSlab.VertexTransfer.count_encode}

Starting at the empty side with count one and processing every vertex
gives exactly the number of proper colourings of the whole rotation
system. Finite vertex enumeration supplies the complete sweep; it is
not a new supply hypothesis. Every fresh cubic step can be evaluated
through its extracted finite code.
\Sverified{Mettapedia.GraphTheory.FourColor.TubeSlab.VertexTransfer.sweepCount_cons_code}
\Sverified{Mettapedia.GraphTheory.FourColor.TubeSlab.VertexTransfer.sweepCount_closed}
\Sverified{Mettapedia.GraphTheory.FourColor.TubeSlab.VertexTransfer.canonicalCount_eq}

The independent program gate compares actual star-and-wire edge colourings
with every extracted letter and every well-formed letter of arities at
most three. Complete sweeps give counts $6,60,0$ for the tetrahedron,
dodecahedron, and nonplanar Petersen zero control. Reordering vertices
or edges preserves the count, not necessarily the peak frontier width.
The width-three alphabet has $96$ ordered letters. From the empty word,
with acceptance at the empty terminal word and ill-typed transitions
rejected, raw reachability, subset reachability and the Nerode quotient
have sizes $10,4,4$. An independent literal-relation implementation
checks these figures. These numerical figures are program checks, not
additional kernel closure certificates.

This constructs the algebraic physical-letter input for a vertex-sweep
version of Proposition~\ref{prop:exact-closure-gate}. It does not identify
that unrestricted alphabet with all and only the source-admissible planar
Cells, supply a uniform frontier bound, or show that deleting a repeated
state preserves the target graph class. Exact natural-number Count is
not asserted to have finite image. Global descent, the target-specific
closure certificate and the generating-base verification remain open;
\subsection{Vertex-splice orientation: a failed matching and its repair}
\label{sec:vertex-splice-orientation}

The exact Count sweep of Section~\ref{sec:constructed-vertex-count} does
not by itself make every support-preserving splice spherical. This can
be separated from colourability on the two-vertex, three-parallel-edge
map. Give its vertices opposite cyclic orders to obtain three faces;
give them the same cyclic order to obtain one face. Both stored rotations
are genuinely cyclic at each vertex, cubic and loopless. Both maps are
Tait colourable, and for \emph{every} vertex side and boundary word their
actual Count functions agree, through an explicit colouring-fibre
bijection. The Euler characteristics are nevertheless $2$ and $0$.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSpliceOrientation.cyclic}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSpliceOrientation.same_side_support}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSpliceOrientation.same_side_count}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSpliceOrientation.colorable}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSpliceOrientation.flat_spherical}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSpliceOrientation.twisted_not_spherical}
Consequently no predicate of the Count function of the varying half alone
can classify sphericity after gluing to the fixed other half in these
stored coordinates.
\Srefuted{Mettapedia.GraphTheory.FourColor.VertexSpliceOrientation.no_count_only_sphericity}

The independent vertex-splice gate starts with spherical bridgeless cubic
controls, extracts their actual cyclic stars and persistent wires, and
deletes intervals between equal nonempty prefix supports. It reconstructs
the smaller rotation system, including its faces and bridges. Across $160$
checked orders (not asserted distinct), $743$ support repeats include $72$
identity-coordinate splices with failed spherical Euler equality. Requiring
equal boundary first-return permutations retains $671$ cases and no
observed failure; adding the boundary component partition leaves this
count unchanged, and also adding the set of component partitions after
one internal-edge deletion leaves $564$ cases. These are finite program
observations, not proofs that any proposed signature is universally
sufficient.

More importantly, \emph{all $72$ failures have a repair in this gate}:
a port permutation preserves the colouring support, identifies the
boundary face-return permutation and component partition, and produces
an actually spherical, connected, bridgeless, loopless smaller map.
One saved example removes two vertices of the tetrahedron and produces
the two-vertex map above. Another removes four vertices of the hexagonal
prism and produces a simple eight-vertex graph. The latter identity
splice has Euler characteristic zero, while the repaired matching gives
Euler characteristic two. Its full colouring count changes from $72$ to
$24$; equality of supports preserves existence, not total multiplicity.
Independent incidence colourings and face walks reproduce both examples.
The eight-vertex identity splice also has other cyclic rotations with
Euler characteristic two, so the failure is \emph{not} a claim that its
underlying graph is nonplanar.

This is a correction to an under-specified matching rule, not a refutation
of geometrically compatible compositional deletion. Parallel edges in
the first repaired example are allowed in the existing smaller-map
minimality class. Neither control is a least counterexample. The next
physical-state obligation is to prove the appropriate context congruence:
equal combined colouring and structural data must permit an admissible
replacement in the required contexts. A check of first-return order alone
is not here promoted to that theorem. The global reduction, target-facing
closure certificate and generating base remain open.

\subsection{Boundary surgery and exterior-independent Euler replacement}
\label{sec:boundary-rewire-euler}

The face-count gap in Section~\ref{sec:vertex-splice-orientation} admits
a generic permutation calculation. Let $p$ be a permutation of a finite
dart set, $H$ its marked interface, $R(p)$ its first-return permutation
on $H$, and $s$ a permutation of $H$, extended by the identity away from
$H$. Write $c$ for the number of cycles, including fixed points. The
existing first-return identity gives $R(p\widetilde{s})=R(p)s$.
Cycles disjoint from $H$ are preserved by an explicit equivalence of
their orbit carriers. Splitting all cycles into those meeting $H$ and
those missing it therefore proves
\[
 c(p\widetilde{s})+c(R(p))=c(p)+c(R(p)s).
\]
\Sverified{Mettapedia.GraphTheory.FourColor.BoundaryRewireCycles.nonHitEquiv}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundaryRewireCycles.cycle_count_rewire}
This includes empty interfaces, disconnected carriers and multiple
boundary cycles; no disc assumption is used.

For the actual open-tangle constructor, temporarily fixing each unpaired
boundary dart defines $p$. The stored composite face permutation is
proved to be $p\widetilde{s}$, where $s$ swaps the matched ports.
Consequently the formula computes its actual quotient-face count, not
just a proposed transfer statistic.
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleRewireFaces.compositePhi_eq_unsewn_mul}
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleRewireFaces.composite_face_count}
Transporting $R(p)$ to the bare port carrier $L\sqcup R$ gives a code
with at most $(|L|+|R|)!$ possibilities. For fixed pieces, two matchings
give the same Euler characteristic exactly when their products with
this code have the same number of cycles; their vertex and edge counts
are unchanged. This raw code bound does not assert physical realizability
of every code.
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleRewireFaces.card_boundary_codes}
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleRewireFaces.composite_euler_eq_iff_code}

Replacement of a piece is stronger than changing the matching of fixed
pieces. Let $A,B$ be two open pieces whose existing named boundary-return
records agree in common port coordinates. Let $X$ be any fixed exterior
with a matching to those coordinates. Define the intrinsic integer
\[
 m(A)=2|V_A|-|I_A|+2c(\phi_A^{\rm open}),
\]
where $I_A$ is the set of \emph{internal darts}, not internal edges.
The open permutation fixes the provisional boundary partners and then
applies the vertex rotation. The disjoint-sum return theorem and the
record-induced seam correspondence give
\[
 F(X\# A)+c(\phi_B^{\rm open})
   =F(X\# B)+c(\phi_A^{\rm open}).
\]
Using twice-edge-count equals dart-count, this yields the exact law
\[
 2\chi(X\# A)-2\chi(X\# B)=m(A)-m(B).
\]
Thus, under equality of the named return records, the two actual
composites have equal Euler characteristic if and only if $m(A)=m(B)$.
The exterior does not enter $m$, and the two pieces may have different
vertex and interior-dart carriers.
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleReplacementEuler.face_count_replacement}
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleReplacementEuler.twice_euler_replacement}
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleReplacementEuler.euler_eq_iff_mass}

The independent finite gate checks all $40{,}789$ permutation, marked-set
and marked-permutation combinations on up to five points. Another
$2{,}229$ sewing comparisons check the replacement correction against
different interior carriers and fixed exteriors. These program controls
are separate from the general kernel proofs. They include untouched
cycles and show why the number of boundary cycles alone loses necessary
information: the cyclic order affects the result of sewing.

This supplies the Euler component of context congruence for the physical
compositional language; it does not by itself preserve connectivity or
bridgelessness. The generic law does not bound $m$; the next section does
so for actual proper vertex sides of spherical maps. Equality of boundary
records alone is not asserted to force equality of $m$. A complete finite
admissibility signature, its transition law, the global reduction and
the generating-base verification remain open. No particular mesh,
corridor shape or universal frontier bound has been assumed.

\subsection{Planar Euler mass is a finite component coordinate}
\label{sec:vertex-side-euler-mass}

The integer in Section~\ref{sec:boundary-rewire-euler} is now controlled
on arbitrary actual vertex sides, rather than on a chosen corridor
shape. Let $G$ be a connected spherical cubic rotation system with one
cyclic rotation on each vertex, let $S$ be a vertex subset, and let $A$
be its literal open tangle \texttt{ofVertexSide}. The constructor takes
a retained outer dart; neither $S$ nor its complement is required to be
connected. Write $c_S$ for the number of connected components of $G[S]$.
Then
\[
             m(A)=4c_S.
\]
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideEulerMass.mass_eq_four_components}

This is inherited planarity, not an assumed disc certificate. On the
original dart carrier, delete every edge not internal to $S$ by fixing
its two darts under the capped edge involution. The remaining permutation
components are precisely the components of $G[S]$, together with one
isolated rotation orbit for every outside vertex. An explicit quotient
equivalence proves that assertion.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideCappedComponents.componentEquiv}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideCappedComponents.wordOrbitCount_capped}
The existing planar edge-deletion theorem supplies Euler equality for
this edge presentation. Splitting its face permutation into the retained
open walk and the outside rotations cancels the outside vertices. Twice
the number of internal edges is the number of internal darts, yielding
the displayed formula.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideEulerMass.capped_plane}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideEulerMass.openPhi_ofVertexSide}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideEulerMass.cycle_count_cappedPhi}

If $S$ is proper, every component of $G[S]$ meets an exposed port:
otherwise it would be a whole component of the connected ambient graph.
The proof constructs the surjection from actual boundary darts to side
components and therefore gives
\[
       0\le m(A)=4c_S\le4|\partial S|.
\]
Consequently, at width at most $k$, $c_S$ is a coordinate in
$\{0,\ldots,k\}$. This is a $k+1$-element carrier, not a claim that every
value is realized by the nonempty open-tangle constructor.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideCappedComponents.boundaryComponent_surjective}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideCappedComponents.components_le_boundary}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideEulerMass.mass_range}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideEulerMass.componentCode}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideEulerMass.mass_eq_four_code}

There is an actual replacement consumer: two such pieces, possibly from
different ambient maps and with different internal carriers, have equal
sewn Euler characteristic in every fixed exterior when their named
boundary-return records and component counts agree.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideEulerMass.replacement_euler}
Equal Euler characteristic is still not a proof of connectivity or
bridgelessness of the replacement. Those structural conditions, the
complete transition law, global descent and the generating base remain
separate obligations. No universal width bound or forced repetition is
inferred,.

The structural gate checks $5{,}656$ actual vertex subsets: every subset
of a tetrahedron and the prisms on $6,8,10,12$ vertices, and seeded samples
on the $16$- and $48$-vertex prisms. Direct open-face walks agree with an
independent leaf-capped construction. Empty sides are included in the
program bookkeeping; they do not instantiate the retained-outer-dart
argument of the open constructor. Negative controls explain the
hypotheses: two disconnected isolated side vertices have mass $8$, not
$4$; a full connected closed side has positive mass with no ports; and
two rotations of the theta multigraph have masses $4$ and $0$, so genus
cannot be ignored. These controls are separate from the universal kernel
theorems and do not enumerate colouring configurations.

\subsection{Boundary connectivity and structural replacement}
\label{sec:boundary-connectivity-replacement}

The component coordinate above can be absorbed into the boundary
connectivity partition. For a port-indexed multigraph $A$, write
$a\sim_A b$ when its ports $a,b$ are joined within $A$. On sewing $X$
to $A$, connectivity between seam ports is exactly the equivalence
closure of ${\sim_X}\cup{\sim_A}$. This is proved by compressing actual
glued paths into alternating within-side paths; conversely each such
boundary path lifts through the literal seam edges. No connected-side
assumption is used, and loops and parallel edges are retained.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundaryConnectivity.boundaryReach_iff}

If every component of each side meets a port, the joined partition
also determines connectivity of the whole composite. For replacement,
the exterior need not even satisfy this assumption: with a nonempty
seam, connectivity of either composite implies that every exterior
component meets the seam. Thus replacing a port-visible piece by one
with the same boundary partition preserves connectivity in every fixed
exterior.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundaryConnectivity.connected_iff}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundaryConnectivity.replacement_connected_any_exterior}

The bridge to actual open tangles uses the existing isomorphism between
their sewn rotation system and literal multigraph gluing. Proper vertex
sides of a connected ambient graph are port-visible, even when they or
their complements are disconnected. In common coordinates $K$, the
partition has a Boolean-table code in $K\to K\to\mathrm{Bool}$, of
cardinality $2^{|K|^2}$. This is a raw carrier bound, not a claim that
all its tables are realizable partitions or a verified executable
extractor.
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleBoundaryConnectivity.visible_ofVertexSide}
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleBoundaryConnectivity.card_code_carrier}
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleBoundaryConnectivity.replacement_connected}

The boundary-component maps of two proper sides are surjective. Equality
of their partition codes identifies the kernels of these maps, hence
their quotient cardinalities and side component counts. With
Section~\ref{sec:vertex-side-euler-mass}, this determines the Euler mass
without an additional component-count field. Consequently, for proper
vertex sides of connected spherical cubic rotation systems with cyclic
vertex rotations, equality of the connectivity and named face-return
records preserves both Euler characteristic and primal connectedness
under sewing to any fixed exterior. The two pieces can have different
internal carriers and come from different ambient maps.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideStructuralReplacement.components_eq_of_code}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexSideStructuralReplacement.replacement_euler_and_connected}

The structural gate checks $62{,}735$ sewings: all simple graphs on at most
three vertices with all port assignments through arity three, and two
differently sized realizations of each partition at arity four. It
compares literal glued-graph paths to an independent boundary union-find.
Negative controls distinguish equal component counts with different
partitions and expose components invisible at the boundary. The empty
graph convention is tested explicitly; the actual open-tangle
replacement theorem obtains a nonempty seam from port visibility.
These program checks are separate from the universal Lean proofs.

This is a structural consumer for the compositional interface, not a
complete admissibility signature. Bridgelessness, executable state
transitions, global descent and the generating base remain separate.
No universal width bound, forced repetition or strict decrease follows
from fixed-arity finiteness.

\subsection{Small-cut profiles retain the bridge condition}
\label{sec:boundary-small-cut-profile}

Connectivity and face return alone do not retain all structural
information needed by replacement. A program control uses two connected
four-port pieces, each consisting of two cubic vertices and one internal
edge. Their port groupings are $(01\mid23)$ and $(12\mid30)$, with
rotations giving the same return cycle $(0123)$. Both have Euler mass
$4$ and the indiscrete boundary connectivity partition. Sew either to
the same exterior consisting of two two-port digons, grouping ports
$01$ and $23$. Both composites are connected spherical cubic
multigraphs with six vertices, nine edges and five faces, but the first
has one bridge and the second has none. This is a reproducible program
specimen, not a kernel-certified refutation. It does not assert equality
of colouring supports or refute a complete Count profile.

The repair need not choose a new corridor geometry. For a vertex subset
$U$ of an open multigraph $A$, retain its boundary membership bits,
whether $U$ and its complement contain vertices, and
$\min(2,|\delta_A U|)$. The two flags exclude empty and full cuts,
including when a component has no ports. The profile is the set of
\emph{realized} records. With $k$ ports, the record carrier has
$12\cdot2^k$ elements and the profile carrier has
$2^{12\cdot2^k}$ elements. These are raw upper bounds, independent of
interior size; no efficient extractor or realizability of every formal
record is asserted.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutProfile.card_state}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutProfile.card_profile_carrier}

For subsets $U$ and $W$ of the two pieces, the actual sewn cut size is
\[
 |\delta_{A\#B}(U\sqcup W)|
 =|\delta_A U|+|\delta_B W|
   +\#\{p:\mathbf{1}_U(p_A)\ne\mathbf{1}_W(p_B)\}.
\]
Capping the two internal contributions at two preserves exactly the
test for a total at most one. Existentially ranging over the realized
records therefore gives the exact nontrivial-small-cut test for the
whole sewing, not a necessary-condition approximation.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutProfile.cutSize_glue}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutProfile.hasSmallCut_glue_iff}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutProfile.replacement_hasSmallCut}

A separate path/cut proof identifies absence of nontrivial zero- and
one-edge cuts with connectedness and bridgelessness. A disconnected
graph supplies a reachability-component cut of size zero; a bridge
supplies such a cut after deleting its own edge identifier. Conversely,
a nontrivial cut in a connected graph crosses an edge, and if that is
its only crossing edge there is no alternate path avoiding it.
\Sverified{Mettapedia.GraphTheory.FourColor.SmallCutConnectedBridgeless.no_small_cut_iff}
\Sverified{Mettapedia.GraphTheory.FourColor.SmallCutConnectedBridgeless.replacement_connected_bridgeless}
The existing seam-multigraph isomorphism transports the result to the
actual open-tangle constructor: equal low-cut codes preserve the
conjunction of primal connectedness and \texttt{EdgeBridgeFree} in
every fixed exterior. No side-connectivity or port-visibility hypothesis
is needed, and parallel edge identifiers are preserved.
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleSmallCutReplacement.replacement_connected_bridgeFree}

The low-cut gate checks the same $62{,}735$ sewings as the connectivity
gate, comparing profile composition with independent graph connectivity
and single-edge-deletion tests. It distinguishes the two bridge
specimens. These are controls for the universal Lean proofs, not a
configuration catalogue. Combined with the preceding Euler replacement
law, this supplies the structural tests needed by an enriched
compositional interface. It does not yet prove the complete state-update
law, a strict size decrease, global coverage or the generating-base
verification. In particular fixed-arity finiteness still does not force
repetition at unbounded arity. No load-bearing premise is closed.

\subsection{Executable structural transfer with exposed outer ports}
\label{sec:small-cut-open-transfer}

The small-cut signature now has an exact open-to-open transition.
Let $A$ have boundary $L\sqcup P$ and $B$ boundary $P\sqcup R$.
Sew the two copies of $P$, keeping $L\sqcup R$ exposed. For a pair
of realized records, the output retains the outer membership bits,
takes the Boolean disjunction of each nonemptiness flag, and sets
\[
 c_{\rm out}=\min\bigl(2,c_A+c_B+
       \#\{p\in P:x_A(p)\ne x_B(p)\}\bigr).
\]
The image of the two realized profiles under this operation is
\emph{exactly} the whole realized output profile. This includes
cost-two records, not merely a closed acceptance test. Every subset of
the sewn vertex sum splits into two subsets, and every pair of input
subsets realizes the prescribed output. Empty seams, hidden components,
repeated port vertices and disconnected pieces require no exceptions.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutTransfer.state_glue}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutTransfer.profile_glue}

The cap commutes with nonnegative addition. Consequently both the
record operation and its image operation on profiles are associative,
with the common outer boundary $L\sqcup R$ retained under either
parenthesization. These laws hold even for formal records not known to
be realizable; realizability is preserved when the inputs are realized.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutTransfer.composeState_assoc}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutTransfer.composeProfile_assoc}

This update also applies to the stack's literal serial-tangle
constructor, in common seam coordinates with identity matching. The
adapter forgets only the distinction between input and output ports.
A chosen dart and its involutive mate enumerate each interior edge
exactly twice, so the symmetric dart sum is twice the actual multigraph
cut size. Splitting the serial dart carrier into old darts and seam
darts gives the same cut formula, without collapsing parallel edges.
Thus the verified colouring-acceptance composition and the structural
profile transition concern the same sewn object.
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleCutDartSum.twice_cutSize}
\Sverified{Mettapedia.GraphTheory.FourColor.SerialTangleSmallCutTransfer.cutSize_serial}
\Sverified{Mettapedia.GraphTheory.FourColor.SerialTangleSmallCutTransfer.profile_serial}
\Sverified{Mettapedia.GraphTheory.FourColor.GoertzelV24OpenTangleComposition.TwoSidedOpenTangleData.serialCompose_acceptsBoundaryWords_iff}

There is a computable extractor for explicitly finite multigraphs,
proved equal to their mathematical profile. It enumerates vertex
subsets; no claim of efficient extraction for large pieces is made.
For literal tangles a second extractor uses the symmetric dart sum
divided by two, avoiding noncomputable choices of quotient-edge
representatives. It is proved equal to the actual side profile and
commutes with serial composition.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutExecutable.extractTangle_correct}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutExecutable.extractTangle_serial}
The computable transfer instead uses only the two finite sets of
records, with duplicate outputs removed. Its correctness, associativity
and equality with extraction from the actual serial tangle are proved.
The Boolean acceptance test rejects any realized nontrivial cut of
size zero or one; on a closed graph it is exactly connectedness and
bridgelessness. It is only a structural test, not colour acceptance
or a test of all counterexample-class conditions.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutExecutable.extract_correct}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutExecutable.transfer_correct}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutExecutable.transfer_assoc}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutExecutable.extract_serial}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutExecutable.accepts_extract}

The preregistered gate checks exact profiles for $138{,}125$ sewings:
all simple pieces on at most three vertices, with every port assignment
and $|L|+|P|+|R|\le3$. Additional controls cover associativity,
parallel edges, loops and saturated costs. Seven small Lean decisions
check the executable definitions themselves, including bridge rejection,
parallel-edge acceptance, an actual open transfer, and direct execution
on a literal two-sided tangle and its serial composite. These are generic
structural tests, not ring-colouring or reducibility catalogues.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutChecks.actual_open_transfer}

This supplies an executable structural coordinate for the source's
compositional closure, without choosing a mesh geometry. It does not
yet assemble the joint Count/face/structural closure, establish global
coverage or strict descent, or verify the generating base. Fixed-arity
finiteness is still not a bound across unbounded interface arities.
The global reduction remains open.

\subsection{Joint serial semantics without a preferred mesh geometry}
\label{sec:joint-serial-semantics}

The colour, face and structural coordinates now have a single joint
transfer on the literal serial constructor. The face update must retain
the vertex rotations: the unordered incidence data sufficient for
colour support do not determine topology. During sewing, record all
ports of both pieces, including the ports that will become internal.
The old return permutations combine by disjoint union; the seam
involution changes partners; first return to the remaining outer ports
then forgets the consumed seam. This computes the complete outgoing
cyclic order, not only its cycle partition.
\Sverified{Mettapedia.GraphTheory.FourColor.SerialTangleFaceTransfer.serial_phi}
\Sverified{Mettapedia.GraphTheory.FourColor.SerialTangleFaceTransfer.outputReturn_eq}
\Sverified{Mettapedia.GraphTheory.FourColor.SerialTangleFaceTransfer.serial_return}

The intermediate permutation lives on $(L\sqcup P)\sqcup(P\sqcup R)$,
regardless of the sizes of the interior dart carriers. If $\pi_A,\pi_B$
are the input boundary returns and $\tau$ exchanges the two seam
copies, the output is the first return of
$(\pi_A\sqcup\pi_B)\tau$ to $L\sqcup R$. The coordinate theorem
identifies this boundary-only operation with extraction from the actual
serial tangle, with no sphere or connectedness assumption.
\Sverified{Mettapedia.GraphTheory.FourColor.SerialTangleFaceCode.encode_serial}

Face cycles that miss the outgoing ports cannot simply be discarded
when checking topology. They are accounted for by the exact correction
to Euler mass $m(T)=2|V_T|-|I_T|+2F_T$, where $F_T$ counts all open
face cycles and $I_T$ is the interior dart carrier. Writing
$\pi=\pi_A\sqcup\pi_B$ gives
\[
 m(A;B)=m(A)+m(B)-2|P|
             +2\operatorname{cyc}(\pi\tau)-2\operatorname{cyc}(\pi).
\]
This is an integer identity, not an assertion that arbitrary pieces
have bounded Euler mass or that every sewing is spherical.
\Sverified{Mettapedia.GraphTheory.FourColor.SerialTangleFaceTransfer.serial_face_count}
\Sverified{Mettapedia.GraphTheory.FourColor.SerialTangleFaceTransfer.serial_eulerMass}

The joint signature contains boundary-colouring support, the full
face-return permutation, the realized small-cut profile, and Euler mass.
Its extraction commutes with serial sewing: colour support composes
relationally, small cuts by the preceding image formula, faces by the
boundary-return operation, and mass by the correction above. These are
four coordinates of the \emph{same} constructed tangle.
\Sverified{Mettapedia.GraphTheory.FourColor.SerialTangleJointTransfer.encode_serial}

The joint vertex-step gate compares $1{,}594$ prefixes with direct
reconstruction, including all vertex orders and all star reversals of
the tetrahedron. Its $384$ sweeps include $48$ spherical and $336$
genus-one rotations, all with the same six Tait colourings. Prism and
nonplanar zero-colouring controls are also included. At width three,
the Python evaluator exhausts all $96$ valid oriented letters, reaches
$71$ summaries, and finds one structurally accepting closed summary,
which has nonzero colour support. Six regression tests additionally
check the receipt, hidden disconnected components, empty support, and
malformed arities. This is a small fixed-point computation over a
generating alphabet, not a catalogue of reducible patches.

The named consumer remains D2b's physical compositional closure. The
generic joint transfer is proved in Lean; the vertex evaluator's
genus rejection and its fixed-point receipt are not yet a kernel
closure certificate. The joint signature's integer mass has no global
finite-range claim here. Neither all-graph bounded-width coverage,
global strict descent nor the generating-base verification follows.
\subsection{Irreversible topology and finite visible components}
\label{sec:topology-viability}

The physical evaluator's rejection rules require proofs independent of
colouring. For a rotation $\rho$ and an edge-transposition word $E$, let
$\phi=\rho\beta_E$, let $C$ count the components generated by $\rho$
and the individual edge swaps, and set
\[
 d(E)=|E|+2C-\operatorname{cyc}(\rho)-\operatorname{cyc}(\phi).
\]
The integer defect is twice the total genus on actual ribbon graphs.
Prepending $e=(a,b)$ changes the face permutation by
$\operatorname{swap}(\rho a,\rho b)\phi$; the face test must use
$\rho a,\rho b$, not the unrotated endpoints. Joining different
components merges exactly two components and two faces. Within one
component, an edge either splits one face or merges two faces, the
latter creating one handle. Thus $d(eE)-d(E)$ is respectively $0$, $0$
or $2$. These are generic permutation statements; the edge swaps need
not be disjoint from one another. For fresh insertion, where $\beta_E$
fixes both ports, the rotated and unrotated face tests agree: each port
and its rotated image are on one old face. The overlapping-word
counterexample in the gate does not refute this physical specialization.
\Sverified{Mettapedia.GraphTheory.FourColor.MapEulerDefect.component_drop}
\Sverified{Mettapedia.GraphTheory.FourColor.MapEulerDefect.defect_cons}

Induction identifies $d(E)$ with twice the number of handle-creating
steps. Euler equality is equivalent to the absence of such steps, and
no sequence of further edge additions repairs a positive defect.
\Sverified{Mettapedia.GraphTheory.FourColor.MapEulerDefect.defect_eq_twice_handles}
\Sverified{Mettapedia.GraphTheory.FourColor.MapEulerDefect.plane_iff_no_handles}
\Sverified{Mettapedia.GraphTheory.FourColor.MapEulerDefect.no_planar_completion}

The existing small-cut profile also determines the exact boundary
connectivity relation: two ports are connected precisely when every
realized zero-cut record gives them equal bits. A nonempty selected
zero-cut record with every exposed port unselected is equivalent to a
component having no exposed port. Such a component cannot join any
nonempty future exterior. This is a rejection of \emph{continuation},
not of a finished connected closed object, which has just such a record.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutViability.sameComponent_profile}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutViability.hidden_iff_not_visible}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutViability.hidden_no_connected_glue}

When no component is hidden, the component quotient of the entire
piece is equivalent to the quotient computed from its boundary records.
In particular there are at most as many components as ports, without
assuming that the prefix is connected. The executable tests are
identified with extraction from the literal two-sided tangle.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutViability.card_components_eq_profile}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutViability.card_components_le}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutViability.rejectsExtension_extractTangle}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundarySmallCutViability.sameComponent_extractTangle}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundaryViabilityChecks.closed_cannot_acquire_exterior}

The calibrated gate checks $38{,}024$ generic edge insertions and
$1{,}466$ graph/port instances. Its hidden-component controls include
$3{,}198$ sewings to nonempty exteriors. Pruning hidden-component
continuations in the width-three experiment reduces $71$ reachable
summaries to $36$ and $2{,}658$ transitions to $1{,}320$, still with
one colourable structurally accepting summary. This remains a Python
fixed-point result. The generic edge rule and the executable visibility
tests are proved; the complete oriented vertex interpreter and its
fixed-point certificate still require kernel verification. No
all-arity bound, global descent or generating-base verification follows.
The consumer is D2b.

\subsection{One physical vertex step, with exact component accounting}
\label{sec:physical-vertex-attachment}

The one-vertex operation needs no preferred mesh geometry. Split the
old boundary into retained ports $L$ and consumed ports $P$, and let
$R$ be the new exposed ports. An explicit cyclic enumeration
$\{0,1,2\}\simeq P+R$ constructs a one-vertex tangle with no interior
edges. Serial composition sews precisely $P$, preserves $L$, and
exposes $R$. In particular, persistent ports are not represented by a
fictitious zero-vertex wire tangle. The supplied enumeration determines
the actual rotation; the vertex accepts exactly the nonzero,
pairwise-distinct assignments on its three darts.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexAttachmentTransfer.star_rotation}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexAttachmentTransfer.star_accepts}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexAttachmentTransfer.joint_append}

Let $d$ be the number of old components met by the consumed ports.
Attaching a new vertex merges those components with the new vertex and
leaves every other component untouched. The component quotient is
therefore the untouched old quotient plus one point, and
\[
 C_{\mathrm{new}}+d=C_{\mathrm{old}}+1.
\]
This includes an empty seam, hidden components, and multiple ports at
the same vertex. Counting consumed ports in place of consumed
components would be wrong. The generic multigraph quotient theorem is
transported to the literal serial tangle through equality of all cut
sizes, not through an assumed component-update hypothesis.
\Sverified{Mettapedia.GraphTheory.FourColor.StarAttachmentComponents.componentEquiv}
\Sverified{Mettapedia.GraphTheory.FourColor.StarAttachmentComponents.component_count_profile}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexAttachmentTransfer.cutSize_append}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexAttachmentTransfer.component_count_append}

The number $d$ is executable from the complete boundary record:
restrict attention to realized zero-cut records, form the column of
bits for each consumed port, and count distinct columns. Equality of
columns is exactly old-component equality. Neither a connected-prefix
assumption nor visibility of every component is needed for this
\emph{consumed}-component count.
\Sverified{Mettapedia.GraphTheory.FourColor.BoundaryComponentCount.column_eq_iff}
\Sverified{Mettapedia.GraphTheory.FourColor.BoundaryComponentCount.count_extractTangle}

Writing $\sigma$ for the old boundary return and $\tau$ for the new
three-cycle, the boundary rewire determines
\[
 \Delta F=\operatorname{cyc}((\sigma+\tau)\,\mathrm{seam})
                 -\operatorname{cyc}(\sigma).
\]
This is the change in the \emph{total} face count, including faces
that close when consumed ports disappear. With $m=2V-I+2F$ and
$D=4C-m$, the literal constructor satisfies
\[
 D_{\mathrm{new}}-D_{\mathrm{old}}
       =2(1+|P|-2d-\Delta F).
\]
For valid vertex rotations $D$ is four times total genus, giving the
tested formula $2\Delta g=1+|P|-2d-\Delta F$. On unrestricted tangles
the theorem is an integer Euler identity; it does not silently assume
cyclicity or nonnegative genus.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexAttachmentTransfer.star_face_code}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexAttachmentTransfer.face_change}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexAttachmentTransfer.defect_change_executable}

The attachment gate exhausts $5{,}795$ small graph/port instances.
Kernel controls cover zero through three consumed ports, persistent
ports, repeated attachments, and the two-star orientation test: the
same cut profile yields three faces or one face depending on cyclic
order. The Python evaluator now uses the distinct-column component
counter; its previous closure receipts are unchanged.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexAttachmentChecks.persistent_and_new_ports_connected}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexAttachmentChecks.flat_faces}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexAttachmentChecks.twisted_faces}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexAttachmentChecks.same_cut_profile}

This closes the component/Euler identity for one constructed vertex
step, not D2b in full. The next subsection supplies the bridge from
every valid finite evaluator letter and its persistent-port coordinates
to this constructor, initialization, and iterated decoding. The required
fixed-point certificate remains open. The empty initial boundary state is
not an empty tangle with a fictitious distinguished dart; the first
vertex can be initialized as the star. Global coverage, strict
reduction, and the generating base remain open.

\subsection{Executable code runs now construct the object they describe}
\label{sec:executable-physical-code-runs}

The physical-realization gap for the finite vertex-code interpreter is
closed. A valid code partitions old ports into retained and consumed
ports, and new ports into retained and fresh ports. The two boundary
coordinate maps and the cyclic three-dart map are proved bijective;
their inverses are finite searches. The cyclic order is retained as
data, not replaced by an arbitrary enumeration of the incident darts.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodePartition.input_bijective}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodePartition.output_bijective}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodePartition.starOrder_spec}

Every nonempty well-typed code run constructs a literal two-sided
tangle: initialize with an actual star, then apply serial attachment
with the proved boundary changes of coordinates. Its extendable
boundary words are exactly the relational evaluation of the codes.
At the empty terminal boundary, acceptance is equivalent to existence
of a proper Tait colouring of that constructed closed object.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeRealization.supports_attach_iff}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeRealization.initial_support}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeTrace.Trace.support_exact}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeTrace.Trace.closed_exact}

The same construction is reflected by the executable joint evaluator:
finite colour supports, boundary-face permutation, complete capped
small-cut profile, and exact integer Euler mass all refer to the
\emph{same} object. In particular, repeated attachment does not merely
match a separately asserted transition relation.
\Sverified{Mettapedia.GraphTheory.FourColor.TwoSidedBoundaryJoint.joint_reindex}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeJoint.Trace.signature_exact}
\Sverified{Mettapedia.GraphTheory.FourColor.ExecutableJointTransfer.transfer_exact}
\Sverified{Mettapedia.GraphTheory.FourColor.ExecutableVertexCode.evaluate_physical}
\Sverified{Mettapedia.GraphTheory.FourColor.ExecutableVertexCode.accepts_exact}

The first-return implementation uses a bounded search of at most the
dart-carrier size, proved equal to the semantic least positive return.
An earlier unbounded least-witness implementation executed in compiled
code but got stuck during a multi-step kernel control; the bounded
implementation removes that dependence on proof reduction.
\Sverified{Mettapedia.GraphTheory.FourColor.FiniteFaceReturn.time_eq}
\Sverified{Mettapedia.GraphTheory.FourColor.FiniteFaceReturn.transfer_eq}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeChecks.returns_to_single_marker}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeChecks.returns_to_next_marker}

There is also a precise negative result. The unrestricted exact-mass
carrier is infinite even at frontier width three: disjoint planar
two-vertex, three-edge components give masses $4(n+1)$. This is a
refutation of unrestricted carrier finiteness, not of the connected
source route. A completed component is invisible at the boundary and
prevents later connected growth. The executable visibility test is
proved exact on decoded runs, as is the connected/bridgeless cut test;
in particular every nonempty closed run rejects \emph{extension},
without thereby rejecting terminal acceptance.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeUnboundedMass.family_width}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeUnboundedMass.unbounded_mass}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeUnboundedMass.distinct_profiles}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViability.connected_bridgeless_exact}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViability.visibility_exact}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViability.closed_rejects_extension}

The coordinate gate exhausts $20{,}820$ raw codes through width three,
finds $96$ valid codes, and compares $18{,}144$ colour pairs with the
independent transfer. Kernel controls include malformed wires,
persistent-port order, and identical colour/cut data with different
Euler mass. These are operation controls, not a configuration catalogue.
D2b remains open. Source-facing cyclic encoding, the geometric round trip,
and lossless admissible-prefix pruning are now connected in
Section~\ref{sec:source-vertex-code-roundtrip}. The exact raw/support/Nerode
closure certificates of Proposition~\ref{prop:exact-closure-gate}, their
enumerated initialization/acceptance data, and the required width interface
still require discharge. Constructing a physical object for every valid code
does not by itself prove those remaining obligations.
No optimized threshold or generating base is inferred.

\subsection{A finite carrier for viable planar prefixes}
\label{sec:viable-planar-prefix-carrier}

The unrestricted exact-mass carrier is infinite already at frontier width
three, so a finite state space cannot come from the width alone. It comes
instead from an admissibility test on the evaluator's own coordinates: no
component hidden from the boundary, and exact integer Euler mass equal to
four times the number of components visible there. The visible count is the
number of distinct zero-cut columns, which never exceeds the interface size,
so the test itself forces $0\le m\le 4w$. The range is \emph{derived} from
the test; no mass bound is assumed of the carrier.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.visibleComponents_le}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.mass_range}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.viable_finite}

Finiteness is what a closure computation needs, but the measured bound
\[
 2^{4^{w}}\cdot w!\cdot 2^{12\cdot 2^{w}}\cdot(4w+1)
\]
also shows what it does not license: a certificate must replay the
\emph{reachable} states, never enumerate $Q$.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.viable_card_le}

On decoded runs the test means what it is meant to mean. Its integer
coordinate is the literal Euler mass of the constructed tangle; when nothing
is hidden, its column count is that tangle's actual number of connected
components; and the conjunction holds exactly when every component is exposed
at the boundary and the Euler defect vanishes.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.mass_exact}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.visibleComponents_exact}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.viable_exact}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.viable_iff_visible_planar}

Both halves are load-bearing and neither implies the other. A bare star
passes; so does a prefix that is genuinely disconnected, since the test
bounds the number of components rather than demanding connectivity. A
completed planar component behind the frontier fails the mass equation
because it contributes no column. A completed \emph{handle} behind the
frontier \emph{satisfies} the mass equation exactly, its two units of defect
cancelling against the component it hides; only the visibility half rejects
it. Dropping visibility would therefore admit nonphysical prefixes, and
dropping the mass equation would leave the carrier unbounded.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.star_viable}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.disjoint_viable}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.hidden_planar_not_viable}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.hidden_handle_passes_mass_test}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.hidden_handle_not_viable}

This is a prefix predicate and not an accepting one. Every closed run fails
the extension test, so initialization and terminal closure remain separate
obligations and this predicate must never be used as the accepting array $F$
of Proposition~\ref{prop:exact-closure-gate}.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeViablePrefix.closed_not_viable}

Finiteness alone does not establish that pruning is lossless. Visibility
is inherited by prefixes of a connected object; the planar half requires
monotonicity of the Euler defect under literal vertex attachment, equivalently
\[
 \Delta F\le 1+|P|-2d
\]
in the notation of \S\ref{sec:physical-vertex-attachment}. This was the
remaining obstruction at the finite-carrier checkpoint; it is now proved
in Section~\ref{sec:vertex-code-lossless-pruning}.

Its content is narrower than it looks. Writing the seams' face effect as
$\Delta F=1-\#\mathrm{merge}+\#\mathrm{split}$ with
$\#\mathrm{merge}+\#\mathrm{split}=|P|$, the slack is
$1+|P|-2d-\Delta F=2(\#\mathrm{merge}-d)$, so the inequality says exactly
that at least $d$ seams merge two faces --- which is the contrapositive of
the already-proved fact that darts sharing a face share a component. The
required step is a presentation bridge: identifying the vertex, face,
component and edge counts of a
constructed tangle with the orbit counts of its edge presentation, after
which the proved monotonicity and nonnegativity of the edge-presentation
defect apply verbatim. Concretely, the tangle defect is twice the
edge-presentation defect once the rotation orbit count is the vertex count
and the word-orbit count is the component count.

The invariant those two identifications rest on is now available. A
constructed run has one rotation cycle per vertex: every vertex is born as a
star whose rotation is a three-cycle, and neither boundary reindexing nor
serial composition alters the rotation at an existing vertex, both being
conjugation by a carrier equivalence and a disjoint union respectively. This
is the tangle-level form of the condition the source-facing encoder supplies,
and it is exactly the hypothesis under which the capped ambient presentation
already identifies its word-orbit count with a component count.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeRotationCyclic.star_rotationCyclic}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeRotationCyclic.reindex_rotationCyclic}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeRotationCyclic.serialCompose_rotationCyclic}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeRotationCyclic.realize_rotationCyclic}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeRotationCyclic.realize_openRotationCyclic}

The transport and losslessness on constructed runs are now proved in
Section~\ref{sec:vertex-code-lossless-pruning} below. The rotation-faithful
source round trip is now proved in Section~\ref{sec:source-vertex-code-roundtrip}.
No closure count, threshold
or generating base is claimed,.

\subsection{Euler transport and lossless vertex-code pruning}
\label{sec:vertex-code-lossless-pruning}

The four counting identifications now hold for an actual open tangle whose
vertex map is surjective and whose vertex fibers are rotation cycles. Its
provisional edge involution fixes boundary darts; its word orbits are the
components of the side multigraph, its rotation orbits are the displayed
vertices, and twice its edge-list length is its interior-dart count. The
resulting identity, including disconnected objects and hidden components, is
\[
 D_T(T)=2D_M\bigl(\rho_T,\operatorname{edgeList}(\alpha_T)\bigr).
\]
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleEulerPresentation.wordOrbitCount_eq_components}
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleEulerPresentation.twice_edges}
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleEulerPresentation.face_count}
\Sverified{Mettapedia.GraphTheory.FourColor.OpenTangleEulerPresentation.defect_eq_twice}

Changing dart coordinates preserves defect, and disjoint union adds it.
Restricting an involution to any invariant subset of its edges cannot increase
defect. These assertions are independent of the chosen edge-list order.
On the actual serial constructor, erasing the seams recovers the disjoint
union on the same dart carrier. Sewing therefore has defect at least the
sum of the two input defects. A newly attached star has defect zero.
\Sverified{Mettapedia.GraphTheory.FourColor.InvolutionEulerDefect.defect_restrict_le}
\Sverified{Mettapedia.GraphTheory.FourColor.InvolutionEulerDefect.defect_congr}
\Sverified{Mettapedia.GraphTheory.FourColor.InvolutionEulerDefect.defect_sum}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeDefectMonotone.serial_defect}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeDefectMonotone.step_mono}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeDefectMonotone.face_change_bound}

Every realized run has a surjective vertex map as well as cyclic vertex
rotations, so its defect is nonnegative and monotone along prefixes. A
zero-defect \emph{completion} consequently forces all earlier defects to
vanish. The terminal hypothesis is essential: starting at a zero-defect star
alone does not prevent subsequent handle creation.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeDefectMonotone.realize_surjective}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeDefectMonotone.realize_defect_nonneg}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeDefectMonotone.planar_prefix}

A hidden component stays hidden after attachment and is separated from the
new vertex. Thus every \emph{proper} prefix of a connected planar constructed
run passes the finite viable-prefix test. No colourability assumption occurs
in this theorem: it also retains any uncolourable object satisfying those
structural hypotheses. The executable pruning schedule tests a prefix only
when attaching another vertex, and leaves terminal acceptance unchanged.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeLosslessPruning.hidden_extension}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeLosslessPruning.viable_of_connected_planar_completion}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeLosslessPruning.prefixesViable_of_connected_planar}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeLosslessPruning.prunedAccepts_eq}

This closes the defect-monotonicity and losslessness gap on decoded runs.
The source bridge below connects these results to actual source sweeps;
it does not produce the optimized closure certificate. The latter still
requires its physical alphabet, exact raw and cumulative-support semantics,
and all three kernel-certified closure counts. In particular the evaluator's
set of boundary colourings is already a support object; it must not be
reported as the raw carrier merely by renaming it.

\subsection{The accepting predicate for the direct Count system}
\label{sec:direct-count-acceptance}

The prefix predicate is deliberately not an accepting one. The separate
terminal test reads the same four coordinates: the constructed closed object
is connected and bridgeless, its exact Euler mass is four, and it carries a
proper Tait colouring. Each clause is identified with the literal property of
that object.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeTerminalAcceptance.accepts_exact}

Connectivity is what turns the mass clause into a planarity test. Mass is
four times the component count minus the Euler defect, so at one component
mass four says exactly that the defect vanishes; without the connectivity
clause, two components sharing one handle would pass.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeTerminalAcceptance.card_component_eq_one}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeTerminalAcceptance.mass_four_iff_planar}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeTerminalAcceptance.accepts_iff_planar_colouring}

The controls are the same two-vertex three-edge pair as before. The planar
map is accepted; its handled twin, with the same colour support and the same
cut data, is rejected. Colour acceptance alone cannot separate them, which is
why the accepting array may not be the colour test on its own. Accumulated
closed components are likewise rejected, their mass being four per component.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeTerminalAcceptance.flat_accepted}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeTerminalAcceptance.twisted_rejected}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeTerminalAcceptance.two_components_rejected}

This supplies the terminal predicate for the direct Count system of
Proposition~\ref{prop:exact-closure-gate}. Its source-realization
biconditional is now proved below: on a complete sweep of a source in the
bridgeless spherical cubic class, terminal acceptance is exactly source
Tait-colourability. The finite enumerated accepting array is still owed
alongside $I$, $E$ and the reachability replays. No closure count is supplied,
and this is not the Lift/Seed accepting array, which remains the cap-composed
predicate.

\subsection{Source-facing cyclic encoding}
\label{sec:source-facing-cyclic-encoding}

The interpreter reads a code's three slots as a cyclic order: the realized
star satisfies $\rho(\text{slot }i)=\text{slot}(i+1)$. An encoder that merely
enumerates the three incident darts by cardinality therefore need not
describe the source rotation. The discrepancy is invisible to the colour
test, which asks only that the three darts carry distinct nonzero colours,
and it is invisible to the cut records; it is \emph{not} invisible to the
Euler mass, as the two cyclic orders of one incidence pattern already give
masses four and zero on identical cut data. An encoder for this interpreter
consequently owes a rotation statement, not merely an incidence statement.

At a cubic vertex whose stored rotation is genuinely one cyclic order per
fiber, no dart is fixed, so the restricted rotation has order three and the
three darts are $d$, $\rho d$, $\rho^{2}d$. That enumeration is a bijection
with the new darts and its successor is the source rotation.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeCyclicEncoder.rot_ne}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeCyclicEncoder.rot_three}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeCyclicEncoder.cyclicStarEquiv_succ}

Encoding is factored through an arbitrary supplied enumeration, so the
earlier encoder is one instance and nothing proved about it is lost. Star
injectivity, wire injectivity, the forced boundary colour and the finite
colour test hold for every choice; the last of these is precisely why a
wrong rotation cannot be detected downstream by colour data alone.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeCyclicEncoder.encodeWith_starEquiv}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeCyclicEncoder.encodeWith_codeAccepts_iff}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeCyclicEncoder.encodeWith_count}

The emitted letter also lies in the interpreter's domain. Each remaining
incidence clause is an actual dichotomy about ambient darts: a new boundary
port is fresh exactly when it is based at the new vertex, and an old boundary
port is consumed exactly when its mate is, so consumption and persistence are
exclusive and exhaustive.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeCyclicEncoder.encodeWith_inr_iff}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeCyclicEncoder.encodeWith_inl_iff}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeCyclicEncoder.encodeWith_some_iff}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeCyclicEncoder.encodeWith_valid}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeCyclicEncoder.cyclicEncode_valid}

These are the single-letter ingredients for source coverage. The following
construction supplies their run-level round trip. Cyclicity is not inferred
from bare cubicity: it is the vertex-rotation field already required by the
route's bridgeless spherical cubic source class.

\subsection{Geometric source round trip and source-prefix retention}
\label{sec:source-vertex-code-roundtrip}

Every nonempty duplicate-free vertex sweep of a cubic rotation map with one
rotation cycle per vertex now constructs an actual valid code trace. The
sweep processes its tail first, in the order of the existing source Count
recurrence. Output boundary coordinates may be supplied arbitrarily;
intermediate finite coordinates and a cyclic root dart at each new vertex
are chosen within the construction. There is no planarity, colourability,
or uniform-width assumption in this encoding theorem.
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodeTrace.source_covered}

The induction invariant is a geometric isomorphism, not merely agreement of
colour support. It bijects decoded vertices with swept source vertices and
decoded darts with retained source darts, commutes with the source rotation
and internal edge flip, and identifies each output port with its prescribed
source boundary dart. Initial stars and actual attachment steps construct
the invariant. Persistent ports retain their names and consumed ports become
the two actual seam darts. The ambient distinguished outer dart is not
preserved as a marking: it need not belong to a proper swept side. The
literal side below is pointed at the image of the decoded outer dart.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeDartGeometry.stepDarts_consumed}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeDartGeometry.stepDarts_retained}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeSourceGeometry.nextDart_port}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeSourceGeometry.nextDart_rho}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeSourceGeometry.nextDart_mate}

This invariant is transported to the existing literal vertex-side tangle.
The dart equivalence conjugates the open face permutation; the internal-dart
and vertex equivalences identify Euler mass; graph steps and reachability
identify connected components. Consequently the two objects have exactly the
same Euler defect, including for disconnected swept sides.
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeSourceSemantics.sourceDarts_phi}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeSourceSemantics.source_faces}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeSourceSemantics.source_mass}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeSourceSemantics.source_reach}
\Sverified{Mettapedia.GraphTheory.FourColor.VertexCodeSourceSemantics.source_defect}

For a connected spherical source with cyclic vertex rotations, the decoded
complete sweep is connected and has zero defect. The earlier lossless-prefix
theorem therefore applies with hypotheses on the \emph{source}, not on an
assumed decoded surrogate. Every proper prefix passes the viable-prefix
test. No Tait colouring is assumed: a hypothetical counterexample is retained
as well. The cap-stable source class used by vertex minimality supplies all
these hypotheses, and its coverage corollary chooses a complete vertex sweep
and constructs the surviving closed trace itself.
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodePlanar.model_planar}
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodePlanar.model_connected}
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodePlanar.source_prefixes_viable}
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodePlanar.class_covered}

The remaining structural terminal clause also transports. The model's dart
and vertex equivalences induce an isomorphism of the literal decoded
multigraph with the source vertex-side multigraph, including edge identity.
Ambient bridge-freeness makes that side boundary-essential; on the complete
sweep its boundary type is empty, so an alleged bridge would construct an
element of an empty type. Therefore the decoded complete sweep is bridgeless,
independently of any colouring. Pulling and pushing edge colours through the
full dart equivalence then identifies decoded and source Tait-colourability.
Combining connectedness, zero defect, bridgelessness and colour transport gives
the promised exact terminal statement.
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodeTerminalAcceptance.sourceSideIso}
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodeTerminalAcceptance.model_bridgeless}
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodeTerminalAcceptance.tangle_taitColorable_iff}
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodeTerminalAcceptance.encoded_terminal_acceptance_iff}
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodeTerminalAcceptance.class_terminal_covered}

There is also a conservative source-derived finite envelope. Every frontier is
a subtype of the ambient dart carrier, hence has width at most $|D|=3|V|$ for
a cubic source, and the decoded trace has exactly one vertex per source
vertex. Thus a source with at most $N$ vertices has a viable, semantics-
preserving trace with at most $N$ vertices and every frontier of width at most
$3N$. This is intentionally not an optimized branchwidth or pathwidth claim;
it is the finite length--width box required before raw machine states and
letters can be enumerated.
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodeBounds.boundaryDart_card_le}
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodeBounds.model_vertex_card}
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodeBounds.class_covered_within_vertex_bound}
\Sverified{Mettapedia.GraphTheory.FourColor.SourceVertexCodeTerminalAcceptance.class_terminal_covered_within_vertex_bound}

An independent dart-level control checked $2{,}000$ generated source maps
and $17{,}740$ prefixes with arbitrary sweep orders, cyclic roots and boundary
coordinates: no round-trip failures, including $7{,}792$ disconnected
prefixes. Sources were not restricted to planar or colourable maps. Reversing
only the encoded cyclic order preserves incidence but is detected by the
rotation comparison. These are sampled constructor controls, not exhaustive
closure computations or configuration data.

The construction now bounds the maximum frontier and trace size conservatively,
but it does not yet enumerate the finite raw state and letter carriers or
identify raw, cumulative-support and Nerode closures with one another. The
enumerated initial/accepting arrays, exact transition semantics, reachability
replays and all three closure certificates remain to be supplied. D2b is a
package of those obligations rather than one linchpin, and remains open.
