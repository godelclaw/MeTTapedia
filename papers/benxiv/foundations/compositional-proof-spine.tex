...
provides a finite record of the required observation. It does not yet
replace the marked-state choice in the geometric supplier.
...
\paragraph{Remaining geometric obligation.}
The supplied Count seam must still be chosen using compatible named-face
records on its two actual shores, with the corresponding naming and port
coordinates. Existing equality of Count support alone is not asserted to
imply this refinement. Nor does preservation of cofaciality preserve an
entire hole word, facial length, or all designated face darts. The result
removes perimeter from this particular observation's local state cost;
it does not yet remove perimeter from the supplied reduction's threshold.
D1b/D1c, fixed-mesh exclusion and the generating base remain open, with
the fixed ledger unchanged at $57/100$.
