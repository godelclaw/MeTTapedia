...
matching.  It attaches a particular ambient matching, containing the selected
mesh step, to each concrete colouring in an adjacent-pair Kempe orbit.  Hence
the already constructed ordered-mesh sites can now be read by the
minimum-residual-defect matching calculus.  What is not yet proved is the
wall-facing statement: that the matchings obtained at neighbouring sites can
be synchronized so that symmetric difference strictly lowers residual
quotient oddness, or instead yields equal exact shore supports for a physical
replacement.
