...
Fix a requested separator bound $B\geq0$ and spacing $s\geq0$.  If a proper
...
\begin{enumerate}
\item the site has a realized cyclic edge cut of width at most $3B$;
\item two strictly nested physical returns on one facial shore have inner
      return separator support larger than $B$;
...
bounded interface is an explicitly long physical return separator.
\end{corollary}
...
each vertex of the inner separator.  If that separator has at most $B$
vertices, the cut has width at most $3B$; otherwise it gives the second horn.
Finally, $2(s+1)(1+1)^2=8(s+1)$.
\end{proof}
...
sweep trichotomy from
Corollary~\ref{cor:geometric-residual-return-sweep-alternative}, at every
requested separator bound and spacing.  Moreover, at each internal row
...
long-separator, and repeated-state horns uniformly, nor does it assert that
choices made at adjacent junctions define nested or disjoint shores.
\end{proof}
...
the physical separator is short.  A long physical separator is retained as
its own explicit horn rather than being mistaken for a bounded interface.
One still has to compress that long-return horn, choose interfaces coherently
...
therefore a graph-level local trichotomy at one alternating site; it proves
