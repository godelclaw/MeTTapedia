...
third edge is the edge of the reference matching.  Its other endpoint cannot
...
third edge, so its neighbour is the reference partner.  If that partner lay
on the carrier, closure of the carrier under the reference matching and
involutivity would put the internal path vertex on the carrier.  But a simple
physical return meets the carrier only at its two endpoints, contradicting
strict internality.  The earlier separator alternative now has only its
face-side branch.
\end{proof}
