...
\begin{proposition}[target-specific square transport]
\label{prop:target-square-migration}
\Sopen
For every facial square arising in the least \(\mathrm{WC}^{*}\) obstruction
of the source ladder, and for the source's actual extendable-word target
\(W\), every state in a nonempty target-free lifted reduction side can reach
a target-bearing side by legal ambient Kempe moves.
\end{proposition}

The refuted statement \(A\ne\varnothing\) would imply this proposition,
whenever the all-equal overlap itself contains a target state.  It is not
necessary:
the first counterexample above has \(A=\varnothing\) but one side is empty,
and its expanded colouring graph is connected.  Nor may the qualifier
``least \(\mathrm{WC}^{*}\) obstruction'' be dropped: the Frucht specimen
has two nonempty connected sides and no path between them.  A repaired proof
must exploit the target predicate or the least-obstruction hypothesis, or
show that the irrelevant side contains no target obstruction.  It may not
restore either universal \(A\ne\varnothing\) or universal SQ3 migration.
...
\text{target-specific }\mathrm{WC}^{*}\text{ square transport}&:&\text{open}.
\end{array}
\]
