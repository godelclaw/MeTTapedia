...
An edge $e$ is a loop of $M$ if and only if it is a bridge of $M^{*}$.
Consequently the dual of a loopless map is bridgeless.
\end{corollary}
...
Steps~1 through~4 are algebra over $\mathbb{F}_2^2$ resting on
Theorem~\ref{thm:cycle-space}, which is already machine-checked; formalizing
them is bounded work with no research content.  The other ingredients are now
