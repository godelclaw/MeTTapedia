...
Corollary~\ref{cor:loop-bridge} supplies dual bridgelessness.  The remaining
classical boundary is therefore exactly the named representation bridge of
Lemma~\ref{lem:planar-spherical-presentation}, together with its routine
componentwise wrapper.  No legacy \texttt{IsPlanar} premise is used.
...
restriction to old vertices are now checked.  What remains outside the
connected spherical statement is solely the adapter from an abstract planar
graph to a spherical rotation-system presentation, recorded explicitly in
Lemma~\ref{lem:planar-spherical-presentation}.
\end{remark}

\begin{remark}[how this closes the classical debts]
With Corollary~\ref{cor:two-sided} established, the route's internal
graph-backed presentation and the connected-spherical form of
Theorem~\ref{thm:tait-reduction} are checked.  The only classical boundary
still named in the theorem statement is the standard finite
plane-embedding representation theorem of
Lemma~\ref{lem:planar-spherical-presentation}; keeping it named prevents the
refuted legacy planarity predicate from re-entering the headline.
\end{remark}
