...
frame, and splits the spatially varying frame multiplier into its frozen part
and an explicit frame-variation remainder.  A spectral-gap perturbation theorem
quantifies that remainder's eigenframe input.  The
...
The module \texttt{PancakeDyadicKernelScaling} starts the passage from this
base chart to actual dyadic shells.  For every positive $r$ it proves
\[
...
periodization.  It also constructs the square-dyadic chart and checks that
at aperture $\delta=1/N$ its source and receiver frequencies are exactly the
physical lattice frequencies divided by $N^2$.  What remains at this endpoint
is the four-transverse/two-axial anisotropic rescaling, finite chart/rotation
assembly, and the variable-frame remainder.
...
summation and isotropic dyadic scaling are supplied by the two subsequent
modules above.  The remaining field-transfer pieces are the anisotropic
square-dyadic rescaling, finite chart/rotation assembly, and control of the
variable-frame remainder.  They are not silently included in the modewise
estimate, and \texttt{coherent\_le} has not yet been instantiated.
...
coherent PDE interaction, mass-preserving isotropic dyadic rescaling, and its
cover-side periodized coefficient formula are now proved as well.  The
remaining steps are the four-transverse/two-axial square-dyadic rescaling,
finite chart/rotation transference, and a
quantitative bound for the spatial eigenframe-variation remainder, followed
by their assembly into \texttt{coherent\_le} for the actual Navier--Stokes
functionals.
...
headlines are checked by the Navier--Stokes live surface and are being added
to the same audit boundary.  The live surface builds green.  Earlier
...
spatially microlocal control of the variable-frame remainder and a dynamical,
time-integrated control of the vorticity-direction misalignment share, strong
enough to cover the gap band identified above.  It must also finish the
 anisotropic square-dyadic and finite chart/rotation assembly of the
now-periodic bilinear kernel rather than assigning the aperture gain to the
projection.
Section
...
\texttt{PancakeFiniteFourierBilinearTransfer}, and
\texttt{PancakeDyadicKernelScaling}), each with a regression companion where
