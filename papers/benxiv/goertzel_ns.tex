...
separated projector is now measurable; the collision band remains), passage
...
sign-invariant projector on every positive-gap region are now checked.  The
...
The remaining field-level steps are the smooth-projector collar, aggregation
of the small-gap branch over space and time, construction of the cellwise
...
the pointwise strain-derived spectral frame on the collision band (the
separated sign-invariant projector is now measurable), pass from the hard
...
\texttt{PancakeSpectralProjectorRegularity} modules), each with a regression companion where
