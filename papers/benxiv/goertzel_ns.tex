...
The pair-indexed complex Biot--Savart stretching polynomial is now also
collected over equal output frequencies into a canonical coefficient field;
reconstructing that field is proved exactly equal to the existing physical
stretching sum.  It therefore supplies the proposed stretching term in the
two-channel residual, connecting the block-direction equation directly to the
audited coherent/misaligned stretching split.
...
this canonical $G$: every retained source/receiver pair is collected in the
...
complex stretching polynomial.  Substituting this $G$ in the three-channel
direction theorem links the same-block direction residual to the existing
coherent/coherent plus three-term misalignment expansion.  The next step is
to identify the remaining coefficient evolution with transport and viscosity
in the true periodic vorticity equation, with all unit-torus constants fixed.
...
\texttt{PancakeFourierStretchingEvolution} modules), each with a regression companion where
