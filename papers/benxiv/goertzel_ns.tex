...
cardinality-free $C/N$ endpoint uniformly to those frames.  It also splits the
spatially varying frame multiplier into its frozen part and an explicit
frame-variation remainder.  A spectral-gap perturbation theorem
...
actual frequency decomposition, and controlling a frame that varies in
space-time, remain separate steps.
...
field-transfer pieces are finite oriented-chart selection/patching and control
of the variable-frame remainder.
...
frozen oriented-frame transference are now proved as well.  The remaining
steps are finite oriented-chart selection/patching and a
...
\texttt{PancakeFrozenFrameKernelTransfer}), each with a regression companion where
