...
physical frames, and applied exactly to the localized products.  Control of
the Fourier coefficients of the concrete spatial cell cutoffs and the remaining smooth
projector collar, space--time treatment of the small-gap branch, and
...
This closes the finite-band localization junction.  It does not turn a
nonconstant spatial cell cutoff into a finitely supported Fourier polynomial:
a genuinely localized smooth cutoff has an infinite Fourier tail.  The next
analytic step is therefore a quantitative truncation/tail estimate, not
another support-coverage lemma.  The operator side of this approximation is
already checked in \texttt{PancakeBilinearActionStability} and
...
cutoff-specific Fourier decay input and the subsequent smooth
sector-projector collar remain at this junction.  More precisely,
...
frame because rotations preserve the Euclidean norm.  The remaining
mathematics here is therefore not convergence bookkeeping: it is to construct
the actual spatial cover cutoffs and prove absolute summability (or an
explicit derivative-decay estimate) for their Fourier coefficients.
...
to it with no support-cardinality loss.  The infinite Fourier tail of a true
spatial cell cutoff and the smooth projector collar remain; a checked physical
operator-stability theorem now converts any uniform cutoff-tail estimate into
the required multiplier error.  Absolute summability is already proved
sufficient for a scale-compatible finite approximation, uniformly over all
frames; that summability must still be established for the concrete cell
cutoffs.
...
localization-stability and smooth-cutoff Fourier-tail modules), each with a regression companion where
