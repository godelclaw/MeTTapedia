...
\paragraph{In progress and open.} Current work extracts exact cut profiles
(vertex-side, boundary-face, all-face) and graph-derived corridor cut
profiles with splice observables, bounding face fragments by interface
width. The endgame of the route is open: the fusion-chain and
chiral-monodromy composition, Trail Completability itself, the L1--L10
dependency chain into the minimal-counterexample contradiction, and a proved
Short-Strand replacement. The v23-era collar obligations are taken up only
where this route consumes them. Nothing in this section claims the
Four-Color Theorem or the completion of the v24 route.
