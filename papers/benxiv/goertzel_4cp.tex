...
annular cellulation.  Thus, \emph{if this two-cap laboratory is used as a
formation adapter}, its next datum is not ``more transport through the
existing opening'', but two suitably separated \emph{facial} pentagon caps
with their oriented face cycles.  From those, surface surgery can construct
the two hole orbits and compare the opened Euler data; without them, treating
the graph-level opening as an annulus would be precisely the unproved picture
...
stubs alone make an annulus.  The unbuilt source-facing part remains selecting
such caps from the relevant minimal frontier, constructing the two named hole
faces and their rotation-level embedding, and transporting the chosen clean
corridor and its local hypotheses.  Addendum~V itself treats the two-cap
