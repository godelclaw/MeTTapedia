...
the Cell--3 center that generated it.  This provenance is the exact local
input needed by the existing remote-center exclusion theorem; the resulting
remote noncollision theorem and the two bounded neighboring cases are not yet
claimed.  A literal successor theorem carries each outgoing flank to an
incoming flank of the next cell on the same ambient edge and exterior face,
then returns the next outgoing flanks.  Lengthwise simplicity and end-cap
...
local windows.  Existing remote rail-support disjointness results and the
source's length coordinate provide ingredients; the formation-to-graph theorem
joining them is the live obligation.
