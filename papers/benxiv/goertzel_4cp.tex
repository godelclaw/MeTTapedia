...
long/end-capped instance from the actual minimal-counterexample annulus and its
Cell--3 corridor.
...
The literal Cell--3 rail construction belongs to the L1 corridor realization.
Its remote noncollision is proved, and its adjacent append has been reduced to
\texttt{CommonNeighborsExact}.  An extra common neighbour would form precisely
the small separating/nonfacial dual triangle excluded by v23's
minimal-counterexample radius--one collar normal form.  The present open-annular
...
provenance from which that bounded-interaction fact is derived.  Connecting
the closed source normal form to the open L1 corridor---rather than adding the
classification as a new assumption---is the live formation-to-graph
obligation.
...
the source.  It does
\emph{not} by itself prove \texttt{CommonNeighborsExact} for the Cell--3
carrier: that carrier has degree-one boundary stubs and is provably
incompatible with the closed cubic minimal-counterexample structure.  The
remaining statement must therefore transport the classified face triangle
through an explicit closed-map-to-open-annulus formation, preserving the
relevant interior faces and their shared boundary edges.  Adding minimality
as a field of the open instance would be inconsistent, not a shortcut.
...
conditional theorem a formation result.  The missing bridge must construct
the deleted-edge/open rotation data from the closed map and prove that faces
remote from the deleted edge retain their orbits and shared edges.
...
by a matching cardinality or assumed through a wrapper.  What remains is
source-specific: prove that the relevant Cell--3 interior faces satisfy the
retention criterion under the actual closed-map-to-annulus formation, then
transport the closed bounded-interaction classification to the literal L1
corridor and assemble its separated facial-dual crosscuts.  These generic
theorems remove one formation gap; they do not themselves construct the
crosscut pair, prove the L1 corridor comes from that opening, or assert that a
flank face cut by the opening is wholly retained.

The first carrier-changing part of the \emph{single-edge deletion} model is
also now constructed, rather than postulated.  The generic permutation lemma
...
a local transport theorem.  It does not yet transport planar-geometry fields,
choose the source's missing edge, construct its container cycles or two holes,
prove that the relevant Cell--3 face orbits avoid the deleted darts, or realize
the long separated crosscut pair.  Those source-specific formation obligations
remain the live gate.

The one-step result has now been lifted to complete untouched faces in
...
Thus deletion induces exactly the old facial-dual adjacency relation on the
family of untouched face orbits: it neither loses a shared edge nor invents
one between their images.  This is enough to transport any closed-map dual
triangle once its three faces are proved untouched.  It does not establish
that source-specific avoidance condition, nor show that every possible common
neighbour in the open Cell--3 map belongs to the untouched family.  Those are
the next local bridge statements.
