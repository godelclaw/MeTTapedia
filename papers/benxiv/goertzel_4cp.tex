...
incidence enumeration --- graph-level Kempe-step soundness remains a named open
obligation; a proper-subspace strictness theorem against a larger unrestricted
...
inclusion (Theorem~4.9) remains a named open target; and a length-2/3
...
audited in an encoded model whose graph fidelity is the named next obligation,
and its most dangerous assembly seam pinned down as broken in that model. A
companion scope note records the v24 working material: its total-curvature
identity is proved, its at-most-twelve pentagon budget requires an explicit
scope gate, and Trail Completability with its L1--L10 dependency package is the
live conditional target.
\end{abstract}
...
v23 $\Wz(H)$ inclusion, and manuscript S2 statement remain named open
obligations. All with no \texttt{sorry} and axioms
...
\paragraph{v24 scope.} For the v24 route, the total face-curvature identity is
proved under explicit cubic spherical incidence and Euler data. The claimed
at-most-twelve pentagon budget is false under minimum face size five and
requires the stated fullerene or zero-negative-curvature scope gate. The Short
Strand addendum is recorded as externally refuted; the kernel-checked datum by
itself refutes the old bound~4. Trail Completability, the L1--L10 dependency
package, the actual-collar cut lift, and effective GWCO threshold coverage
remain open.
