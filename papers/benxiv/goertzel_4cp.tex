...
The possible transport problem is now precise, but it is not currently a
source-critical construction.  Addendum~XXV's Seed Lemma first reduces a
minimal annular-frontier counterexample only to girth at least five and no
2-cuts; its L1 is then a finite classification producing a bulk all-hexagonal
ladder.  It does not supply cyclic five-edge-connectivity, a closed
graph-backed minimal map, or a cap opening.  Consequently this closed theorem
cannot yet discharge \texttt{CommonNeighborsExact} on the open carrier.  Such
a use would first need a source-faithful finite L1 collar classification that
handles the separating-three-edge-cut case explicitly, rather than silently
excluding it through stronger closed-map minimality.  The degree-one boundary
stubs also make adding that minimality directly to the open instance
inconsistent, not a shortcut.
