...
their vertex rotation, and the chosen outer dart across any graph isomorphism;
\texttt{openingGraphData} applies it to the literal two-cap graph.  Thus the
toolchain graph is no longer missing a rotation merely because the computed
open-region graph has a different vertex type.  This is deliberately below
the annular claim: a graph isomorphism transports no distinguished hole faces,
their cyclic order, or Euler proof.
