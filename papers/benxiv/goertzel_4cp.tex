...
There is, however, now a direct L1 handoff which narrows this question further.
...
The theorem
\texttt{exists\_sourceRealizedBoundaryCleanOrbitHexCorridor\_of\_weightedL1}
...
real L1 conclusion, conditional only on the stated quantitative bound; it is
not yet either a pair of separated transverse crosscuts or a splice.
...
weighted L1 theorem now returns a source-realized boundary-clean corridor from
those fields.  The missing bridge is instead from that literal corridor to the
long separated transverse crosscuts needed by the splice.  The generic
...
current Cell--3 L1 path already has its own direct annular realization.  Neither
route supplies the long, end-capped crosscut pair, which remains the live
