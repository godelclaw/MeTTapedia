...
local windows.  Nor is the live problem the general existence of two disjoint
graph paths: the required primal pair is already constructed.  The missing
bridge is the source-faithful ribbon-boundary dualization of that primal
witness.  The literal Cell--3 rail construction is the local candidate for
this bridge.  Its remote noncollision is proved, and its adjacent append has
been reduced to \texttt{CommonNeighborsExact}.  An extra common neighbour
would form precisely the small separating/nonfacial dual triangle that closed
minimal-counterexample theory is designed to exclude.  Those exclusion
theorems exist for a closed graph-backed minimal counterexample, but the
present open-annular \texttt{Instance} does not retain that provenance.
Connecting the closed minimal-counterexample hypotheses to the open annular
corridor---rather than adding the classification as a new assumption---is the
live formation-to-graph obligation.
...
exactly the three edges read from the dual cycle.  This is the closed-map
bounded-interaction classification needed by the source.  It does
...
that the complete ambient and open dart cycles are equivalent.  It also proves
an exact edge-incidence statement: two transported face darts name the same
computed open edge if and only if they named the same ambient edge.  Thus an
untouched face is transported as an actual finite orbit without splitting or
identifying its old boundary edges, not merely identified by a matching
cardinality or assumed through a wrapper.  What remains is source-specific:
prove that the Cell--3 flank faces satisfy the retention criterion, apply the
edge transport simultaneously to their shared boundary, and use the resulting
ribbon boundary to turn the existing primal radial witness into the two
separated facial-dual crosscuts.  The new theorem removes one formation gap;
it does not itself construct the crosscut pair.
