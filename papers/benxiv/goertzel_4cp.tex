...
the source.  Combined with the new equivalence, it names the remaining bridge
sharply: the three canonical shared edges of the closed dual triangle must be
transported through the actual source opening, after which the cubic rotation
at their common vertex must identify the two centre-to-neighbour edges with
the two literal flank edges.  It does \emph{not} by itself prove
\texttt{CommonNeighborsExact} for the Cell--3
...
conditional theorem a formation result.  For Cell--3, the missing bridge must
construct the actual annular subregion obtained from the closed map (including
its cap or region deletion), identify the displayed L1 corridor inside that
opening, and prove that the relevant interior faces and their shared edges are
the transported closed-map objects.
