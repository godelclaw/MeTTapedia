...
\begin{enumerate}
  \item \textbf{Derive the extraction/recombination constant on the actual map.}
    ...
    rooted branching count, not a total-contact bound. \emph{What settles it:}
    first select one source-faithful relation policy and exact block quotient
    (or label the choice as a repaired construction), then supply the actual
    dimension-$16$ lattice Wilson representatives, quotient basis, and dual
    jets, prove their uniform conditioning, and prove the weighted Wilson
    cumulant/tree estimate in the same norm. The resulting actual constant can
    then be checked against $C\,b^{\,3-\dmax}\leq1/3$.
...
instance) --- all in the same namespace, all axiom-guarded to
$\{\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}\}$ with no
\texttt{sorry}. The primary mathematical source is Ben Goertzel's manuscript
