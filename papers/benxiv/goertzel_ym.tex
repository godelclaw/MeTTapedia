...
but v14 does not provide those data for the dimension-$16$ action sector.
...
Those action-sector matrices and uniform bounds are absent from v14. Hence the
...
\begin{itemize}
  ...
  \item \textbf{The extraction realization and transfer obstruction.} The
    ...
    (\texttt{ExtendedExtractionTransfer}).
...
\begin{enumerate}
  \item \textbf{Derive the extraction/recombination constant on the actual map.}
    ...
    supply the dimension-$16$ invariant basis and dual jets, prove their
    uniform conditioning, and prove the weighted Wilson cumulant/tree estimate
    in the same norm. The resulting actual constant can then be checked against
    $C\,b^{\,3-\dmax}\leq1/3$.
