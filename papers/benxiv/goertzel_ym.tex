...
is taken as an input to the checked arithmetic. The manuscript's \emph{displayed}
bound for that norm prints the summand $(r_1/r_0)^{-d}$, which for $r_1/r_0=1/2$
would grow like $2^{d}$; read literally this is an apparent obstruction. That
printed exponent is a sign erratum: the intended Cauchy estimate uses
$(r_1/r_0)^{+d}$, a convergent geometric series summing to $1/(1-r_1/r_0)=2$, so
$C_{\mathrm{extract}} \leq 2$ and the contraction $\Lambda \approx 0.27 < 1$
stands. We therefore record a two-line displayed-formula erratum, not a defect in
the route; the genuinely open endpoint is not this constant but the
\emph{continuum limit} (clustering, glue, and OS reconstruction), which we gate open. The
...
The displayed exponent $(r_1/r_0)^{-d}$ is a sign erratum (it should read
$(r_1/r_0)^{+d}$); with that correction the sum is the convergent geometric series
consistent with the asserted bound $\leq 2$. We return to this in
\S\ref{sec:honest}; it is the load-bearing input on which the smallness of $C_1$,
and hence the contraction, rests.
...
\paragraph{The extraction-projection norm: a displayed-formula erratum, not a
defect.}
...
\[
  ...
  \;\leq\; 1 + O\!\big((r_0/r_1)^{\dmax}\big),
\]
asserting the value $\leq 2$ for $r_1/r_0 = 1/2$, $\dmax = 16$. There is a tension
internal to the written expression: taken literally the summand
$(r_1/r_0)^{-d} = 2^{d}$ \emph{grows} in $d$, whereas the asserted closed form
$1 + O\big((r_0/r_1)^{\dmax}\big)$ is a convergent, \emph{decaying} tail. The two
are reconciled by the intended reading, in which the exponent carries the
opposite sign: the Cauchy estimate is the convergent geometric series
$\sum_{d \geq 0} (r_1/r_0)^{d} = 1/(1 - r_1/r_0) = 2$, whose exact tail is the
displayed closed form. On that reading $C_{\mathrm{extract}} \leq 2$,
$C_1 \leq 2224$, and $\Lambda \approx 0.27 < 1$ all stand; the printed
$(r_1/r_0)^{-d}$ is a sign erratum in two displayed lines, not a growth in the
norm. The corresponding Lean module records this as a \emph{retracted erratum}
rather than a break in the contraction. We remain precise that the \emph{sharp}
value of $C_1$ is taken as an input to the checked arithmetic, not independently
derived; but the apparent growth is a transcription typo, and the load-bearing
open question for this route is the continuum endpoint below, not this constant.
...
\begin{itemize}
  ...
  \item \textbf{The extraction norm, settled at the level of the written
    expressions.} The manuscript's literal displayed sum is $\geq 3$ for every
    $n\geq 1$ and never $\leq 2$
    (\texttt{three\_le\_literalExtractionOperatorSum},
    \texttt{not\_literalExtractionOperatorSum\_le\_two}); a
    coefficient-majorant (weighted $\ell^{1}$) norm repairs the argument
    cleanly, with monomial truncation contractive ($C_{\mathrm{extract}}\leq 1$)
    and tail gain $(r_1/r_0)^{\dmax+1}$ (module \texttt{ExtractionMajorant}).
...
\begin{enumerate}
  \item \textbf{One derivation of the recombination constant.}
    \emph{Status: derived in v14, with an internal inconsistency.} The
    manuscript carries two factorizations of $C_1$ (the main text's and
    Computation~O.7's $1.65\times 3b^{4}\times 14\times 2\approx 139\,b^{4}$);
    they do not agree. \emph{What settles it:} fix one derivation. Any supplied
    $C$ is then machine-checkable instantly against the feasibility condition
    $C\,b^{\,3-\dmax}\leq 1/3$; e.g.\ $C\approx 3.3\times 10^{4}$ would simply
    move the required depth to $\dmax=20$. If extraction uses a non-orthogonal
    gauge-invariant operator basis, the honest constant includes a finite
    matrix-conditioning factor $M_A(r,\dmax)$ that must be computed and fed
    back into the depth selection.
  ...
  \item \textbf{KP convergence at the stopping scale and Wilson-measure
    reflection positivity.} \emph{Status: invoked in v14 via the classical
    literature (Koteck\'y--Preiss; Osterwalder--Seiler), correctly.} These are
    established mathematics; machine-checking them is mechanical work with no
    research risk, and does not require anything further from the manuscript.
...
Items (1)--(3) are short clarifications at the manuscript level; item (4) is
mechanical formalization; item (5) is where the proof lives. The Yang--Mills
...
\texttt{ExtractionMajorant} (displayed-sum audit and majorant repair),
