...
\section{Concrete Wilson Extraction and Recombination Audit}

The numerical statements above deliberately treat the recursion constant as an
input.  A later source audit asked the distinct question that controls whether
those statements apply to the manuscript's Wilson map: can the extraction and
recombination factors actually be derived from the objects specified in v14?

The module
\[
  \lean{Mettapedia/QuantumTheory/YangMills/WilsonBlockMajorant.lean}
\]
constructs finite group-valued link fields, local gauge transformations,
plaquette holonomies, conjugation-invariant class potentials, and the normalized
\(SU(N)\) Wilson plaquette potential.  A finite Wilson block assigns positive
canonical dimensions to finitely many plaquette coordinates.  Finitely
supported polynomials in those coordinates evaluate to genuine gauge-invariant
observables.  In the explicit weighted coefficient-majorant norm, Lean proves
\[
  \lean{FiniteWilsonBlock.polynomialMajorantNorm\_truncate\_le},
\]
so canonical-dimension truncation has operator constant at most one on this
realization.

This is an explicit realization of the extraction idea, not a proof that v14's
unspecified action-sector projection is the same operator.  The transfer issue
is decided in
\[
  \lean{Mettapedia/QuantumTheory/YangMills/ExtendedExtractionTransfer.lean}.
\]
Lean constructs an idempotent skew projection onto the requested low-coordinate
range whose coefficient \(\ell^1\) norm exceeds two:
\[
  \lean{projectionRangeSpecification\_does\_not\_imply\_bound\_two}.
\]
Thus specifying only the range of \(P_{\mathrm{ext}}\) cannot imply the advertised
operator bound.  The same module proves the positive finite-matrix theorem
\[
  \lean{coefficientL1\_basisJetProjection\_le}:
\]
once explicit invariant-basis and dual-jet matrices are supplied, their
column-sum bounds multiply.  V14 does not provide those matrices or their
uniform conditioning for the dimension-16 action sector.

The follow-up construction attempt is recorded in
\[
  \lean{Mettapedia/QuantumTheory/YangMills/Dimension16WilsonExtractionCertificate.lean}.
\]
It reaches a precise source obstruction before any basis can be certified.
Appendix F.4 uses equations of motion to identify independent operators,
whereas Appendix O.9 explicitly chooses an off-shell census that retains
EOM-related operators.  The two policies disagree on the displayed
dimension-six divergence-square term, which lies inside the requested
dimension-sixteen sector.  Moreover, the manuscript supplies no lattice
Wilson representatives for the schematic continuum templates, no quotient
basis modulo Bianchi/IBP/trace relations, and no dimension-sixteen dual jets or
biorthogonality matrix.  Appendix I's explicit jets belong to the separate
dimension-four observable projection.  The checked ten-row transfer table
therefore marks only the Wilson/SU(N) object constructed; the relation quotient
conflicts, the coordinate certificate is absent, the displayed conditioning
bound is refuted, and neither \(C_{\mathrm{extract}}\) nor
\(C_{\mathrm{true}}\) is derived.  This is an as-written source
underdetermination, not a theorem that no repaired Wilson basis can exist.

The module
\[
  \lean{Mettapedia/QuantumTheory/YangMills/WilsonPolymerRecombination.lean}
\]
formalizes the regrouping of connected-family contributions by their union
support.  For every nonnegative target weight,
\[
  \lean{polymerActivityMajorant\_recombineByUnion\_le}
\]
gives operator constant one.  The factor \(14\) therefore cannot be a further
loss from this regrouping.  A concrete pair of connected rails in a finite
\(2\times16\) lattice strip has sixteen contacts, refuting a universal
total-contact interpretation.  The exact rooted interpretation is separately
proved: in four dimensions there are two choices of marked endpoint and seven
nonbacktracking directions, hence \(2(2\cdot4-1)=14\).  What remains is the
weighted tree/KP theorem for the preceding map from Wilson activities to
connected-family contributions.

Finally,
\[
  \lean{Mettapedia/QuantumTheory/YangMills/WilsonExtractionRecombinationConstant.lean}
\]
checks the manuscript ledgers and the other proposed inferences.  The displayed
Appendix-F.5 product is \(33152\) at \(b=2\); Appendix O's different product is
\(11088/5\) with extraction factor two.  The bound
\(\exp(O(\beta^{-1}))\leq\exp(1/2)\) does not follow without an explicit uniform
big-O constant, and \(3b^4\) is not a raw connected-tree count.  Substituting
the proved extraction factor one into Appendix O's \emph{other unproved
factors} gives the conditional ledger \(5544/5\).  Under that conditional
ledger the least unrestricted two-source depth is \(15\), while the least even
depth is \(16\).  This is arithmetic under proposed factors, not a derived
Wilson recursion constant.

The resulting status is exact: there is no currently derived
\(C_{\mathrm{true}}\) for the v14 Wilson RG recursion.  A completion must first
select one relation quotient, exact block convention, and faithful finite raw
syntax, with any choice not fixed by v14 labeled as a repair.  It must then
supply the actual dimension-16 lattice Wilson representatives, quotient basis,
and dual jets, prove their uniform conditioning, and establish the fluctuation
and weighted cumulant/tree/KP estimates in the same norm on the same Wilson
family.

\section{Hypercubic Census Decision}

The finite-spacing lattice has hypercubic symmetry, not full \(O(4)\)
symmetry.  The exact \(\mathrm{SU}(2)\) Cartan Wilson chart gives a
canonical-dimension-eight quartic jet proportional to
\(\sum_{\mu<\nu}F_{\mu\nu}^{4}\).  A rational orthogonal rotation changes its
value from \(1\) to \(337/625\).  Therefore the Lorentz-scalar census in
Appendix O.9 cannot span the actual lattice sector, independently of the
on-shell/off-shell, Bianchi, integration-by-parts, trace, or
Cayley--Hamilton relation choice.

The hypercubic replacement succeeds completely on the unpadded
derivative-free Cartan quartic carrier.  The concrete signed-permutation group
has order \(384\).  All \(126\) quartic monomials have checked signed-orbit
certificates; four admissible orbit functions survive, evaluation at their
representatives gives an identity \(4\times4\) conditioning matrix, and every
hypercubic-covariant rational quartic coefficient function reconstructs from
those four coordinates.  This is an exact basis theorem at quartic order, not
a claim about the full noncommutative derivative quotient.

The exact gauge-invariant Wilson fourth jet has census coordinate vector
\((6,0,0,0)\).  The curvature observables invariant under every orthogonal
spacetime matrix form a proper linear subspace of the full observable space,
and the pure-fourth Wilson polynomial gives a nonzero element of the quotient
by that subspace.  Hence the Lorentz-breaking artifact and its Wilson coefficient are
both certified on the same quartic object
(\lean{quarticWilson\_hypercubicArtifact\_packet}).

The first dimension-sixteen carrier is refuted for a separate reason.  Its
fixed-width representation retains inactive field and derivative slots in raw
equality.  Lean constructs two distinct representations with zero active
fields and derivatives but identical active syntax.  If every basis column is
faithful to active syntax, the corresponding basis rows are equal; an exact
right inverse instead requires them to be distinct rows of the identity
matrix.  Thus no invertible exact census on this padded carrier can have every
basis vector padding-invariant
(\lean{faithful\_dimension16\_exactCensus\_uninhabited}).  A valid continuation
must first construct finite unpadded/dependent \(F,D\) syntax or a checked
padding quotient on which the \(H(4)\) action and explicit relation rows
descend.  No quartic conditioning constant transfers automatically to that
new object.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item instantiating \lean{AffineRGStepBound} with the actual polymer norm
    recurrence, including the one-step RG estimates and a newly derived
    \(C_{\mathrm{true}}\); the manuscript's advertised \(2224\) is not presently
    established.
  \item resolving F.4's on-shell versus O.9's off-shell relation policy and
    fixing the exact block quotient and an unpadded/dependent raw \(F,D\)
    syntax before supplying the dimension-16 lattice Wilson representatives,
    quotient basis, dual jets, and uniform matrix-conditioning estimates
    needed to transfer the norm-one Wilson-polynomial realization to a
    repaired extraction operator.
  \item proving the fluctuation, connected-cumulant/tree, stopping-scale KP,
    source-summability, and decaying-remainder estimates on the same actual
    Wilson RG map and in the same norm.
