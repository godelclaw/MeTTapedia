...
E (\lid{--cpu-limit=30}). The runner's parallelism and resource policy are
supplied by the caller. E's binary is built from an E prover source checkout
with the ABGC source files linked into
...
ABGC_LOG_PATH=atps/logs/abgc_telemetry/PHASE.log
  ...
  4
\end{lstlisting}
with the runner-wrapper resource policy supplied by the caller.
