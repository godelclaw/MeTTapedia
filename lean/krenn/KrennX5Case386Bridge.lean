...
private def c1 : Fin 6 → Fin 3 := ![2, 1, 1, 1, 0, 2]
...
theorem selectedEquations_vanish (W : WeightsN 6 3 ℂ)
    ...
    ∀ index : Fin 15,
      eval₂Hom (Int.castRingHom ℂ) (values W)
        (Krenn.X5Case386Selected.selectedEquations index).toPoly = 0 := by
  intro index
  have r0 := hEq c0
  have r1 := hEq c1
  have r2 := hEq c2
...
theorem noNormalizedCase (W : WeightsN 6 3 ℂ)
  ...
  exact (Krenn.X5Case386Selected.selectedHasNoCommonZero (values W))
    (selectedEquations_vanish W hEq hcase)

#print axioms Krenn.X5Case386Bridge.noNormalizedCase

end Krenn.X5Case386Bridge
