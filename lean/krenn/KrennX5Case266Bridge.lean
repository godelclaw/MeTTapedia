...
private def c0 : Fin 6 → Fin 3 := ![0, 1, 0, 2, 0, 0]
private def c1 : Fin 6 → Fin 3 := ![0, 1, 1, 2, 1, 0]
private def c2 : Fin 6 → Fin 3 := ![0, 1, 2, 0, 0, 0]
private def c3 : Fin 6 → Fin 3 := ![0, 1, 2, 1, 1, 0]
...
private def c8 : Fin 6 → Fin 3 := ![1, 1, 1, 1, 0, 1]
private def c9 : Fin 6 → Fin 3 := ![0, 1, 1, 2, 1, 1]
private def c10 : Fin 6 → Fin 3 := ![0, 0, 2, 0, 0, 0]
private def c11 : Fin 6 → Fin 3 := ![0, 1, 2, 1, 1, 1]
...
theorem selectedEquations_vanish (W : WeightsN 6 3 ℂ)
    ...
    ∀ index : Fin 19,
      eval₂Hom (Int.castRingHom ℂ) (values W)
        (Krenn.X5Case266Selected.selectedEquations index).toPoly = 0 := by
  intro index
  have r0 := hEq c0
  have r1 := hEq c1
  have r2 := hEq c2
  ...
  fin_cases index
  · simp [-mul_eq_zero, Krenn.X5Case266Selected.selectedEquations, SparsePoly.toPoly, SparseTerm.toPoly, values, entry, Matrix.cons_val_zero, Matrix.cons_val_one, Matrix.cons_val_fin_one]
...
theorem noNormalizedCase (W : WeightsN 6 3 ℂ)
  ...
  exact (Krenn.X5Case266Selected.selectedHasNoCommonZero (values W))
    (selectedEquations_vanish W hEq hcase)

#print axioms Krenn.X5Case266Bridge.noNormalizedCase

end Krenn.X5Case266Bridge
