...
private def c15 : Fin 6 → Fin 3 := ![2, 2, 0, 0, 0, 1]
private def c16 : Fin 6 → Fin 3 := ![0, 2, 2, 2, 2, 1]
private def c17 : Fin 6 → Fin 3 := ![2, 1, 1, 0, 1, 1]
private def c18 : Fin 6 → Fin 3 := ![2, 2, 2, 2, 1, 2]
private def c19 : Fin 6 → Fin 3 := ![2, 2, 2, 0, 2, 2]
private def c20 : Fin 6 → Fin 3 := ![0, 0, 2, 0, 0, 0]
private def c21 : Fin 6 → Fin 3 := ![0, 1, 2, 1, 1, 1]
private def c22 : Fin 6 → Fin 3 := ![2, 2, 2, 0, 0, 0]
...
private def c27 : Fin 6 → Fin 3 := ![2, 0, 2, 0, 0, 2]
...
theorem selectedEquations_vanish (W : WeightsN 6 3 ℂ)
    ...
    ∀ index : Fin 37,
      eval₂Hom (Int.castRingHom ℂ) (values W)
        (Krenn.X5Case395Selected.selectedEquations index).toPoly = 0 := by
  intro index
  have r0 := hEq c0
  have r1 := hEq c1
  have r2 := hEq c2
  ...
  fin_cases index
  · simp [-mul_eq_zero, Krenn.X5Case395Selected.selectedEquations, SparsePoly.toPoly, SparseTerm.toPoly, values, entry, Matrix.cons_val_zero, Matrix.cons_val_one, Matrix.cons_val_fin_one]
...
theorem noNormalizedCase (W : WeightsN 6 3 ℂ)
  ...
  exact (Krenn.X5Case395Selected.selectedHasNoCommonZero (values W))
    (selectedEquations_vanish W hEq hcase)

#print axioms Krenn.X5Case395Bridge.noNormalizedCase

end Krenn.X5Case395Bridge
