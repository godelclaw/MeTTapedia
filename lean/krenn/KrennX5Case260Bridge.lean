...
private def c0 : Fin 6 → Fin 3 := ![0, 1, 1, 1, 2, 0]
private def c1 : Fin 6 → Fin 3 := ![0, 1, 1, 2, 1, 0]
private def c2 : Fin 6 → Fin 3 := ![0, 1, 2, 1, 1, 0]
...
theorem selectedEquations_vanish (W : WeightsN 6 3 ℂ)
    ...
    ∀ index : Fin 7,
      eval₂Hom (Int.castRingHom ℂ) (values W)
        (Krenn.X5Case260Selected.selectedEquations index).toPoly = 0 := by
  intro index
  have r0 := hEq c0
  have r1 := hEq c1
  have r2 := hEq c2
  ...
  fin_cases index
  · simpa [Krenn.X5Case260Selected.selectedEquations, SparsePoly.toPoly, SparseTerm.toPoly, values, entry] using r0
  · simpa [Krenn.X5Case260Selected.selectedEquations, SparsePoly.toPoly, SparseTerm.toPoly, values, entry] using r1
  · simpa [Krenn.X5Case260Selected.selectedEquations, SparsePoly.toPoly, SparseTerm.toPoly, values, entry] using r2
  · simpa [Krenn.X5Case260Selected.selectedEquations, SparsePoly.toPoly, SparseTerm.toPoly, values, entry] using r3
  · simpa [Krenn.X5Case260Selected.selectedEquations, SparsePoly.toPoly, SparseTerm.toPoly, values, entry] using r4
  · simpa [Krenn.X5Case260Selected.selectedEquations, SparsePoly.toPoly, SparseTerm.toPoly, values, entry] using r5
  · simp [Krenn.X5Case260Selected.selectedEquations, SparsePoly.toPoly, SparseTerm.toPoly, values, entry]
    linear_combination r6

theorem noNormalizedCase (W : WeightsN 6 3 ℂ)
    (hEq : EqSystemN 6 3 W) (hcase : NormalizedCase W) : False := by
  exact (Krenn.X5Case260Selected.selectedHasNoCommonZero (values W))
    (selectedEquations_vanish W hEq hcase)

#print axioms Krenn.X5Case260Bridge.noNormalizedCase

end Krenn.X5Case260Bridge
