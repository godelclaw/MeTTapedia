...
private def c0 : Fin 6 → Fin 3 := ![1, 0, 1, 1, 2, 1]
private def c1 : Fin 6 → Fin 3 := ![1, 0, 1, 2, 1, 1]
private def c2 : Fin 6 → Fin 3 := ![1, 0, 2, 1, 1, 1]
private def c3 : Fin 6 → Fin 3 := ![1, 1, 1, 1, 2, 1]
private def c4 : Fin 6 → Fin 3 := ![1, 1, 1, 2, 1, 1]
private def c5 : Fin 6 → Fin 3 := ![1, 1, 2, 1, 1, 1]
...
theorem selectedEquations_vanish (W : WeightsN 6 3 ℂ)
    ...
    ∀ index : Fin 7,
      eval₂Hom (Int.castRingHom ℂ) (values W)
        (Krenn.X5Case774Selected.selectedEquations index).toPoly = 0 := by
  intro index
  have r0 := hEq c0
  have r1 := hEq c1
  have r2 := hEq c2
  ...
  simp [-mul_eq_zero, pm15, w, c0, c1, c2, c3, c4, c5, c6, allEqual, allEqualList, vertices, hcase.fixed_01_01, hcase.fixed_02_12, hcase.fixed_03_12, hcase.fixed_04_12, hcase.fixed_15_01, hcase.outside05, hcase.outside12, hcase.outside13, hcase.outside14, hcase.outside23, hcase.outside24, hcase.outside25, hcase.outside34, hcase.outside35, hcase.outside45] at r0 r1 r2 r3 r4 r5 r6
  fin_cases index
  · simp [-mul_eq_zero, Krenn.X5Case774Selected.selectedEquations, SparsePoly.toPoly, SparseTerm.toPoly, values, entry, Matrix.cons_val_zero, Matrix.cons_val_one, Matrix.cons_val_fin_one]
...
theorem noNormalizedCase (W : WeightsN 6 3 ℂ)
  ...
  exact (Krenn.X5Case774Selected.selectedHasNoCommonZero (values W))
    (selectedEquations_vanish W hEq hcase)

#print axioms Krenn.X5Case774Bridge.noNormalizedCase

end Krenn.X5Case774Bridge
