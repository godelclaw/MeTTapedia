...
private def c16 : Fin 6 → Fin 3 := ![0, 0, 2, 0, 0, 0]
private def c17 : Fin 6 → Fin 3 := ![0, 1, 2, 1, 1, 1]
...
theorem selectedEquations_vanish (W : WeightsN 6 3 ℂ)
    ...
    ∀ index : Fin 24,
      eval₂Hom (Int.castRingHom ℂ) (values W)
        (Krenn.X5Case355Selected.selectedEquations index).toPoly = 0 := by
  intro index
  have r0 := hEq c0
  have r1 := hEq c1
  have r2 := hEq c2
...
theorem noNormalizedCase (W : WeightsN 6 3 ℂ)
  ...
  exact (Krenn.X5Case355Selected.selectedHasNoCommonZero (values W))
    (selectedEquations_vanish W hEq hcase)

#print axioms Krenn.X5Case355Bridge.noNormalizedCase

end Krenn.X5Case355Bridge
