...
private def c2 : Fin 6 → Fin 3 := ![2, 1, 2, 2, 2, 0]
private def c3 : Fin 6 → Fin 3 := ![1, 1, 2, 2, 2, 0]
private def c4 : Fin 6 → Fin 3 := ![2, 2, 2, 2, 1, 2]
private def c5 : Fin 6 → Fin 3 := ![2, 2, 2, 1, 2, 2]
private def c6 : Fin 6 → Fin 3 := ![1, 2, 2, 2, 2, 2]
private def c7 : Fin 6 → Fin 3 := ![2, 2, 2, 2, 2, 2]
...
theorem selectedEquations_vanish (W : WeightsN 6 3 ℂ)
    ...
    ∀ index : Fin 8,
      eval₂Hom (Int.castRingHom ℂ) (values W)
        (Krenn.X5Case860Selected.selectedEquations index).toPoly = 0 := by
  intro index
  have r0 := hEq c0
  have r1 := hEq c1
  have r2 := hEq c2
  ...
  have r6 := hEq c6
  have r7 := hEq c7
  rw [official_pmSum_eq_pm15] at r0 r1 r2 r3 r4 r5 r6 r7
  simp [-mul_eq_zero, pm15, w, c0, c1, c2, c3, c4, c5, c6, c7, allEqual, allEqualList, vertices, hcase.fixed_01_01, hcase.fixed_02_12, hcase.fixed_03_21, hcase.fixed_04_21, hcase.fixed_15_10, hcase.outside05, hcase.outside12, hcase.outside13, hcase.outside14, hcase.outside23, hcase.outside24, hcase.outside25, hcase.outside34, hcase.outside35, hcase.outside45] at r0 r1 r2 r3 r4 r5 r6 r7
  fin_cases index
  · simp [-mul_eq_zero, Krenn.X5Case860Selected.selectedEquations, SparsePoly.toPoly, SparseTerm.toPoly, values, entry, Matrix.cons_val_zero, Matrix.cons_val_one, Matrix.cons_val_fin_one]
...
theorem noNormalizedCase (W : WeightsN 6 3 ℂ)
  ...
  exact (Krenn.X5Case860Selected.selectedHasNoCommonZero (values W))
    (selectedEquations_vanish W hEq hcase)

#print axioms Krenn.X5Case860Bridge.noNormalizedCase

end Krenn.X5Case860Bridge
