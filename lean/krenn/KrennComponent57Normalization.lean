import KrennCollapseComponentContainmentCore
import KrennCollapseReductionReflection

/-!
Exact normalization of external minimal-prime component 57 into
zero-based Lean component row 56.  All exponent-map equalities are
rechecked by Lean below; this source is generated only for serialization.
-/

namespace Krenn.Component57Normalization

open Krenn.LatticeCombinationReflection
open Krenn.CollapseReductionReflection
open Krenn.CollapseComponentContainment
open Krenn.CollapseComponentContainmentData

noncomputable def rawPositive : Fin 26 → Fin 30 →₀ Nat := ![
  Finsupp.single 8 1 + Finsupp.single 24 1,
  Finsupp.single 6 1 + Finsupp.single 22 1,
  Finsupp.single 3 1 + Finsupp.single 20 1 + Finsupp.single 24 1 + Finsupp.single 27 1,
  Finsupp.single 3 1 + Finsupp.single 20 1 + Finsupp.single 21 1 + Finsupp.single 27 1,
  Finsupp.single 3 1 + Finsupp.single 6 1 + Finsupp.single 20 1 + Finsupp.single 27 1,
  Finsupp.single 2 1 + Finsupp.single 20 1,
  Finsupp.single 2 1 + Finsupp.single 9 1 + Finsupp.single 10 1 + Finsupp.single 22 1 + Finsupp.single 26 1,
  Finsupp.single 2 1 + Finsupp.single 9 1 + Finsupp.single 10 1 + Finsupp.single 21 1 + Finsupp.single 26 1,
  Finsupp.single 2 1 + Finsupp.single 8 1 + Finsupp.single 26 1,
  Finsupp.single 1 1 + Finsupp.single 27 1,
  Finsupp.single 1 1 + Finsupp.single 9 1 + Finsupp.single 10 1 + Finsupp.single 22 1,
  Finsupp.single 1 1 + Finsupp.single 9 1 + Finsupp.single 10 1 + Finsupp.single 21 1,
  Finsupp.single 1 1 + Finsupp.single 8 1,
  Finsupp.single 29 1,
  Finsupp.single 28 1,
  Finsupp.single 23 1,
  Finsupp.single 19 1,
  Finsupp.single 18 1,
  Finsupp.single 17 1,
  Finsupp.single 15 1,
  Finsupp.single 14 1,
  Finsupp.single 13 1,
  Finsupp.single 12 1,
  Finsupp.single 5 1,
  Finsupp.single 4 1,
  Finsupp.single 0 1
]

noncomputable def rawNegative : Fin 26 → Option (Fin 30 →₀ Nat) := ![
  some (Finsupp.single 9 1 + Finsupp.single 10 1 + Finsupp.single 21 1),
  some (Finsupp.single 7 1 + Finsupp.single 21 1),
  some (Finsupp.single 9 1 + Finsupp.single 10 1 + Finsupp.single 22 1 + Finsupp.single 25 1 + Finsupp.single 26 1),
  some (Finsupp.single 8 1 + Finsupp.single 22 1 + Finsupp.single 25 1 + Finsupp.single 26 1),
  some (Finsupp.single 7 1 + Finsupp.single 8 1 + Finsupp.single 25 1 + Finsupp.single 26 1),
  some (Finsupp.single 7 1 + Finsupp.single 25 1),
  some (Finsupp.single 3 1 + Finsupp.single 7 1 + Finsupp.single 24 1 + Finsupp.single 27 1),
  some (Finsupp.single 3 1 + Finsupp.single 6 1 + Finsupp.single 24 1 + Finsupp.single 27 1),
  some (Finsupp.single 3 1 + Finsupp.single 6 1 + Finsupp.single 27 1),
  some (Finsupp.single 2 1 + Finsupp.single 26 1),
  some (Finsupp.single 3 1 + Finsupp.single 7 1 + Finsupp.single 24 1),
  some (Finsupp.single 3 1 + Finsupp.single 6 1 + Finsupp.single 24 1),
  some (Finsupp.single 3 1 + Finsupp.single 6 1),
  none,
  some (Finsupp.single 15 1 + Finsupp.single 26 1),
  some (Finsupp.single 10 1 + Finsupp.single 21 1),
  none,
  none,
  some (Finsupp.single 16 1 + Finsupp.single 29 1),
  none,
  some (Finsupp.single 19 1),
  none,
  some (Finsupp.single 11 1 + Finsupp.single 24 1),
  some (Finsupp.single 20 1),
  some (Finsupp.single 9 1),
  some (Finsupp.single 25 1)
]

def normalizedIndex : Fin 26 → Option (Fin 78) := ![
  some 24,
  some 18,
  some 63,
  some 64,
  some 68,
  some 14,
  some 73,
  some 53,
  some 47,
  some 48,
  some 25,
  some 26,
  some 20,
  some 0,
  some 1,
  some 5,
  some 15,
  some 77,
  some 8,
  some 28,
  none,
  some 9,
  some 10,
  none,
  none,
  none
]

def selectedIndices : List (Fin 78) := [
  24, 18, 63, 64, 68, 14, 73, 53, 47, 48, 25, 26, 20, 0, 1, 5, 15, 77, 8, 28, 9, 10
]

set_option maxRecDepth 100000 in
theorem selectedIndices_exact : componentGenerators 56 = selectedIndices := by decide

noncomputable def rawGeneratorEval {R : Type*} [CommRing R]
    (values : Fin 30 → R) (index : Fin 26) : R :=
  monomial values (rawPositive index) -
    match rawNegative index with
    | none => 0
    | some negative => monomial values negative

set_option maxRecDepth 100000 in
theorem rawPositiveMap00 :
    Finsupp.mapDomain rawToReduced (rawPositive 0) = generatorPositive 24 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap00 :
    Finsupp.mapDomain rawToReduced ((rawNegative 0).getD 0) =
      (generatorNegative 24).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap01 :
    Finsupp.mapDomain rawToReduced (rawPositive 1) = generatorPositive 18 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap01 :
    Finsupp.mapDomain rawToReduced ((rawNegative 1).getD 0) =
      (generatorNegative 18).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap02 :
    Finsupp.mapDomain rawToReduced (rawPositive 2) = generatorPositive 63 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap02 :
    Finsupp.mapDomain rawToReduced ((rawNegative 2).getD 0) =
      (generatorNegative 63).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap03 :
    Finsupp.mapDomain rawToReduced (rawPositive 3) = generatorPositive 64 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap03 :
    Finsupp.mapDomain rawToReduced ((rawNegative 3).getD 0) =
      (generatorNegative 64).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap04 :
    Finsupp.mapDomain rawToReduced (rawPositive 4) = generatorPositive 68 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap04 :
    Finsupp.mapDomain rawToReduced ((rawNegative 4).getD 0) =
      (generatorNegative 68).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap05 :
    Finsupp.mapDomain rawToReduced (rawPositive 5) = generatorPositive 14 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap05 :
    Finsupp.mapDomain rawToReduced ((rawNegative 5).getD 0) =
      (generatorNegative 14).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap06 :
    Finsupp.mapDomain rawToReduced (rawPositive 6) = generatorPositive 73 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap06 :
    Finsupp.mapDomain rawToReduced ((rawNegative 6).getD 0) =
      (generatorNegative 73).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap07 :
    Finsupp.mapDomain rawToReduced (rawPositive 7) = generatorPositive 53 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap07 :
    Finsupp.mapDomain rawToReduced ((rawNegative 7).getD 0) =
      (generatorNegative 53).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap08 :
    Finsupp.mapDomain rawToReduced (rawPositive 8) = generatorPositive 47 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap08 :
    Finsupp.mapDomain rawToReduced ((rawNegative 8).getD 0) =
      (generatorNegative 47).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap09 :
    Finsupp.mapDomain rawToReduced (rawPositive 9) = generatorPositive 48 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap09 :
    Finsupp.mapDomain rawToReduced ((rawNegative 9).getD 0) =
      (generatorNegative 48).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap10 :
    Finsupp.mapDomain rawToReduced (rawPositive 10) = generatorPositive 25 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap10 :
    Finsupp.mapDomain rawToReduced ((rawNegative 10).getD 0) =
      (generatorNegative 25).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap11 :
    Finsupp.mapDomain rawToReduced (rawPositive 11) = generatorPositive 26 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap11 :
    Finsupp.mapDomain rawToReduced ((rawNegative 11).getD 0) =
      (generatorNegative 26).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap12 :
    Finsupp.mapDomain rawToReduced (rawPositive 12) = generatorPositive 20 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap12 :
    Finsupp.mapDomain rawToReduced ((rawNegative 12).getD 0) =
      (generatorNegative 20).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap13 :
    Finsupp.mapDomain rawToReduced (rawPositive 13) = generatorPositive 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap14 :
    Finsupp.mapDomain rawToReduced (rawPositive 14) = generatorPositive 1 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap14 :
    Finsupp.mapDomain rawToReduced ((rawNegative 14).getD 0) =
      (generatorNegative 1).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap15 :
    Finsupp.mapDomain rawToReduced (rawPositive 15) = generatorPositive 5 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap15 :
    Finsupp.mapDomain rawToReduced ((rawNegative 15).getD 0) =
      (generatorNegative 5).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap16 :
    Finsupp.mapDomain rawToReduced (rawPositive 16) = generatorPositive 15 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap17 :
    Finsupp.mapDomain rawToReduced (rawPositive 17) = generatorPositive 77 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap18 :
    Finsupp.mapDomain rawToReduced (rawPositive 18) = generatorPositive 8 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap18 :
    Finsupp.mapDomain rawToReduced ((rawNegative 18).getD 0) =
      (generatorNegative 8).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap19 :
    Finsupp.mapDomain rawToReduced (rawPositive 19) = generatorPositive 28 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawZeroMap20 :
    Finsupp.mapDomain rawToReduced (rawPositive 20) =
      Finsupp.mapDomain rawToReduced ((rawNegative 20).getD 0) := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap21 :
    Finsupp.mapDomain rawToReduced (rawPositive 21) = generatorPositive 9 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawPositiveMap22 :
    Finsupp.mapDomain rawToReduced (rawPositive 22) = generatorPositive 10 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawNegativeMap22 :
    Finsupp.mapDomain rawToReduced ((rawNegative 22).getD 0) =
      (generatorNegative 10).getD 0 := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawZeroMap23 :
    Finsupp.mapDomain rawToReduced (rawPositive 23) =
      Finsupp.mapDomain rawToReduced ((rawNegative 23).getD 0) := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawZeroMap24 :
    Finsupp.mapDomain rawToReduced (rawPositive 24) =
      Finsupp.mapDomain rawToReduced ((rawNegative 24).getD 0) := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawZeroMap25 :
    Finsupp.mapDomain rawToReduced (rawPositive 25) =
      Finsupp.mapDomain rawToReduced ((rawNegative 25).getD 0) := by
  ext coordinate
  fin_cases coordinate <;>
    simp [rawPositive, rawNegative, rawToReduced, Finsupp.mapDomain_add, Finsupp.mapDomain_single, generatorPositive, generatorPositiveSupport, generatorNegative, generatorNegativeSupport, squarefreeExponent, Nat.testBit, Nat.shiftRight_eq_div_pow]

set_option maxRecDepth 100000 in
theorem rawGeneratorNormalizes {R : Type*} [CommRing R]
    (values : Fin 26 → R) (index : Fin 26) :
    rawGeneratorEval (liftReduced values) index =
      match normalizedIndex index with
      | none => 0
      | some generator => generatorEval values generator := by
  fin_cases index
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 0) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 0).getD 0) =
        generatorEval values 24
    rw [monomial_mapDomain rawToReduced values (rawPositive 0),
      monomial_mapDomain rawToReduced values ((rawNegative 0).getD 0), rawPositiveMap00,
      rawNegativeMap00]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 1) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 1).getD 0) =
        generatorEval values 18
    rw [monomial_mapDomain rawToReduced values (rawPositive 1),
      monomial_mapDomain rawToReduced values ((rawNegative 1).getD 0), rawPositiveMap01,
      rawNegativeMap01]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 2) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 2).getD 0) =
        generatorEval values 63
    rw [monomial_mapDomain rawToReduced values (rawPositive 2),
      monomial_mapDomain rawToReduced values ((rawNegative 2).getD 0), rawPositiveMap02,
      rawNegativeMap02]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 3) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 3).getD 0) =
        generatorEval values 64
    rw [monomial_mapDomain rawToReduced values (rawPositive 3),
      monomial_mapDomain rawToReduced values ((rawNegative 3).getD 0), rawPositiveMap03,
      rawNegativeMap03]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 4) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 4).getD 0) =
        generatorEval values 68
    rw [monomial_mapDomain rawToReduced values (rawPositive 4),
      monomial_mapDomain rawToReduced values ((rawNegative 4).getD 0), rawPositiveMap04,
      rawNegativeMap04]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 5) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 5).getD 0) =
        generatorEval values 14
    rw [monomial_mapDomain rawToReduced values (rawPositive 5),
      monomial_mapDomain rawToReduced values ((rawNegative 5).getD 0), rawPositiveMap05,
      rawNegativeMap05]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 6) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 6).getD 0) =
        generatorEval values 73
    rw [monomial_mapDomain rawToReduced values (rawPositive 6),
      monomial_mapDomain rawToReduced values ((rawNegative 6).getD 0), rawPositiveMap06,
      rawNegativeMap06]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 7) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 7).getD 0) =
        generatorEval values 53
    rw [monomial_mapDomain rawToReduced values (rawPositive 7),
      monomial_mapDomain rawToReduced values ((rawNegative 7).getD 0), rawPositiveMap07,
      rawNegativeMap07]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 8) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 8).getD 0) =
        generatorEval values 47
    rw [monomial_mapDomain rawToReduced values (rawPositive 8),
      monomial_mapDomain rawToReduced values ((rawNegative 8).getD 0), rawPositiveMap08,
      rawNegativeMap08]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 9) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 9).getD 0) =
        generatorEval values 48
    rw [monomial_mapDomain rawToReduced values (rawPositive 9),
      monomial_mapDomain rawToReduced values ((rawNegative 9).getD 0), rawPositiveMap09,
      rawNegativeMap09]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 10) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 10).getD 0) =
        generatorEval values 25
    rw [monomial_mapDomain rawToReduced values (rawPositive 10),
      monomial_mapDomain rawToReduced values ((rawNegative 10).getD 0), rawPositiveMap10,
      rawNegativeMap10]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 11) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 11).getD 0) =
        generatorEval values 26
    rw [monomial_mapDomain rawToReduced values (rawPositive 11),
      monomial_mapDomain rawToReduced values ((rawNegative 11).getD 0), rawPositiveMap11,
      rawNegativeMap11]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 12) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 12).getD 0) =
        generatorEval values 20
    rw [monomial_mapDomain rawToReduced values (rawPositive 12),
      monomial_mapDomain rawToReduced values ((rawNegative 12).getD 0), rawPositiveMap12,
      rawNegativeMap12]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 13) - 0 =
        generatorEval values 0
    rw [monomial_mapDomain rawToReduced values (rawPositive 13), rawPositiveMap13]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 14) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 14).getD 0) =
        generatorEval values 1
    rw [monomial_mapDomain rawToReduced values (rawPositive 14),
      monomial_mapDomain rawToReduced values ((rawNegative 14).getD 0), rawPositiveMap14,
      rawNegativeMap14]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 15) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 15).getD 0) =
        generatorEval values 5
    rw [monomial_mapDomain rawToReduced values (rawPositive 15),
      monomial_mapDomain rawToReduced values ((rawNegative 15).getD 0), rawPositiveMap15,
      rawNegativeMap15]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 16) - 0 =
        generatorEval values 15
    rw [monomial_mapDomain rawToReduced values (rawPositive 16), rawPositiveMap16]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 17) - 0 =
        generatorEval values 77
    rw [monomial_mapDomain rawToReduced values (rawPositive 17), rawPositiveMap17]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 18) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 18).getD 0) =
        generatorEval values 8
    rw [monomial_mapDomain rawToReduced values (rawPositive 18),
      monomial_mapDomain rawToReduced values ((rawNegative 18).getD 0), rawPositiveMap18,
      rawNegativeMap18]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 19) - 0 =
        generatorEval values 28
    rw [monomial_mapDomain rawToReduced values (rawPositive 19), rawPositiveMap19]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 20) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 20).getD 0) = 0
    rw [monomial_mapDomain rawToReduced values (rawPositive 20),
      monomial_mapDomain rawToReduced values ((rawNegative 20).getD 0), rawZeroMap20]
    ring
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 21) - 0 =
        generatorEval values 9
    rw [monomial_mapDomain rawToReduced values (rawPositive 21), rawPositiveMap21]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 22) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 22).getD 0) =
        generatorEval values 10
    rw [monomial_mapDomain rawToReduced values (rawPositive 22),
      monomial_mapDomain rawToReduced values ((rawNegative 22).getD 0), rawPositiveMap22,
      rawNegativeMap22]
    simp [generatorEval, generatorNegative, generatorNegativeSupport]
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 23) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 23).getD 0) = 0
    rw [monomial_mapDomain rawToReduced values (rawPositive 23),
      monomial_mapDomain rawToReduced values ((rawNegative 23).getD 0), rawZeroMap23]
    ring
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 24) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 24).getD 0) = 0
    rw [monomial_mapDomain rawToReduced values (rawPositive 24),
      monomial_mapDomain rawToReduced values ((rawNegative 24).getD 0), rawZeroMap24]
    ring
  · change monomial (fun coordinate => values (rawToReduced coordinate)) (rawPositive 25) -
      monomial (fun coordinate => values (rawToReduced coordinate)) ((rawNegative 25).getD 0) = 0
    rw [monomial_mapDomain rawToReduced values (rawPositive 25),
      monomial_mapDomain rawToReduced values ((rawNegative 25).getD 0), rawZeroMap25]
    ring

set_option maxRecDepth 100000 in
theorem rawComponent57_vanishes {R : Type*} [CommRing R]
    (values : Fin 30 → R) (collapse : SatisfiesRawCollapse values)
    (selectedVanish : ∀ generator ∈ componentGenerators 56,
      generatorEval (reduceRaw values) generator = 0) :
    ∀ index, rawGeneratorEval values index = 0 := by
  have identified : values = liftReduced (reduceRaw values) :=
    raw_eq_liftReduced_reduceRaw values
      (raw_binomial_equalities_of_collapse collapse)
  rw [identified]
  intro index
  rw [rawGeneratorNormalizes]
  fin_cases index
  · exact selectedVanish 24 (by decide)
  · exact selectedVanish 18 (by decide)
  · exact selectedVanish 63 (by decide)
  · exact selectedVanish 64 (by decide)
  · exact selectedVanish 68 (by decide)
  · exact selectedVanish 14 (by decide)
  · exact selectedVanish 73 (by decide)
  · exact selectedVanish 53 (by decide)
  · exact selectedVanish 47 (by decide)
  · exact selectedVanish 48 (by decide)
  · exact selectedVanish 25 (by decide)
  · exact selectedVanish 26 (by decide)
  · exact selectedVanish 20 (by decide)
  · exact selectedVanish 0 (by decide)
  · exact selectedVanish 1 (by decide)
  · exact selectedVanish 5 (by decide)
  · exact selectedVanish 15 (by decide)
  · exact selectedVanish 77 (by decide)
  · exact selectedVanish 8 (by decide)
  · exact selectedVanish 28 (by decide)
  · simp [normalizedIndex]
  · exact selectedVanish 9 (by decide)
  · exact selectedVanish 10 (by decide)
  · simp [normalizedIndex]
  · simp [normalizedIndex]
  · simp [normalizedIndex]

end Krenn.Component57Normalization

#print axioms Krenn.Component57Normalization.rawComponent57_vanishes
