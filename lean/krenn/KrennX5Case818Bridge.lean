...
private def c0 : Fin 6 → Fin 3 := ![1, 1, 2, 1, 1, 0]
private def c1 : Fin 6 → Fin 3 := ![2, 1, 1, 1, 0, 2]
private def c2 : Fin 6 → Fin 3 := ![2, 1, 2, 2, 0, 2]
private def c3 : Fin 6 → Fin 3 := ![2, 1, 1, 0, 1, 2]
private def c4 : Fin 6 → Fin 3 := ![2, 1, 2, 0, 2, 2]
private def c5 : Fin 6 → Fin 3 := ![1, 1, 2, 2, 2, 0]
...
private def c10 : Fin 6 → Fin 3 := ![1, 1, 2, 1, 1, 1]
private def c11 : Fin 6 → Fin 3 := ![0, 1, 2, 2, 2, 2]
private def c12 : Fin 6 → Fin 3 := ![2, 1, 1, 1, 1, 2]
...
theorem selectedEquations_vanish (W : WeightsN 6 3 ℂ)
    ...
    ∀ index : Fin 17,
      eval₂Hom (Int.castRingHom ℂ) (values W)
        (Krenn.X5Case818Selected.selectedEquations index).toPoly = 0 := by
  intro index
  have r0 := hEq c0
  have r1 := hEq c1
  have r2 := hEq c2
...
theorem noNormalizedCase (W : WeightsN 6 3 ℂ)
  ...
  exact (Krenn.X5Case818Selected.selectedHasNoCommonZero (values W))
    (selectedEquations_vanish W hEq hcase)

#print axioms Krenn.X5Case818Bridge.noNormalizedCase

end Krenn.X5Case818Bridge
