import KrennComponent55RootBridge

namespace Krenn.Component55RootCommonZero.Chunk6
...
theorem rootBlock_of_base_and_component {R : Type*} [CommRing R]
    ...
    ∀ offset : Fin 40, eval₂Hom (Int.castRingHom R) (rootValues full)
      (Krenn.Component55.Tree.rootEquations ⟨240 + offset.val, by omega⟩).toPoly = 0 := by
  have collapse := raw_collapse_of_base_common_zero full base
  have rawZero : ∀ index, rawGeneratorEval (rawValues full) index = 0 :=
    rawComponent55_vanishes (rawValues full) collapse selectedVanish
  have identified := full_eq_reconstruct_of_raw_component full rawZero
  intro offset
  fin_cases offset
  · have source := base 626
    rw [identified] at source
    simp [Krenn.Component55.Tree.rootEquations, Krenn.Component55.Root.rootEquations, Krenn.FrozenCase0System.equations, SparsePoly.toPoly, SparseTerm.toPoly, rootValues, reconstructFull, rootCoordinate, sub_eq_add_neg] at source ⊢
...
end Krenn.Component55RootCommonZero.Chunk6
