...
/-! ### Three-matching skeletons over an arbitrary vertex set

A hypothetical three-colour solution supplies one perfect matching per colour.
Their union is the object every uniform argument must reason about: an
edge-coloured multigraph in which every vertex has degree exactly three, counted
with multiplicity, however many vertices there are.
-/
