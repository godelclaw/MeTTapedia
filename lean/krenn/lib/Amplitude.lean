...
/-! ### The Krenn–Gu conjecture for an arbitrary number of vertices

Everything below quantifies over the vertex count.  `Fin 6` appears nowhere: the
six-vertex case enters only as the base of an induction, and the conjecture is
reduced to a single descent hypothesis about arbitrary even vertex sets.
-/
