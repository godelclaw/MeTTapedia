...
/-! Positive-stabilizer certificate for dimension-sixteen representative 2. -/
...
theorem octic_representative_2_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 2).act h = octicRepresentative 2 →
        (octicRepresentative 2).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 2).tensorSign h =
        (octicRepresentative 2).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 2).act
          (unsignedHypercubic permutation) = octicRepresentative 2 →
        (octicRepresentative 2).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 2)).symm.trans hstable
