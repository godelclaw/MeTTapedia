...
/-! Positive-stabilizer certificate for dimension-sixteen representative 16. -/
...
theorem octic_representative_16_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 16).act h = octicRepresentative 16 →
        (octicRepresentative 16).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 16).tensorSign h =
        (octicRepresentative 16).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 16).act
          (unsignedHypercubic permutation) = octicRepresentative 16 →
        (octicRepresentative 16).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 16)).symm.trans hstable
