...
/-! Positive-stabilizer certificate for dimension-sixteen representative 9. -/
...
theorem octic_representative_9_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 9).act h = octicRepresentative 9 →
        (octicRepresentative 9).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 9).tensorSign h =
        (octicRepresentative 9).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 9).act
          (unsignedHypercubic permutation) = octicRepresentative 9 →
        (octicRepresentative 9).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 9)).symm.trans hstable
