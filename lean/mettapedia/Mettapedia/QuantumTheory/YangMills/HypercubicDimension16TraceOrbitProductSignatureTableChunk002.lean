...
namespace YangMills
namespace HypercubicDimension16TraceOrbitProductSignatureTableChunk002
...
def chunkIndex (localIndex : Fin 1024) : PositiveRepresentativeIndex :=
  ⟨2048 + localIndex.1, by
    change 2048 + localIndex.1 < 11654
    omega⟩
...
theorem chunk_sorted_lt :
    ∀ localIndex : Fin 1024,
      sortedProductSignatureAt (2048 + localIndex.1) <
        sortedProductSignatureAt (2048 + localIndex.1 + 1) := by
  decide +kernel

end HypercubicDimension16TraceOrbitProductSignatureTableChunk002
end YangMills
end QuantumTheory
end Mettapedia
