...
/-! Positive-stabilizer certificate for dimension-sixteen representative 0. -/
...
theorem octic_representative_0_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 0).act h = octicRepresentative 0 →
        (octicRepresentative 0).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 0).tensorSign h =
        (octicRepresentative 0).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 0).act
          (unsignedHypercubic permutation) = octicRepresentative 0 →
        (octicRepresentative 0).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 0)).symm.trans hstable
