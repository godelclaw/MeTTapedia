...
/-! Positive-stabilizer certificate for dimension-sixteen representative 12. -/
...
theorem octic_representative_12_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 12).act h = octicRepresentative 12 →
        (octicRepresentative 12).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 12).tensorSign h =
        (octicRepresentative 12).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 12).act
          (unsignedHypercubic permutation) = octicRepresentative 12 →
        (octicRepresentative 12).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 12)).symm.trans hstable
