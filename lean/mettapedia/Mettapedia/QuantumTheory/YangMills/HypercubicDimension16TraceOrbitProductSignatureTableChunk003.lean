...
namespace YangMills
namespace HypercubicDimension16TraceOrbitProductSignatureTableChunk003
...
def chunkIndex (localIndex : Fin 1024) : PositiveRepresentativeIndex :=
  ⟨3072 + localIndex.1, by
    change 3072 + localIndex.1 < 11654
    omega⟩
...
theorem chunk_sorted_lt :
    ∀ localIndex : Fin 1024,
      sortedProductSignatureAt (3072 + localIndex.1) <
        sortedProductSignatureAt (3072 + localIndex.1 + 1) := by
  decide +kernel

end HypercubicDimension16TraceOrbitProductSignatureTableChunk003
end YangMills
end QuantumTheory
end Mettapedia
