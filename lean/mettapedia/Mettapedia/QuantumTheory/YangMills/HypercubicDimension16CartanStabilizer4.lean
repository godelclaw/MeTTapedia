...
/-! Positive-stabilizer certificate for dimension-sixteen representative 4. -/
...
theorem octic_representative_4_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 4).act h = octicRepresentative 4 →
        (octicRepresentative 4).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 4).tensorSign h =
        (octicRepresentative 4).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 4).act
          (unsignedHypercubic permutation) = octicRepresentative 4 →
        (octicRepresentative 4).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 4)).symm.trans hstable
