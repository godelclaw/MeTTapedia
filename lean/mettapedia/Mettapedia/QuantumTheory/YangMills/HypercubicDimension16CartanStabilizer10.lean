...
/-! Positive-stabilizer certificate for dimension-sixteen representative 10. -/
...
theorem octic_representative_10_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 10).act h = octicRepresentative 10 →
        (octicRepresentative 10).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 10).tensorSign h =
        (octicRepresentative 10).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 10).act
          (unsignedHypercubic permutation) = octicRepresentative 10 →
        (octicRepresentative 10).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 10)).symm.trans hstable
