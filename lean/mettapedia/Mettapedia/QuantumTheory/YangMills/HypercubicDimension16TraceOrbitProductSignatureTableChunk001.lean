...
namespace YangMills
namespace HypercubicDimension16TraceOrbitProductSignatureTableChunk001
...
def chunkIndex (localIndex : Fin 1024) : PositiveRepresentativeIndex :=
  ⟨1024 + localIndex.1, by
    change 1024 + localIndex.1 < 11654
    omega⟩
...
theorem chunk_sorted_lt :
    ∀ localIndex : Fin 1024,
      sortedProductSignatureAt (1024 + localIndex.1) <
        sortedProductSignatureAt (1024 + localIndex.1 + 1) := by
  decide +kernel

end HypercubicDimension16TraceOrbitProductSignatureTableChunk001
end YangMills
end QuantumTheory
end Mettapedia
