...
/-! Positive-stabilizer certificate for dimension-sixteen representative 7. -/
...
theorem octic_representative_7_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 7).act h = octicRepresentative 7 →
        (octicRepresentative 7).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 7).tensorSign h =
        (octicRepresentative 7).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 7).act
          (unsignedHypercubic permutation) = octicRepresentative 7 →
        (octicRepresentative 7).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 7)).symm.trans hstable
