...
namespace YangMills
namespace HypercubicDimension16TraceOrbitProductSignatureTableChunk008
...
def chunkIndex (localIndex : Fin 1024) : PositiveRepresentativeIndex :=
  ⟨8192 + localIndex.1, by
    change 8192 + localIndex.1 < 11654
    omega⟩
...
theorem chunk_sorted_lt :
    ∀ localIndex : Fin 1024,
      sortedProductSignatureAt (8192 + localIndex.1) <
        sortedProductSignatureAt (8192 + localIndex.1 + 1) := by
  decide +kernel

end HypercubicDimension16TraceOrbitProductSignatureTableChunk008
end YangMills
end QuantumTheory
end Mettapedia
