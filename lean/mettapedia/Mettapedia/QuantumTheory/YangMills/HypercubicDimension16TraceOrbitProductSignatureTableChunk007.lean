...
namespace YangMills
namespace HypercubicDimension16TraceOrbitProductSignatureTableChunk007
...
def chunkIndex (localIndex : Fin 1024) : PositiveRepresentativeIndex :=
  ⟨7168 + localIndex.1, by
    change 7168 + localIndex.1 < 11654
    omega⟩
...
theorem chunk_sorted_lt :
    ∀ localIndex : Fin 1024,
      sortedProductSignatureAt (7168 + localIndex.1) <
        sortedProductSignatureAt (7168 + localIndex.1 + 1) := by
  decide +kernel

end HypercubicDimension16TraceOrbitProductSignatureTableChunk007
end YangMills
end QuantumTheory
end Mettapedia
