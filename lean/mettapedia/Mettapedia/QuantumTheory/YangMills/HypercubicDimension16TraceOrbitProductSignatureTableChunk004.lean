...
namespace YangMills
namespace HypercubicDimension16TraceOrbitProductSignatureTableChunk004
...
def chunkIndex (localIndex : Fin 1024) : PositiveRepresentativeIndex :=
  ⟨4096 + localIndex.1, by
    change 4096 + localIndex.1 < 11654
    omega⟩
...
theorem chunk_sorted_lt :
    ∀ localIndex : Fin 1024,
      sortedProductSignatureAt (4096 + localIndex.1) <
        sortedProductSignatureAt (4096 + localIndex.1 + 1) := by
  decide +kernel

end HypercubicDimension16TraceOrbitProductSignatureTableChunk004
end YangMills
end QuantumTheory
end Mettapedia
