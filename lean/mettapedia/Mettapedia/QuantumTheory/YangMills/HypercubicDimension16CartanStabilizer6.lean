...
/-! Positive-stabilizer certificate for dimension-sixteen representative 6. -/
...
theorem octic_representative_6_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 6).act h = octicRepresentative 6 →
        (octicRepresentative 6).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 6).tensorSign h =
        (octicRepresentative 6).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 6).act
          (unsignedHypercubic permutation) = octicRepresentative 6 →
        (octicRepresentative 6).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 6)).symm.trans hstable
