...
/-! Positive-stabilizer certificate for dimension-sixteen representative 3. -/
...
theorem octic_representative_3_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 3).act h = octicRepresentative 3 →
        (octicRepresentative 3).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 3).tensorSign h =
        (octicRepresentative 3).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 3).act
          (unsignedHypercubic permutation) = octicRepresentative 3 →
        (octicRepresentative 3).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 3)).symm.trans hstable
