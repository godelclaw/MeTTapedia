...
/-! Positive-stabilizer certificate for dimension-sixteen representative 13. -/
...
theorem octic_representative_13_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 13).act h = octicRepresentative 13 →
        (octicRepresentative 13).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 13).tensorSign h =
        (octicRepresentative 13).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 13).act
          (unsignedHypercubic permutation) = octicRepresentative 13 →
        (octicRepresentative 13).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 13)).symm.trans hstable
