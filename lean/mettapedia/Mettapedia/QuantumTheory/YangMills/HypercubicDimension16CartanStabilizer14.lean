...
/-! Positive-stabilizer certificate for dimension-sixteen representative 14. -/
...
theorem octic_representative_14_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 14).act h = octicRepresentative 14 →
        (octicRepresentative 14).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 14).tensorSign h =
        (octicRepresentative 14).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 14).act
          (unsignedHypercubic permutation) = octicRepresentative 14 →
        (octicRepresentative 14).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 14)).symm.trans hstable
