...
namespace YangMills
namespace HypercubicDimension16TraceOrbitProductSignatureTableChunk006
...
def chunkIndex (localIndex : Fin 1024) : PositiveRepresentativeIndex :=
  ⟨6144 + localIndex.1, by
    change 6144 + localIndex.1 < 11654
    omega⟩
...
theorem chunk_sorted_lt :
    ∀ localIndex : Fin 1024,
      sortedProductSignatureAt (6144 + localIndex.1) <
        sortedProductSignatureAt (6144 + localIndex.1 + 1) := by
  decide +kernel

end HypercubicDimension16TraceOrbitProductSignatureTableChunk006
end YangMills
end QuantumTheory
end Mettapedia
