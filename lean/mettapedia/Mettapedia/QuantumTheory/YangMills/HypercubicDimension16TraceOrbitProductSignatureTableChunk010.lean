...
namespace YangMills
namespace HypercubicDimension16TraceOrbitProductSignatureTableChunk010
...
def chunkIndex (localIndex : Fin 1024) : PositiveRepresentativeIndex :=
  ⟨10240 + localIndex.1, by
    change 10240 + localIndex.1 < 11654
    omega⟩
...
theorem chunk_sorted_lt :
    ∀ localIndex : Fin 1024,
      sortedProductSignatureAt (10240 + localIndex.1) <
        sortedProductSignatureAt (10240 + localIndex.1 + 1) := by
  decide +kernel

end HypercubicDimension16TraceOrbitProductSignatureTableChunk010
end YangMills
end QuantumTheory
end Mettapedia
