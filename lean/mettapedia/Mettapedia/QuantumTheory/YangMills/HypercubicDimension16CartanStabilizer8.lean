...
/-! Positive-stabilizer certificate for dimension-sixteen representative 8. -/
...
theorem octic_representative_8_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 8).act h = octicRepresentative 8 →
        (octicRepresentative 8).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 8).tensorSign h =
        (octicRepresentative 8).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 8).act
          (unsignedHypercubic permutation) = octicRepresentative 8 →
        (octicRepresentative 8).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 8)).symm.trans hstable
