...
def yangMillsJointPhysicalRelationRoadmapEntry : YangMillsRoadmapEntry where
  ...
  itvUpperPercent := 100
  progressPercent := 100
  evidence := "currentYangMillsJointPhysicalRelation_packet gives the lower bound 11558 for the coordinate-free joint invariant relation submodule under either policy. It combines 11556 field-eight trace rows, a seven-to-eight commutator relation, and a field-five separator. OUR finite F,D/IBP cochain bridge is equivariant and has range equal to this whole submodule; common off-shell cochains have the same ambient invariant semantics on shell. OUR WilsonCartanJointMismatch makes the remaining Wilson-coordinate requirement precise: the trace-order-blind raw Cartan restriction of the eighth jet kills the explicit incoming pair whose ordinary trace class is nonzero, and no 17-coordinate chart can inject that 98-dimensional trace quotient. OUR WilsonCartanTraceOrderFaithfulnessNoGo proves the sharper local mismatch: no ordinary-trace-quotient coordinate that agrees with the actual Cartan eighth jet on all raw labelled inputs can detect the certified incoming trace-order class, so no injective realization of that Cartan coordinate exists even without an H(4) requirement. OUR WilsonTraceOrderRepair proves the local cross-sector necessity theorem: the canonical eight-field trace coordinate cannot descend through the coupled commutator quotient, any descended correction has a forced nonzero seven-field value, and the concrete compensator gives a policy-uniform quotient coordinate which retains the isolated trace-order signal. OUR WilsonTraceOrderCoordinateCompletion supplies its exact 98-coordinate completion: synthesis after the packet is the identity on the ordinary trace quotient, and the locally compensated packet kills the full projected physical commutator while retaining its exact nonzero two-pivot eight-field signal. OUR WilsonTraceOrderInvariantRepair constructs the two-coordinate signed-H(4)-invariant extension that repairs the first explicit IBP obstruction. OUR WilsonTraceOrderInvariantThreeCoordinateRepair constructs a distinct split-derivative second IBP row on which that two-coordinate correction is exactly nonzero, with value (1/2) times the incoming trace class. Its third field-relabel-invariant plane/derivative coordinate has calibrated value one on the new row and zero signed weights on the commutator, first-IBP, Bianchi, and EOM rows. Subtracting one half of the new trace contribution kills the new physical and Reynolds-averaged invariant relation member while retaining all the previous physical and invariant checks. This is a finite repair family, not a complete relation rank, quotient dimension, conditioning result, full-relation descent, or noncommutative Wilson-functional coordinate construction."
  nextObligation := "Enumerate and test the remaining Bianchi, EOM, IBP, commutator, and trace relation families against the three-coordinate trace-order repair. Extend its coordinate family only when a kernel-certified mismatch requires a new direction; otherwise prove full-relation descent or certify a decisive finite obstruction, then complete the policy-indexed census, sparse dual conditioning certificate, and analytic Wilson-functional estimates in the same repaired norm."
