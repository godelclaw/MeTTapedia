...
def yangMillsJointPhysicalRelationRoadmapEntry : YangMillsRoadmapEntry where
  ...
  itvUpperPercent := 100
  progressPercent := 100
  evidence := "currentYangMillsJointPhysicalRelation_packet gives the lower bound 11558 for the coordinate-free joint invariant relation submodule under either policy. It combines 11556 field-eight trace rows, a seven-to-eight commutator relation, and a field-five separator. OUR finite F,D/IBP cochain bridge is equivariant and has range equal to this whole submodule; common off-shell cochains have the same ambient invariant semantics on shell. OUR WilsonCartanJointMismatch makes the remaining Wilson-coordinate requirement precise: the trace-order-blind raw Cartan restriction of the eighth jet kills the explicit incoming pair whose ordinary trace class is nonzero, and no 17-coordinate chart can inject that 98-dimensional trace quotient. OUR WilsonTraceOrderRepair then proves the local cross-sector necessity theorem: the canonical eight-field trace coordinate cannot descend through the coupled commutator quotient, any descended correction has a forced nonzero seven-field value, and the concrete compensator gives a policy-uniform quotient coordinate which retains the isolated trace-order signal. It is packaged beside, not identified with, the exact Cartan Wilson coefficient packet. This is not a complete relation rank, quotient dimension, conditioning result, or a full Wilson-functional coordinate construction."
  nextObligation := "Extend the certified local trace-order repair to full noncommutative Wilson-functional coordinates for the identified quotient, then complete its policy-indexed relation census, sparse dual conditioning certificate, and analytic estimates in the same repaired norm."
