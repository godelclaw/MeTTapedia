...
def yangMillsJointPhysicalRelationRoadmapEntry : YangMillsRoadmapEntry where
  ...
  itvUpperPercent := 100
  progressPercent := 100
  evidence := "currentYangMillsJointPhysicalRelation_packet gives the lower bound 11558 for the coordinate-free joint invariant relation submodule under either policy. It combines 11556 field-eight trace rows, a seven-to-eight commutator relation, and a field-five separator. OUR finite F,D/IBP cochain bridge is equivariant and has range equal to this whole submodule; common off-shell cochains have the same ambient invariant semantics on shell. OUR WilsonCartanJointMismatch makes the remaining Wilson-coordinate requirement precise: the trace-order-blind raw Cartan restriction of the eighth jet kills the explicit incoming pair whose ordinary trace class is nonzero, and no 17-coordinate chart can inject that 98-dimensional trace quotient. This is not a complete relation rank, quotient dimension, conditioning result, or a full Wilson-functional coordinate construction."
  nextObligation := "Construct trace-order-aware, cross-sector Wilson-functional coordinates for the identified quotient, then complete its policy-indexed relation census, sparse dual conditioning certificate, and analytic estimates in the same repaired norm."
