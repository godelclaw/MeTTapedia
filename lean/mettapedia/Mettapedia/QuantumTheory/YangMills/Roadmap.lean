...
def yangMillsJointPhysicalRelationRoadmapEntry : YangMillsRoadmapEntry where
  ...
  itvUpperPercent := 100
  progressPercent := 100
  evidence := "currentYangMillsJointPhysicalRelation_packet gives the lower bound 11558 for the coordinate-free joint invariant relation submodule under either policy. OUR finite F,D/IBP cochain bridge realizes that same invariant relation range. The raw Cartan eighth-jet restriction is certified trace-order blind, so the local 98-coordinate trace packet and its signed-H(4)-invariant repair family remain necessary. The three-coordinate packet repaired two explicit IBP rows but fails on a canonical polarized SU(2) trace-anticommutator row with exact value (1/2) times the incoming trace class. OUR WilsonTraceOrderTraceTopologyRepair adds a fourth scalar: the derivative-axis character gated by the relabel-invariant count of trace two-cycles. Exact signed weights 0, 0, and 32 on the source, swap, and split terms normalize this scalar to -1 on the canonical row, and adding one half of its trace contribution annihilates that physical row and its Reynolds member. The canonical six-term fundamental three-cut row then gives the four-coordinate residual (-1/2) times the incoming trace class. OUR fifth scalar gates the same derivative-axis character by one six-cycle; its signed weights 0, 32, 0, 32, 0, 0 normalize to -2, and its coefficient -1/4 kills this residual without changing the displayed trace-anticommutator cancellation. OUR complete three-cut cycle-profile calculation then enumerates all 210 ordered distinct labels on the same seven-cycle. Its four topology equations together with the polarized trace-anticommutator equation force coefficients (6/5, 13/10, -1, -7/10, -3/5) for the two-through-six-cycle gates, and its kernel replay gives zero for every enumerated alternating six-term cycle profile. The resulting signed-H(4) coordinate kills the two displayed canonical physical rows and their Reynolds members. OUR HypercubicDimension16FundamentalTracePhysicalFamily maps every certified label to a distinct actual FundamentalTraceSite and policy-indexed physical fundamental-trace generator, embeds their finite cochains injectively into the F,D/IBP source, and carries them by signed Reynolds semantics into the coordinate-free joint relation submodule; its canonical label recovers the previously displayed row. OUR HypercubicDimension16FundamentalTracePhysicalFieldEight proves that every finite cochain in that actual family has zero exact field-eight invariant projection for either policy, because each trace-rewire summand remains seven-field. This closes the source-side identification and its field-eight sector, but does not prove profile-correction annihilation on every remaining seven-field component, full-relation descent, complete joint rank or conditioning, or noncommutative Wilson-functional coordinate construction."
  nextObligation := "Prove or refute profile-correction annihilation on the remaining seven-field component of every actual certified fundamental-trace generator, then test the remaining canonical join/split and relation families before attempting descent through the coordinate-free relation submodule. The policy-indexed census, sparse dual conditioning certificate, and analytic noncommutative Wilson-functional estimates remain unproved."
