...
def yangMillsJointPhysicalRelationRoadmapEntry : YangMillsRoadmapEntry where
  ...
  itvUpperPercent := 100
  progressPercent := 100
  evidence := "currentYangMillsJointPhysicalRelation_packet gives the lower bound 11558 for the coordinate-free joint invariant relation submodule under either policy. It combines 11556 field-eight trace rows, a seven-to-eight commutator relation, and a field-five separator. OUR finite F,D/IBP cochain bridge is equivariant and has range equal to this whole submodule; common off-shell cochains have the same ambient invariant semantics on shell. OUR WilsonCartanJointMismatch makes the remaining Wilson-coordinate requirement precise: the trace-order-blind raw Cartan restriction of the eighth jet kills the explicit incoming pair whose ordinary trace class is nonzero, and no 17-coordinate chart can inject that 98-dimensional trace quotient. OUR WilsonTraceOrderRepair proves the local cross-sector necessity theorem: the canonical eight-field trace coordinate cannot descend through the coupled commutator quotient, any descended correction has a forced nonzero seven-field value, and the concrete compensator gives a policy-uniform quotient coordinate which retains the isolated trace-order signal. OUR WilsonTraceOrderInvariantRepair tests the first signed-H(4)-invariant extension: its one-coordinate version has a certified nonzero value on an explicit physical IBP row; an added field-relabel-invariant split-derivative/plane coordinate cancels exactly that value without changing the commutator normalization. The resulting two-coordinate correction annihilates the explicit commutator for either policy, the displayed IBP row, and every lifted derivative-free field-eight trace generator. OUR WilsonTraceOrderInvariantBianchi tests a genuine three-distinct-axis innermost Bianchi row: both coordinates have zero signed weights on all three normalized summands, so the correction annihilates the physical row for either policy and its signed-Reynolds invariant relation member. OUR WilsonTraceOrderInvariantEOM tests a genuine on-shell EOM row: all four contracted-axis terms have zero signed weights for both coordinates; exact normalization removes the diagonal term and leaves orientations 1, 1, and -1, so the correction annihilates both the physical row and its signed-Reynolds invariant relation member. This is a finite repair family, not a complete relation rank, quotient dimension, conditioning result, full-relation descent, or noncommutative Wilson-functional coordinate construction."
  nextObligation := "Test the two-coordinate trace-order repair against remaining Bianchi, EOM, IBP, and trace relation families until full-relation descent or a precise next mismatch is certified; then complete its policy-indexed relation census, sparse dual conditioning certificate, and analytic Wilson-functional estimates in the same repaired norm."
