...
/-! Positive-stabilizer certificate for dimension-sixteen representative 5. -/
...
theorem octic_representative_5_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 5).act h = octicRepresentative 5 →
        (octicRepresentative 5).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 5).tensorSign h =
        (octicRepresentative 5).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 5).act
          (unsignedHypercubic permutation) = octicRepresentative 5 →
        (octicRepresentative 5).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 5)).symm.trans hstable
