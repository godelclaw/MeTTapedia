...
/-! Positive-stabilizer certificate for dimension-sixteen representative 11. -/
...
theorem octic_representative_11_stabilizer_positive :
    ∀ h : Hypercubic4,
      (octicRepresentative 11).act h = octicRepresentative 11 →
        (octicRepresentative 11).tensorSign h = 1 := by
  intro h hstable
  have reflectionCancellation :
      (octicRepresentative 11).tensorSign h =
        (octicRepresentative 11).tensorSign
  ...
  have unsignedCheck : ∀ permutation : Equiv.Perm Axis,
      (octicRepresentative 11).act
          (unsignedHypercubic permutation) = octicRepresentative 11 →
        (octicRepresentative 11).tensorSign
          (unsignedHypercubic permutation) = 1 := by
    set_option maxRecDepth 100000 in decide
  apply unsignedCheck h.perm
  exact (OcticMonomial.act_eq_unsigned h
    (octicRepresentative 11)).symm.trans hstable
