...
/-- Upstream canonical deduction confidence from `hyperon/PLN/lib_pln.metta`. -/
noncomputable def truthDeductionConf (pq qr : TV) : ℝ :=
  pq.s * qr.s * pq.c * qr.c

/-- Upstream canonical induction confidence from `hyperon/PLN/lib_pln.metta`. -/
noncomputable def truthInductionConf (ba bc : TV) : ℝ :=
  Mettapedia.PLN.TruthValues.PeTTaLibPLNTruthFunctions.w2c (bc.s * bc.c * ba.c)

/-- Upstream canonical abduction confidence from `hyperon/PLN/lib_pln.metta`. -/
noncomputable def truthAbductionConf (ab cb : TV) : ℝ :=
  Mettapedia.PLN.TruthValues.PeTTaLibPLNTruthFunctions.w2c (ab.s * ab.c * cb.c)

/-- Upstream canonical modus-ponens confidence from `hyperon/PLN/lib_pln.metta`. -/
noncomputable def truthModusPonensConf (p pq : TV) : ℝ :=
  p.s * pq.s * p.c * pq.c

/-- Upstream canonical disjunction helper from `hyperon/PLN/lib_pln.metta`. -/
noncomputable def truthOr (a b : ℝ) : ℝ :=
  1 - (1 - a) * (1 - b)

/-- Upstream canonical symmetric-modus-ponens confidence from `hyperon/PLN/lib_pln.metta`. -/
noncomputable def truthSymmetricModusPonensConf (a ab : TV) : ℝ :=
  a.c * ab.c * truthOr a.s ab.s

/-- Upstream canonical transitive-similarity confidence from `hyperon/PLN/lib_pln.metta`. -/
