...
theorem currentNavierRoadmap_records_nonzero_schwartz_kernel_and_open_canary :
    ({ stage := NavierRoadmapStage.nonzeroEnergyKernel
      ...
      ({ stage := NavierRoadmapStage.nonzeroEnergyKernel
         ...
         truthValue := ⟨91, 89⟩
         obligation :=
          "Use the checked no-uniform-past-dissipation and no-past-spectral-floor obstructions to reject bounded eternal positive-viscosity candidates certified by a uniform positive past dissipation gap or by a positive Poincare/spectral floor of corrected dissipation over normalized energy. Future canary work must construct the exact nonzero evolution without such a floor, or prove a stronger structural emptiness theorem for the Stokes/pressure-closure class." } :
        NavierRoadmapEntry) ∈ currentNavierRoadmap ∧
