...
def currentNavierRoadmap : List NavierRoadmapEntry :=
  [ { stage := .sourceMap
    ...
    { stage := .goertzelGalerkinBakryEmeryCurvature
      ...
      status := .checked
      truthValue := ⟨68, 87⟩
      obligation := "Use FiniteGalerkinBochnerRealization.curvatureDimension as the exact finite-N certificate interface: finite structure constants, skew/Jacobi closure, exact Gamma/Gamma2 Bochner jet realization, and a Ricci quadratic-form lower bound imply CD(-K,infinity), hence the existing BE gradient estimate and conditional BKM gate. AbelianTranslationJet.gamma2_lowerBound_zero certifies the flat two-translation finite sanity case. SuNComputedCurvature.kNTable records only the bracket-closed su(N) trace/Killing metric sanity computation for N=2..6: rho_N = N/2, mode count N^2-1, and CD(-K,infinity) constant K=0. SuNComputedCurvature.metricScope marks this as bi-invariant trace/Killing curvature only, and BochnerData.toFiniteGalerkinBochnerRealization wires only matching bi-invariant finite Bochner data into curvatureDimension and gradientEstimate. Retracted interpretation: this does not realize Ben's NS curvature intuition, does not close the SG/Navier-Stokes CD input, and is not Arnold right-invariant L2/H^m curvature. The genuine next input is RightInvariantSobolevArnoldOpenInput: right-invariant Sobolev-weighted H^m metric, non-Ad-invariance, non-totally-antisymmetric structure constants, mixed-sign Arnold/Misiolek curvature handled directly, and connection to Ben's local H1/H2 generator. Do not treat this as a Navier-Stokes regularity theorem." },
    ...
    { stage := .bkmContinuation
      ...
      status := .checked
      truthValue := ⟨86, 88⟩
      obligation := "Use the checked material-minus-diffusion growth estimates, pointwise derivative d/dt (1/2 |omega|^2) = omega dot partial_t omega, scalar and two-profile Schwartz vorticity time-pairing derivatives, transport algebra omega dot ((u.grad)omega) = (u.grad)(1/2 |omega|^2), Schwartz-slice transport cancellation from incompressibility, coordinate vorticity diffusion dissipation, Schwartz-slice Laplacian IBP, the checked Schwartz curl bridge from velocity slices to vorticity slices, first-seminorm coordinate derivative control by spatialDerivativeComponent_abs_le_schwartzSeminorm_zero_one, witness-level Schwartz-slice a-priori enstrophy estimates, standard-equation-to-raw-enstrophy pairing algebra, conditional dE/dt <= ||grad u||_inf E, and the residual-curl-derived affine-log witness enstrophy estimate with raw integrability derived from Schwartz slices. Remaining enstrophy work is lifting these time-pairing and Schwartz-slice identities to arbitrary finite-energy witnesses." },
    { stage := .bkmContinuation
      proofNodeId := "navier.bkm.log-sobolev-gradient-control"
      status := .checked
      truthValue := ⟨87, 88⟩
      obligation := "Prove the analytic affine log-Sobolev/Biot-Savart pointwise estimate ||grad u|| <= C0 + C1 * Omega log(exp(1) + H). The Biot-Savart attempt now exposes the exact one-slice singular-integral theorem BKMSchwartzSliceVorticityToGradientAffineLogEstimate; BKMSchwartzSliceBiotSavartAffineLogPointwiseEstimate_of_vorticityToGradient and BKMLogSobolevAffinePointwiseFromEnvelope_of_schwartzHighNormEnvelope_and_biotSavart give the checked route from that theorem plus a finite fourth-order Schwartz high-norm envelope to BKMLogSobolevAffinePointwiseFromEnvelope. bkmSchwartzHighNormSeminorm, seminorm_zero_one_le_bkmSchwartzHighNormSeminorm, and spatialFDeriv_norm_le_bkmSchwartzHighNormSeminorm verify that this finite high norm controls the frozen gradient operator norm, but this is only support and not an endpoint Biot-Savart/Riesz-transform bound. BKMFiniteTimeWitnessSchwartzHighNormEnvelopeFromBKMData_of_bkmEnvelopeSchwartzHighNormContinuation shows that the single high-norm/Sobolev frontier supplies that envelope, so BKMLogSobolevAffinePointwiseFromEnvelope_of_bkmEnvelopeSchwartzHighNormContinuation_and_vorticityToGradient reduces the affine component to the one-slice singular-integral theorem when the high-norm frontier is available. mathlib supplies Fourier multipliers, Schwartz/Sobolev membership, and Sobolev inequality tools here, but the targeted audit still found no named Biot-Savart/Riesz-transform/Calderon-Zygmund/BMO/Littlewood-Paley endpoint layer. Strong affine-constant and spectral reconstruction reruns calibrate Taylor-Green/random solenoidal cases only; they show the nonzero-affine C0 form is necessary at small amplitude and remain numerical evidence, not a proof of the analytic gradient estimate." },
    { stage := .bkmContinuation
      proofNodeId := "navier.bkm.single-analytic-lemma"
      status := .checked
      truthValue := ⟨86, 88⟩
      obligation := "Full arbitrary-witness BKM continuation is reduced to the exact one-slice Biot-Savart/log-Sobolev theorem BKMSchwartzSliceVorticityToGradientAffineLogEstimate plus the single high-norm/Sobolev frontier BKMFiniteEnergyWitnessBKMEnvelopeSchwartzHighNormContinuation. The latter reduces Lemma A by BKMHighNormContinuationFromLogControl_of_bkmEnvelopeSchwartzHighNormContinuation and also supplies the high-norm envelope used by the affine component through BKMFiniteTimeWitnessSchwartzHighNormEnvelopeFromBKMData_of_bkmEnvelopeSchwartzHighNormContinuation. BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma_of_vorticityToGradient_and_bkmEnvelopeSchwartzHighNormContinuation records the two-frontier reduction to the bundled arbitrary-witness lemma. The checked reduction is axiom-clean at the Lean baseline and is not a global regularity claim." },
    { stage := .bkmContinuation
...
theorem currentNavierRoadmap_records_bkm_standard_vorticity_growth :
    ({ stage := NavierRoadmapStage.bkmContinuation
       ...
       truthValue := ⟨86, 88⟩
       obligation :=
        "Use the checked material-minus-diffusion growth estimates, pointwise derivative d/dt (1/2 |omega|^2) = omega dot partial_t omega, scalar and two-profile Schwartz vorticity time-pairing derivatives, transport algebra omega dot ((u.grad)omega) = (u.grad)(1/2 |omega|^2), Schwartz-slice transport cancellation from incompressibility, coordinate vorticity diffusion dissipation, Schwartz-slice Laplacian IBP, the checked Schwartz curl bridge from velocity slices to vorticity slices, first-seminorm coordinate derivative control by spatialDerivativeComponent_abs_le_schwartzSeminorm_zero_one, witness-level Schwartz-slice a-priori enstrophy estimates, standard-equation-to-raw-enstrophy pairing algebra, conditional dE/dt <= ||grad u||_inf E, and the residual-curl-derived affine-log witness enstrophy estimate with raw integrability derived from Schwartz slices. Remaining enstrophy work is lifting these time-pairing and Schwartz-slice identities to arbitrary finite-energy witnesses." } :
      NavierRoadmapEntry) ∈ currentNavierRoadmap := by
  simp [currentNavierRoadmap]
...
theorem currentNavierRoadmap_records_bkm_single_analytic_lemma :
    ({ stage := NavierRoadmapStage.bkmContinuation
       ...
       truthValue := ⟨86, 88⟩
       obligation :=
        "Full arbitrary-witness BKM continuation is reduced to the exact one-slice Biot-Savart/log-Sobolev theorem BKMSchwartzSliceVorticityToGradientAffineLogEstimate plus the single high-norm/Sobolev frontier BKMFiniteEnergyWitnessBKMEnvelopeSchwartzHighNormContinuation. The latter reduces Lemma A by BKMHighNormContinuationFromLogControl_of_bkmEnvelopeSchwartzHighNormContinuation and also supplies the high-norm envelope used by the affine component through BKMFiniteTimeWitnessSchwartzHighNormEnvelopeFromBKMData_of_bkmEnvelopeSchwartzHighNormContinuation. BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma_of_vorticityToGradient_and_bkmEnvelopeSchwartzHighNormContinuation records the two-frontier reduction to the bundled arbitrary-witness lemma. The checked reduction is axiom-clean at the Lean baseline and is not a global regularity claim." } :
      NavierRoadmapEntry) ∈ currentNavierRoadmap := by
  simp [currentNavierRoadmap]
