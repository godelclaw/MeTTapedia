...
def currentNavierRoadmap : List NavierRoadmapEntry :=
  [ { stage := .sourceMap
    ...
    { stage := .bkmContinuation
      ...
      status := .checked
      truthValue := ⟨86, 88⟩
      obligation := "Full arbitrary-witness BKM continuation is reduced to the affine Biot-Savart/log-Sobolev frontier BKMLogSobolevAffinePointwiseFromEnvelope and Lemma A, BKMHighNormContinuationFromLogControl, bundled as BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma over the genuine finite-energy ExplicitFiniteTimeRegularityWitness class. Lemma A is now reduced by BKMHighNormContinuationFromLogControl_of_bkmEnvelopeSchwartzHighNormContinuation to one named high-norm/Sobolev frontier, BKMFiniteEnergyWitnessBKMEnvelopeSchwartzHighNormContinuation: from the standard vorticity equation plus a time-integrable BKM vorticity envelope, produce a bounded BKMLogSobolevSchwartzHighNormEnvelopeOn on the closed slab and the existing explicit global-output surface. The first frontier is still sharpened internally to BKMFiniteTimeWitnessSchwartzHighNormEnvelopeFromBKMData plus the exact one-slice singular-integral theorem BKMSchwartzSliceVorticityToGradientAffineLogEstimate. Thus the current public residual surface is exactly two named analytic frontiers, Biot-Savart and high-norm/Sobolev continuation; the checked reduction is axiom-clean at the Lean baseline and is not a global regularity claim." },
    { stage := .bkmContinuation
      proofNodeId := "navier.bkm.analytic-components"
      status := .checked
      truthValue := ⟨87, 88⟩
      obligation := "Use BKMContinuation_reduced_to_single_affineLogBiotSavartGronwallLemma with the checked residual-curl expansion defect closure, pointwise density derivative, scalar/two-profile/finite-family Schwartz time-pairing packages, vorticity raw-balance integrability, transport-cancellation algebra plus Schwartz-slice transport cancellation, the Schwartz curl bridge from velocity slices to vorticity slices, witness-level Schwartz-slice enstrophy control, gradient-growth packages, affine-log stretching integral bound, log-envelope coefficient majorization, scalar Gronwall comparisons, affine log-Sobolev normalization, and BKMHighNormContinuationFromLogControl_of_bkmEnvelopeSchwartzHighNormContinuation. BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma_of_affinePointwiseFromEnvelope_and_bkmEnvelopeSchwartzHighNormContinuation records the current sharp reduction: the repaired arbitrary-witness BKM target follows from BKMLogSobolevAffinePointwiseFromEnvelope plus the single high-norm/Sobolev frontier BKMFiniteEnergyWitnessBKMEnvelopeSchwartzHighNormContinuation. The older scalar/enstrophy high-norm lemmas remain checked support, not separate public residuals; the zero BKM witness premise records non-vacuity, and this is not a global regularity claim." },
...
theorem currentNavierRoadmap_records_bkm_single_analytic_lemma :
    ({ stage := NavierRoadmapStage.bkmContinuation
       ...
       truthValue := ⟨86, 88⟩
       obligation :=
        "Full arbitrary-witness BKM continuation is reduced to the affine Biot-Savart/log-Sobolev frontier BKMLogSobolevAffinePointwiseFromEnvelope and Lemma A, BKMHighNormContinuationFromLogControl, bundled as BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma over the genuine finite-energy ExplicitFiniteTimeRegularityWitness class. Lemma A is now reduced by BKMHighNormContinuationFromLogControl_of_bkmEnvelopeSchwartzHighNormContinuation to one named high-norm/Sobolev frontier, BKMFiniteEnergyWitnessBKMEnvelopeSchwartzHighNormContinuation: from the standard vorticity equation plus a time-integrable BKM vorticity envelope, produce a bounded BKMLogSobolevSchwartzHighNormEnvelopeOn on the closed slab and the existing explicit global-output surface. The first frontier is still sharpened internally to BKMFiniteTimeWitnessSchwartzHighNormEnvelopeFromBKMData plus the exact one-slice singular-integral theorem BKMSchwartzSliceVorticityToGradientAffineLogEstimate. Thus the current public residual surface is exactly two named analytic frontiers, Biot-Savart and high-norm/Sobolev continuation; the checked reduction is axiom-clean at the Lean baseline and is not a global regularity claim." } :
      NavierRoadmapEntry) ∈ currentNavierRoadmap := by
  simp [currentNavierRoadmap]
