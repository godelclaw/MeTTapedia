...
def currentNavierRoadmap : List NavierRoadmapEntry :=
  [ { stage := .sourceMap
    ...
    { stage := .nonzeroEnergyKernel
      ...
      status := .checked
      truthValue := ⟨94, 90⟩
      obligation := "Use the checked no-uniform-past-dissipation, no-past-spectral-floor, past dissipation-per-energy collapse, corrected-dissipation Rayleigh collapse, and spatial Rayleigh collapse obstructions to reject bounded eternal positive-viscosity candidates certified by a uniform positive past dissipation gap, a positive Poincare/spectral floor of corrected dissipation or coordinate enstrophy over normalized energy, or a positive whole-past Rayleigh-quotient floor. Future canary work must construct the exact nonzero evolution while allowing arbitrarily small past spatial Rayleigh-quotient samples, or prove a stronger structural emptiness theorem for the Stokes/pressure-closure class." },
    { stage := .nonzeroEnergyKernel
