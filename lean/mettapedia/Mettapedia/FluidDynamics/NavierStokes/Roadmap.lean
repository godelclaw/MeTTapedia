...
def currentNavierRoadmap : List NavierRoadmapEntry :=
  [ { stage := .sourceMap
    ...
    { stage := .bkmContinuation
      ...
      status := .checked
      truthValue := ⟨86, 88⟩
      obligation := "Use the checked material-minus-diffusion growth estimates, pointwise derivative d/dt (1/2 |omega|^2) = omega dot partial_t omega, scalar and two-profile Schwartz vorticity time-pairing derivatives, transport algebra omega dot ((u.grad)omega) = (u.grad)(1/2 |omega|^2), Schwartz-slice transport cancellation from incompressibility, coordinate vorticity diffusion dissipation, Schwartz-slice Laplacian IBP, the checked Schwartz curl bridge from velocity slices to vorticity slices, witness-level Schwartz-slice a-priori enstrophy estimates, standard-equation-to-raw-enstrophy pairing algebra, conditional dE/dt <= ||grad u||_inf E, and the residual-curl-derived affine-log witness enstrophy estimate with raw integrability derived from Schwartz slices. Remaining enstrophy work is lifting these time-pairing and Schwartz-slice identities to arbitrary finite-energy witnesses." },
    { stage := .bkmContinuation
      proofNodeId := "navier.bkm.log-sobolev-gradient-control"
      status := .checked
      truthValue := ⟨87, 88⟩
      obligation := "Prove the analytic affine log-Sobolev/Biot-Savart pointwise estimate ||grad u|| <= C0 + C1 * Omega log(exp(1) + H). The Biot-Savart attempt now exposes the exact one-slice singular-integral theorem BKMSchwartzSliceVorticityToGradientAffineLogEstimate; BKMSchwartzSliceBiotSavartAffineLogPointwiseEstimate_of_vorticityToGradient and BKMLogSobolevAffinePointwiseFromEnvelope_of_schwartzHighNormEnvelope_and_biotSavart give the checked route from that theorem plus BKMFiniteTimeWitnessSchwartzHighNormEnvelopeFromBKMData to BKMLogSobolevAffinePointwiseFromEnvelope. mathlib supplies Fourier/Schwartz/Sobolev tools here, but no named Biot-Savart/Riesz-transform/Calderon-Zygmund/BMO layer. Checked coefficient-majorization lemmas now turn slabwise Omega/H bounds into the constant gradient coefficient used by the Gronwall layer, and bkmLogSobolevGradientEnvelope_le_scalarLogGrowthCoefficient turns the BKM log envelope into the scalar log-Gronwall coefficient when the chosen scalar controls H. The affine-constant and spectral reconstruction labs calibrate Taylor-Green/random solenoidal cases only; this is not a proof of the analytic gradient estimate." },
    { stage := .bkmContinuation
      proofNodeId := "navier.bkm.single-analytic-lemma"
      status := .checked
      truthValue := ⟨86, 88⟩
      obligation := "Full arbitrary-witness BKM continuation is reduced to the two open analytic lemmas BKMLogSobolevAffinePointwiseFromEnvelope and BKMHighNormContinuationFromLogControl, bundled as BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma over the genuine finite-energy ExplicitFiniteTimeRegularityWitness class. The first open lemma is sharpened internally to BKMFiniteTimeWitnessSchwartzHighNormEnvelopeFromBKMData plus the exact one-slice singular-integral theorem BKMSchwartzSliceVorticityToGradientAffineLogEstimate. The high-norm side now has checked scalar Gronwall comparisons for normalized vorticity enstrophy under constant, constant-majorized, supplied-antiderivative variable-coefficient, and logarithmic BKM growth forms, plus checked algebra turning bounded log-Sobolev envelopes into a constant majorant, the direct scalar-log-Gronwall enstrophy bound normalizedVorticityEnstrophyAt_le_exp_of_logSobolevControl_balance, the bounded-primitive uniform bound normalizedVorticityEnstrophyAt_le_exp_of_logSobolevControl_balance_of_antiderivative_bounded, the reduction BKMHighNormContinuationFromLogControl_of_enstrophyGronwallData_and_boundedEnstrophyBridge, the sharper route BKMHighNormContinuationFromLogControl_of_scalarGronwallData_velocitySchwartz_and_boundedEnstrophyBridge, and the closed-time-pairing route BKMHighNormContinuationFromLogControl_of_primitiveDominationData_velocitySchwartz_closedTimePairing_and_boundedEnstrophyBridge. The remaining high-norm work is exactly to prove BKMLogControlWitnessPrimitiveDominationDataFromWitness, arbitrary-witness velocity-Schwartz extraction plus closed-slab time-pairing extraction or a direct replacement for them, and BKMBoundedEnstrophyContinuationBridge: extract signs, a bounded primitive for 2*C*(1+Omega), and H <= normalized enstrophy from arbitrary witnesses, then connect a finite enstrophy bound to ExplicitConcreteNavierStokesGlobalOutput. The checked reduction is axiom-clean at the Lean baseline and is not a global regularity claim." },
    { stage := .bkmContinuation
      proofNodeId := "navier.bkm.analytic-components"
      status := .checked
      truthValue := ⟨87, 88⟩
      obligation := "Use BKMContinuation_reduced_to_single_affineLogBiotSavartGronwallLemma with the checked residual-curl expansion defect closure, pointwise density derivative, scalar/two-profile/finite-family Schwartz time-pairing packages, vorticity raw-balance integrability, transport-cancellation algebra plus Schwartz-slice transport cancellation, the Schwartz curl bridge from velocity slices to vorticity slices, witness-level Schwartz-slice enstrophy control, gradient-growth packages, affine-log stretching integral bound, log-envelope coefficient majorization, scalar Gronwall comparisons for constant-majorized, supplied-antiderivative variable-coefficient, logarithmic BKM growth, the log-Sobolev-control enstrophy exponential bound, the bounded-primitive uniform enstrophy bound, and the checked high-norm reductions through BKMLogControlWitnessEnstrophyGronwallDataFromWitness plus BKMBoundedEnstrophyContinuationBridge, through BKMLogControlWitnessScalarGronwallDataFromWitness plus velocity-Schwartz/time-pairing extraction plus BKMBoundedEnstrophyContinuationBridge, and through BKMLogControlWitnessPrimitiveDominationDataFromWitness plus velocity-Schwartz/closed-time-pairing extraction plus BKMBoundedEnstrophyContinuationBridge, plus affine log-Sobolev normalization. The repaired arbitrary-witness BKM target follows from BKMLogSobolevAffinePointwiseFromEnvelope plus BKMHighNormContinuationFromLogControl via the bundled lemma BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma; the zero BKM witness premise records non-vacuity. This is not a global regularity claim." },
