...
def currentNavierRoadmap : List NavierRoadmapEntry :=
  [ { stage := .sourceMap
    ...
    { stage := .bkmContinuation
      ...
      status := .checked
      truthValue := ⟨88, 88⟩
      obligation := "Use residualCurlLinearityClosedOn_of_smooth and residualCurlExpansionDefect_eq_differentialIdentityDefects to reduce BKMResidualCurlExpansionDefectVanishes to curl/time commutation, curl/Laplacian commutation, and the incompressible curl-convection identity." },
    { stage := .bkmContinuation
...
theorem currentNavierRoadmap_records_bkm_residual_curl_differential_identities :
    ({ stage := NavierRoadmapStage.bkmContinuation
       ...
       truthValue := ⟨88, 88⟩
       obligation :=
        "Use residualCurlLinearityClosedOn_of_smooth and residualCurlExpansionDefect_eq_differentialIdentityDefects to reduce BKMResidualCurlExpansionDefectVanishes to curl/time commutation, curl/Laplacian commutation, and the incompressible curl-convection identity." } :
      NavierRoadmapEntry) ∈ currentNavierRoadmap := by
  simp [currentNavierRoadmap]
