...
def navierNonzeroSchwartzAntiProfileAmplitudeBoundaryNode : NavierProofNode where
  ...
  status := .checked
  truthValue := ⟨88, 91⟩
  evidence := "antiProfileSchwartzVelocity_nonzero_iff_exists_amplitude_ne proves that, for any nonzero Schwartz profile f, the anti-profile reconstruction a(t)f + b(t)(-f) is nonzero exactly when the scalar amplitudes differ somewhere; localizedStreamAntiProfile_velocity_nonzero_iff_exists_amplitude_ne specializes this to the localized stream seed, not_exists_nonzeroSchwartzConcreteSolution_equalAmplitudeAntiProfileVelocity excludes the equal-amplitude boundary from the nonzero interface, nonzeroSchwartzConcreteSolution_localizedStreamAntiProfileVelocity_forces_amplitude_ne gives the necessary unequal-amplitude condition, and nonzeroSchwartzConcreteSolution_localizedStreamAntiProfileVelocity_forces_amplitudeDifference_nonconstant proves that at positive viscosity any exact nonzero solution using this reconstruction must have genuinely time-varying amplitude difference. PLN STV <s=.88,c=.91>, ITV [.8008,.8908], PROGRESS 65%."
  blocker := "This gives the exact anti-profile nonzero and positive-viscosity nonstationary amplitude boundary, not a pressure-slice momentum closure. The remaining canary must still prove that a time-varying unequal-amplitude localized stream/anti-stream or another non-polynomial Schwartz profile satisfies the full Navier-Stokes equation with Schwartz pressure slices."
