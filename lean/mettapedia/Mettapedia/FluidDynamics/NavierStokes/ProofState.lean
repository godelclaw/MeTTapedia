...
def navierBKMAnalyticComponentsNode : NavierProofNode where
  ...
  status := .checked
  truthValue := ⟨87, 88⟩
  evidence := "BKMLogSobolevGradientControlFromEnvelope and BKMHighNormContinuationFromLogControl name the two analytic estimates still downstream of BKMResidualCurlExpansionDefectVanishes, while BKMVorticityPointwiseEnstrophyDerivativeClosed_proved, BKMVorticityRawBalanceFromStandardEquationClosed_proved, BKMVorticityTransportCancellationAlgebraClosed_proved, BKMVorticityEnstrophyBalanceAssemblyClosed_proved, BKMVorticityEnstrophyAprioriEstimateClosed_proved, and BKMVorticityEnstrophyGradientGrowthClosed_proved supply the checked pointwise density derivative, standard-equation pairing algebra, transport-cancellation algebra, enstrophy-balance assembly, stretching-power derivative control, and conditional gradient-bound enstrophy growth estimate once the remaining integral-analysis identities are proved. BKMAnalyticContinuationLemma_of_components proves these components imply BKMAnalyticContinuationLemma, and BKMContinuation_reduced_to_analytic_components proves the repaired nonnegative-horizon BKM target follows from the component bundle. PLN STV <s=.91,c=.89>, ITV [.8099,.9199], PROGRESS 69%."
