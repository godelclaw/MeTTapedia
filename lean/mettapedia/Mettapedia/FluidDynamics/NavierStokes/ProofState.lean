...
def navierBKMAnalyticComponentsNode : NavierProofNode where
  ...
  status := .checked
  truthValue := ⟨87, 88⟩
  evidence := "BKMLogSobolevGradientControlFromEnvelope and BKMHighNormContinuationFromLogControl name the two analytic estimates still downstream of BKMResidualCurlExpansionDefectVanishes, while BKMVorticityPointwiseEnstrophyDerivativeClosed_proved, BKMVorticityRawBalanceFromStandardEquationClosed_proved, BKMVorticityEnstrophyBalanceAssemblyClosed_proved, and BKMVorticityEnstrophyAprioriEstimateClosed_proved supply the checked pointwise density derivative, standard-equation pairing algebra, enstrophy-balance assembly, and derivative control once the remaining integral-analysis identities are proved. BKMAnalyticContinuationLemma_of_components proves these components imply BKMAnalyticContinuationLemma, and BKMContinuation_reduced_to_analytic_components proves the repaired nonnegative-horizon BKM target follows from the component bundle. PLN STV <s=.89,c=.89>, ITV [.7921,.9021], PROGRESS 64%."
