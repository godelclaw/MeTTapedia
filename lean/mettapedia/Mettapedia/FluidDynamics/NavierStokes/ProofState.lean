...
def navierBKMAnalyticReductionNode : NavierProofNode where
  ...
  blocker := "BKMAnalyticContinuationLemma remains a broad historical reduction. The sharper current missing surface is BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma: prove the affine log-Sobolev/Biot-Savart pointwise estimate and the remaining high-norm continuation bridge on the genuine arbitrary finite-energy ExplicitFiniteTimeRegularityWitness class. The scalar Gronwall comparison for normalized vorticity enstrophy is now checked for constant, constant-majorized, and supplied-antiderivative variable-coefficient gradient growth; the scalar BKM logarithmic Gronwall comparison is checked for a nonnegative controlling quantity satisfying F' <= R(t) * (1 + log(exp(1) + F)) * F; and normalizedVorticityEnstrophyAt_le_exp_of_logSobolevControl_balance instantiates that comparison from log-Sobolev control plus balance/integrability, a primitive for 2*C*(1+Omega), and H <= normalized enstrophy. Constructing the primitive from slab hypotheses, arbitrary-witness slab-data extraction, domination by the actual high norm, and the explicit global-output continuation step remain open."
