...
def navierBKMLogSobolevControlNode : NavierProofNode where
  ...
  blocker := "This closes the downstream interface, sign algebra, affine-to-envelope normalization, the checked implication from BKMSchwartzSliceVorticityToGradientAffineLogEstimate to BKMSchwartzSliceBiotSavartAffineLogPointwiseEstimate, and the checked implication from the one-slice theorem plus the high-norm/Sobolev frontier to BKMLogSobolevAffinePointwiseFromEnvelope. The analytic gap is still real: prove BKMSchwartzSliceVorticityToGradientAffineLogEstimate; current mathlib search found Fourier/Schwartz/Sobolev tools but no named Biot-Savart/Riesz-transform/Calderon-Zygmund/BMO layer."
