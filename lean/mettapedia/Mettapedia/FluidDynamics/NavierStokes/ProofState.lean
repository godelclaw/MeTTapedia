...
def navierBKMAnalyticReductionNode : NavierProofNode where
  ...
  blocker := "BKMAnalyticContinuationLemma remains a broad historical reduction. The sharper current missing surface is BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma: prove the affine log-Sobolev/Biot-Savart pointwise estimate and the remaining high-norm continuation bridge on the genuine arbitrary finite-energy ExplicitFiniteTimeRegularityWitness class. The scalar Gronwall comparison for normalized vorticity enstrophy is now checked for constant, constant-majorized, supplied-antiderivative variable-coefficient gradient growth; the scalar BKM logarithmic Gronwall comparison is checked for a nonnegative controlling quantity satisfying F' <= R(t) * (1 + log(exp(1) + F)) * F; normalizedVorticityEnstrophyAt_le_exp_of_logSobolevControl_balance instantiates that comparison from log-Sobolev control plus balance/integrability, a primitive for 2*C*(1+Omega), and H <= normalized enstrophy; and normalizedVorticityEnstrophyAt_le_exp_of_logSobolevControl_balance_of_antiderivative_bounded turns a bounded primitive into one finite scalar enstrophy bound on the slab. BKMLogControlWitnessEnstrophyGronwallDataFromWitness_of_scalarGronwallData_velocitySchwartz proves that scalar Gronwall data plus velocity-Schwartz slice extraction and the time-pairing derivative discharge the balance/integrability part of the high-norm data residual. continuousOn_normalizedVorticityEnstrophyAt_of_timePairingDerivativeAt derives scalar continuity from a closed-slab time-pairing derivative, and BKMHighNormContinuationFromLogControl_of_primitiveDominationData_velocitySchwartz_closedTimePairing_and_boundedEnstrophyBridge reduces the high-norm component to primitive/sign/domination data, arbitrary-witness velocity-Schwartz extraction, closed-slab time-pairing extraction, and BKMBoundedEnstrophyContinuationBridge. These residuals, plus the exact one-slice singular-integral theorem BKMSchwartzSliceVorticityToGradientAffineLogEstimate, remain open."
...
def navierBKMAnalyticComponentsNode : NavierProofNode where
  ...
  blocker := "Full arbitrary-witness BKM continuation is reduced to the two open analytic lemmas BKMLogSobolevAffinePointwiseFromEnvelope and BKMHighNormContinuationFromLogControl, bundled as BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma over the genuine ExplicitFiniteTimeRegularityWitness class. The first open lemma is now sharpened internally to a Schwartz high-norm envelope extraction plus the exact one-slice singular-integral theorem BKMSchwartzSliceVorticityToGradientAffineLogEstimate; the second now has checked scalar Gronwall comparison lemmas for normalized vorticity enstrophy under constant, constant-majorized, supplied-antiderivative variable-coefficient, and logarithmic BKM growth forms, checked algebra turning bounded log-Sobolev envelopes into a constant majorant, a direct log-Sobolev-control enstrophy exponential bound, a bounded-primitive uniform enstrophy bound, a checked implication from BKMLogControlWitnessEnstrophyGronwallDataFromWitness plus BKMBoundedEnstrophyContinuationBridge to BKMHighNormContinuationFromLogControl, the sharper implication from BKMLogControlWitnessScalarGronwallDataFromWitness plus arbitrary-witness velocity-Schwartz/time-pairing extraction plus BKMBoundedEnstrophyContinuationBridge to BKMHighNormContinuationFromLogControl, and the closed-time-pairing implication from BKMLogControlWitnessPrimitiveDominationDataFromWitness plus velocity-Schwartz extraction plus BKMBoundedEnstrophyContinuationBridge to BKMHighNormContinuationFromLogControl. The remaining high-norm work is exactly primitive/sign/domination extraction, arbitrary-witness velocity-Schwartz extraction plus closed-slab time-pairing extraction or a direct replacement for them, and the bounded-enstrophy continuation bridge to the explicit global-output surface. The checked reduction is axiom-clean at the Lean baseline, the zero BKM witness premise proves non-vacuity, and this is not a global regularity claim."
