...
def navierBakryEmeryConditionalReductionNode : NavierProofNode where
  ...
  status := .checked
  truthValue := ⟨100, 91⟩
  evidence := "BakryEmeryConditionalReduction.lean defines an abstract carre-du-champ semigroup with linear semigroup/generator/evaluation maps, Gamma, an abstract derivative operator, order preservation, diffusion product and self-adjointness facts as fields, and gamma2 defined by the Bochner generator identity. interpolationDerivativeIdentity_func proves d/ds P_s Gamma(P_(t-s) f) = 2 P_s gamma2(P_(t-s) f); interpolationDerivativeLowerBound_of_CD derives the lower bound from CD(-K,infinity). bakryEmeryGradientEstimate then uses the checked identity, the checked CD lower bound, and the scalar Gronwall comparison field. NSBakryEmeryConditionalReduction.reductionPacket proves that CD plus H2, Gamma(phi) <= Cphi^2, gives the identity gradient bound and feeds the abstract Proposition 7.1 / G.7 / G.8 BKM gate chain."
  blocker := "The local SG curvature-dimension bound CD(-K,infinity) near the identity is the exact open SG input LocalSGCurvatureDimensionOpenInput; it is never discharged here. The generic scalar Gronwall comparison is still a framework field because the analytic comparison is not mechanized against Mathlib derivative APIs in this abstraction. The BKM gate remains an abstract conditional scaffold, so this is not a Navier-Stokes regularity theorem."
...
def navierBKMVorticityGrowthNode : NavierProofNode where
  ...
  status := .checked
  truthValue := ⟨86, 88⟩
  evidence := "vorticityMaterialDiffusionRemainder names partial_t omega + (u.grad)omega - nu Delta omega. vorticityMaterialDiffusionRemainder_eq_vorticityStretchingTerm proves that concreteVorticityEquationOn identifies this remainder with (omega.grad)u, and uniform_vorticityMaterialDiffusionRemainder_boundUpTo plus uniform_vorticityMaterialDiffusionPower_boundUpTo lift the checked stretching bounds to the material-minus-diffusion growth and enstrophy-production terms. vorticityEnstrophyAt and normalizedVorticityEnstrophyAt define the BKM quantity 1/2 int |omega|^2; normalizedVorticityEnstrophyDensity and vorticityEnstrophyDensityTimeDerivativeIntegrand identify the pointwise density derivative, and BKMVorticityPointwiseEnstrophyDerivativeClosed_proved proves d/dt (1/2 |omega|^2) = omega dot partial_t omega from vorticityTimeDerivativeRepresentedAt. normalizedVorticityEnstrophyDensityTransport_eq_transportPower proves the pointwise transport algebra omega dot ((u.grad)omega) = (u.grad)(1/2 |omega|^2), BKMVorticityTransportCancellationAlgebraClosed_proved reduces transport cancellation to the zero integral of that transported half-enstrophy density, and BKMVorticityTransportCancellationSchwartzClosed_proved proves that zero integral from incompressibility when velocity and vorticity slices are Schwartz. schwartzSpatialVorticitySlice_apply and finiteTimeWitnessVorticitySchwartzSlices_of_velocitySchwartzSlices prove that a Schwartz velocity slice supplies the vorticity Schwartz slice by taking the checked Schwartz curl, and spatialDerivativeComponent_abs_le_schwartzSeminorm_zero_one proves each frozen coordinate derivative is pointwise controlled by the first unweighted Schwartz seminorm. vorticityDiffusionDissipationDensity is now the coordinate Hilbert-gradient square of the vorticity field, vorticityDiffusionDissipationDensity_eq_coordinateEnstrophyDensity_vorticityField and vorticityDiffusionDissipationAt_eq_coordinateEnstrophyAt_vorticityField align it with the coordinate energy layer, BKMVorticityDiffusionIntegrationByPartsSchwartzClosed_proved closes Laplacian integration by parts for Schwartz vorticity slices, BKMVorticityFiniteTimeWitnessSchwartzSliceAprioriClosed_proved packages raw balance plus transport cancellation plus witness-level Schwartz vorticity slices into the a-priori stretching-controlled enstrophy estimate, BKMVorticityFiniteTimeWitnessVelocityVorticitySchwartzAprioriClosed_proved removes the separate transport-cancellation assumption when the witness has Schwartz velocity and vorticity slices, and BKMVorticityFiniteTimeWitnessVelocitySchwartzAprioriClosed_proved removes the separate vorticity-slice witness input as well. vorticityStretchingPowerIntegral_le_gradient_mul_enstrophyAt proves int omega dot ((omega.grad)u) <= G int |omega|^2 from ||grad u|| <= G plus integrability, and BKMVorticityEnstrophyGradientGrowthClosed_proved packages the derivative growth inequality dE/dt <= G E once the enstrophy balance is available. vorticityTimePowerIntegral records int omega dot partial_t omega; vorticityTimePairing_eq_rawBalanceIntegrand_of_concreteVorticityEquationOn pairs the standard vorticity equation with omega pointwise; vorticityTimePowerIntegral_eq_rawBalanceDerivative_of_concreteVorticityEquationOn integrates that pointwise identity under explicit integrability hypotheses; BKMVorticityRawBalanceFromStandardEquationClosed_proved converts a time-pairing derivative into the raw enstrophy balance; BKMVorticityEnstrophyBalanceAssemblyClosed_proved assembles raw balance plus transport cancellation and diffusion IBP into vorticityEnstrophyBalanceAt; and BKMVorticityEnstrophyAprioriEstimateClosed_proved packages derivative control by the stretching-power integral. Validation labs ns-bkm-enstrophy-log-sobolev-lab-20260629.json, ns-bkm-vorticity-diffusion-ibp-lab-20260629.log, and ns-bkm-hardcore-log-enstrophy-dealiased-lab-20260629.log checked transport cancellation, viscous integration by parts, the coordinate/operator dissipation distinction, the log-envelope scale, and enstrophy finite differences on periodic solenoidal samples. PLN STV <s=.94,c=.90>, ITV [.846,.946], PROGRESS 76%."
  blocker := "This closes the pointwise density derivative, pointwise transport algebra, Schwartz-slice transport cancellation from incompressibility, Schwartz-vorticity-slice diffusion IBP, Schwartz raw-balance integrability, scalar and two-profile Schwartz vorticity time-pairing derivatives, the witness-level Schwartz-slice a-priori enstrophy estimate, the integrated stretching-growth inequality under explicit integrability/gradient-bound hypotheses, and standard-equation-to-raw-enstrophy algebra once the general time-pairing derivative is supplied. The remaining analytic work is the arbitrary finite-energy/witness time-pairing derivative, instantiating or extending the Schwartz-slice transport/diffusion identities for finite-energy witnesses, and proving the genuine Biot-Savart/log-Sobolev pointwise inequality."
...
def navierBKMLogSobolevControlNode : NavierProofNode where
  ...
  status := .checked
  truthValue := ⟨87, 88⟩
  evidence := "bkmLogSobolevGradientEnvelope records the downstream envelope shape C * (1 + Omega(t) * log(exp(1) + H(t))). bkmLogSobolevLogFactor_nonneg_of_nonneg and bkmLogSobolevGradientEnvelope_nonneg_of_nonneg prove the needed sign algebra. bkmLogSobolevLogFactor_le_of_bounds, bkmLogSobolevGradientEnvelope_le_of_bounds, bkmLogSobolevGradientEnvelope_le_constant_on_Ico_of_bounds, and BKMLogSobolevGradientControlOn.to_uniformSpatialGradientOperatorBoundUpTo_of_bounds turn slabwise Omega/H bounds into the constant gradient coefficient used by the checked Gronwall layer. bkmLogSobolevGradientEnvelope_le_scalarLogGrowthCoefficient turns the BKM log envelope into C * (1 + Omega) * (1 + log(exp(1) + F)) when the chosen scalar F dominates H, matching the scalar BKM log-Gronwall coefficient. BKMLogSobolevAffinePointwiseInequalityOn isolates the conventional affine Biot-Savart/log-Sobolev estimate ||grad u|| <= C0 + C1 * Omega log(exp(1) + H), and bkmLogSobolevGradientEnvelope_dominates_affineLog, BKMLogSobolevPointwiseInequalityOn.of_affinePointwiseInequality, BKMLogSobolevGradientControlOn.of_affinePointwiseInequality, and BKMLogSobolevAffineReductionClosed_proved normalize it into the one-constant downstream control predicate. BKMLogSobolevGradientControlOn.to_spatialGradientOperatorEnvelopeOn and BKMLogSobolevGrowthEstimateClosed_proved feed the envelope into the checked material-vorticity and enstrophy-production bounds. vorticityStretchingPowerIntegral_le_affineLog_enstrophyAt and BKMVorticityStretchingAffineLogIntegralBoundClosed_proved prove the direct integral hard-core estimate int omega dot ((omega.grad)u) <= (C0 + C1 * Omega log(exp(1) + H)) * int |omega|^2 from the affine pointwise gradient bound. vorticityEnstrophyGradientControlledAt_of_balance_logSobolev_control and vorticityEnstrophyGradientControlledAt_of_balance_logSobolev_pointwiseInequality prove that the one-constant logarithmic coefficient gives dE/dt <= C * (1 + Omega log(exp(1) + H)) * E once the vorticity enstrophy balance and integrability hypotheses are supplied; BKMVorticityEnstrophyLogSobolevGrowthClosed_proved packages that bridge. vorticityEnstrophyGradientControlledAt_of_balance_logSobolev_affinePointwiseInequality and BKMVorticityEnstrophyAffineLogGrowthClosed_proved keep the conventional affine coefficient directly as dE/dt <= (C0 + C1 * Omega log(exp(1) + H)) * E. vorticityEnstrophyGradientControlledAt_of_finiteTimeWitness_affinePointwiseInequality and BKMVorticityFiniteTimeWitnessAffineLogGrowthClosed_proved package the same affine-log differential inequality at the finite-time witness level from the standard vorticity equation, time-pairing derivative, raw-balance integrability, and Schwartz velocity/vorticity slices; BKMVorticityFiniteTimeWitnessVelocitySchwartzAffineLogGrowthClosed_proved derives the vorticity slice from velocity Schwartz slices. BKMVorticityFiniteTimeWitnessResidualCurlAprioriClosed_proved and BKMVorticityFiniteTimeWitnessResidualCurlAffineLogGrowthClosed_proved remove the separate standard-equation premise for finite-time witnesses by deriving the a-priori and affine-log estimates from residual-curl, and the residual-curl velocity-only packages remove the separate vorticity Schwartz-slice witness input. bkmSchwartzHighNormSeminorm is the finite supremum of Schwartz seminorms with weight/order up to four; seminorm_zero_one_le_bkmSchwartzHighNormSeminorm and spatialFDeriv_norm_le_bkmSchwartzHighNormSeminorm check that this finite high norm controls the frozen gradient operator norm. BKMLogSobolevSchwartzHighNormEnvelopeOn, BKMFiniteTimeWitnessSchwartzHighNormEnvelopeFromBKMData, BKMSchwartzSliceVorticityToGradientAffineLogEstimate, BKMSchwartzSliceBiotSavartAffineLogPointwiseEstimate_of_vorticityToGradient, BKMSchwartzSliceBiotSavartAffineLogPointwiseEstimate, and BKMLogSobolevAffinePointwiseFromEnvelope_of_schwartzHighNormEnvelope_and_biotSavart expose the exact lower-level route from a one-slice Schwartz vorticity-to-gradient affine-log theorem plus a finite high-norm envelope to the current affine-envelope lemma. This is still local high-norm support, not the missing endpoint singular-integral estimate. BKMFiniteTimeWitnessSchwartzHighNormEnvelopeFromBKMData_of_bkmEnvelopeSchwartzHighNormContinuation and BKMLogSobolevAffinePointwiseFromEnvelope_of_bkmEnvelopeSchwartzHighNormContinuation_and_vorticityToGradient show that the single high-norm/Sobolev frontier supplies that envelope, leaving the one-slice singular-integral theorem as the affine component's remaining analytic content once the high-norm frontier is available. Validation probes ns-bkm-log-sobolev-probe-20260629.json, ns-bkm-log-sobolev-probe-hardcore-rerun-20260629.json, ns-bkm-hardcore-log-enstrophy-rerun-20260629.json, ns-bkm-biot-savart-affine-constant-lab-20260629.json, ns-bkm-biot-savart-reconstruction-lab-20260629.json, ns-bkm-biot-savart-affine-constant-lab-strong-20260630.json, and ns-bkm-biot-savart-reconstruction-lab-strong-20260630.json sampled the log envelope ratio, affine constants, spectral Biot-Savart reconstruction, and stretching/enstrophy bound on periodic solenoidal fields. The strong reconstruction rerun passed 455 Taylor-Green/random-solenoidal amplitude cases and confirms the nonzero-affine C0 form is needed at small amplitude; these labs remain numerical evidence only. PLN STV <s=.925,c=.89>, ITV [.82325,.93325], PROGRESS 72%."
  blocker := "This closes the downstream interface, sign algebra, affine-to-envelope normalization, the checked implication from BKMSchwartzSliceVorticityToGradientAffineLogEstimate to BKMSchwartzSliceBiotSavartAffineLogPointwiseEstimate, and the checked implication from the one-slice theorem plus the high-norm/Sobolev frontier to BKMLogSobolevAffinePointwiseFromEnvelope. The analytic gap is final and explicit here: prove BKMSchwartzSliceVorticityToGradientAffineLogEstimate. The lab and audit record is ns-bkm-biot-savart-affine-constant-lab-strong-20260630.json, ns-bkm-biot-savart-reconstruction-lab-strong-20260630.json, and ns-bkm-biot-savart-mathlib-audit-20260630.log; the audit found Fourier multiplier, Schwartz/Sobolev membership, and Sobolev inequality tools, but no named Biot-Savart/Riesz-transform/Calderon-Zygmund/BMO/Littlewood-Paley endpoint layer. The labs calibrate the affine-log form only; they are not a proof of the singular-integral theorem."
...
def navierBKMAnalyticReductionNode : NavierProofNode where
  ...
  status := .checked
  truthValue := ⟨86, 88⟩
  evidence := "BKMVorticityStretchingEstimateClosed_proved closes the stretching subdependency, finite-time witnesses now supply vorticityResidualCurlEquationOn, BKMResidualCurlExpansionAlgebraClosed_proved closes the algebra from residual-curl zero plus vanishing expansion defect to concreteVorticityEquationOn, BKMResidualCurlExpansionDefectVanishes_proved closes that vanishing defect for smooth incompressible slabs, BKMStandardVorticityGrowthEstimateClosed_proved closes the standard-equation growth estimate, BKMVorticityPointwiseEnstrophyDerivativeClosed_proved closes the pointwise derivative d/dt (1/2 |omega|^2) = omega dot partial_t omega, BKMVorticityRawBalanceFromStandardEquationClosed_proved closes the standard-equation-to-raw-enstrophy algebra, BKMVorticityTransportCancellationAlgebraClosed_proved closes the pointwise algebra behind transport cancellation, BKMVorticityTransportCancellationSchwartzClosed_proved closes transport cancellation for incompressible Schwartz velocity/vorticity slices, BKMVorticityDiffusionIntegrationByPartsSchwartzClosed_proved closes the Schwartz-vorticity-slice diffusion IBP subcase, finiteTimeWitnessVorticitySchwartzSlices_of_velocitySchwartzSlices derives that vorticity slice from velocity Schwartz slices, BKMVorticityFiniteTimeWitnessSchwartzSliceAprioriClosed_proved closes the raw-balance/transport/Schwartz-slice witness a-priori estimate, BKMVorticityFiniteTimeWitnessVelocityVorticitySchwartzAprioriClosed_proved closes the raw-balance/Schwartz-velocity/Schwartz-vorticity witness a-priori estimate without a separate transport-cancellation premise, BKMVorticityFiniteTimeWitnessVelocitySchwartzAprioriClosed_proved removes the separate vorticity-slice witness input, BKMVorticityEnstrophyBalanceAssemblyClosed_proved closes the raw-balance/cancellation/IBP assembly, BKMVorticityEnstrophyAprioriEstimateClosed_proved closes the enstrophy derivative control from the balance identity, BKMVorticityEnstrophyGradientGrowthClosed_proved closes the conditional dE/dt <= ||grad u||_inf E growth estimate, BKMVorticityEnstrophyLogSobolevGrowthClosed_proved closes the supplied-log-Sobolev-control dE/dt <= C * (1 + Omega log(exp(1) + H)) * E estimate, BKMVorticityEnstrophyAffineLogGrowthClosed_proved closes the conventional affine-log dE/dt <= (C0 + C1 * Omega log(exp(1) + H)) * E estimate, BKMVorticityStretchingAffineLogIntegralBoundClosed_proved closes the direct affine-log bound on the integrated stretching production, BKMVorticityFiniteTimeWitnessAffineLogGrowthClosed_proved and BKMVorticityFiniteTimeWitnessVelocitySchwartzAffineLogGrowthClosed_proved close the witness-level standard-equation/time-pairing/Schwartz-slice affine-log enstrophy growth estimate with and without an explicit vorticity-slice input, BKMVorticityFiniteTimeWitnessResidualCurlAprioriClosed_proved, BKMVorticityFiniteTimeWitnessResidualCurlVelocitySchwartzAprioriClosed_proved, BKMVorticityFiniteTimeWitnessResidualCurlAffineLogGrowthClosed_proved, and BKMVorticityFiniteTimeWitnessResidualCurlVelocitySchwartzAffineLogGrowthClosed_proved close the same witness-level a-priori and affine-log estimates using the residual-curl equation directly, BKMLogSobolevAffineReductionClosed_proved closes the affine-log normalization, BKMLogSobolevGrowthEstimateClosed_proved closes the supplied-log-Sobolev-control growth interface, and BKMContinuation_reduced_to_single_analytic_lemma proves that the repaired nonnegative-horizon BKM target follows from the single named hypothesis BKMAnalyticContinuationLemma. PLN STV <s=.95,c=.90>, ITV [.855,.955], PROGRESS 84%."
  blocker := "BKMAnalyticContinuationLemma remains a broad historical reduction. The current public missing surface is exactly two named analytic frontiers over the genuine arbitrary finite-energy ExplicitFiniteTimeRegularityWitness class: the exact one-slice affine log-Sobolev/Biot-Savart theorem BKMSchwartzSliceVorticityToGradientAffineLogEstimate and the high-norm/Sobolev continuation frontier BKMFiniteEnergyWitnessBKMEnvelopeSchwartzHighNormContinuation. BKMHighNormContinuationFromLogControl_of_bkmEnvelopeSchwartzHighNormContinuation proves Lemma A from that single frontier, BKMFiniteTimeWitnessSchwartzHighNormEnvelopeFromBKMData_of_bkmEnvelopeSchwartzHighNormContinuation shows that the same frontier supplies the envelope used by the affine component, and BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma_of_vorticityToGradient_and_bkmEnvelopeSchwartzHighNormContinuation records the bundled two-frontier reduction. The scalar/enstrophy Gronwall declarations remain checked support, not separate public residuals. This is not a global regularity claim."
...
def navierBKMAnalyticComponentsNode : NavierProofNode where
  ...
  status := .checked
  truthValue := ⟨87, 88⟩
  evidence := "BKMResidualCurlExpansionDefectVanishes_proved closes the residual-curl expansion defect component for smooth incompressible slabs. BKMLogSobolevAffinePointwiseFromEnvelope names the sharper conventional affine log-Sobolev/Biot-Savart component, and BKMLogSobolevGradientControlFromEnvelope_of_affinePointwiseFromEnvelope proves it implies the older one-constant BKMLogSobolevGradientControlFromEnvelope component. BKMLogSobolevAffinePointwiseFromEnvelope_of_schwartzHighNormEnvelope_and_biotSavart shows that a genuine Schwartz high-norm envelope extraction plus BKMSchwartzSliceBiotSavartAffineLogPointwiseEstimate would close the affine component, and BKMSchwartzSliceBiotSavartAffineLogPointwiseEstimate_of_vorticityToGradient reduces that space-time input to the exact one-slice theorem BKMSchwartzSliceVorticityToGradientAffineLogEstimate. BKMHighNormContinuationFromLogControl names the high-norm/Gronwall component still downstream, while BKMVorticityPointwiseEnstrophyDerivativeClosed_proved, BKMVorticityRawBalanceFromStandardEquationClosed_proved, BKMVorticityTransportCancellationAlgebraClosed_proved, BKMVorticityTransportCancellationSchwartzClosed_proved, BKMVorticityDiffusionIntegrationByPartsSchwartzClosed_proved, BKMVorticityFiniteTimeWitnessSchwartzSliceAprioriClosed_proved, BKMVorticityFiniteTimeWitnessVelocityVorticitySchwartzAprioriClosed_proved, BKMVorticityFiniteTimeWitnessVelocitySchwartzAprioriClosed_proved, BKMVorticityEnstrophyBalanceAssemblyClosed_proved, BKMVorticityEnstrophyAprioriEstimateClosed_proved, BKMVorticityEnstrophyGradientGrowthClosed_proved, BKMVorticityEnstrophyLogSobolevGrowthClosed_proved, BKMVorticityEnstrophyAffineLogGrowthClosed_proved, BKMVorticityStretchingAffineLogIntegralBoundClosed_proved, BKMVorticityFiniteTimeWitnessAffineLogGrowthClosed_proved, BKMVorticityFiniteTimeWitnessVelocitySchwartzAffineLogGrowthClosed_proved, BKMVorticityFiniteTimeWitnessResidualCurlAprioriClosed_proved, BKMVorticityFiniteTimeWitnessResidualCurlVelocitySchwartzAprioriClosed_proved, BKMVorticityFiniteTimeWitnessResidualCurlAffineLogGrowthClosed_proved, BKMVorticityFiniteTimeWitnessResidualCurlVelocitySchwartzAffineLogGrowthClosed_proved, BKMVorticityFiniteTimeWitnessResidualCurlVelocitySchwartzFiniteFamilyHardCoreAffineLogClosed_proved, BKMLogSobolevAffineReductionClosed_proved, bkmLogSobolevGradientEnvelope_le_constant_on_Ico_of_bounds, BKMLogSobolevGradientControlOn.to_uniformSpatialGradientOperatorBoundUpTo_of_bounds, bkmLogSobolevGradientEnvelope_le_scalarLogGrowthCoefficient, normalizedVorticityEnstrophyAt_le_gronwallBound_of_gradientControlled_constant, normalizedVorticityEnstrophyAt_le_gronwallBound_of_gradientControlled_majorized, normalizedVorticityEnstrophyAt_mul_exp_neg_antiderivative_le_initial, normalizedVorticityEnstrophyAt_le_initial_mul_exp_antiderivative, bkmScalarLogGronwallFactor_mul_exp_neg_antiderivative_le_initial, bkmScalarLogGronwallFactor_le_initial_mul_exp_antiderivative, bkmScalarLogGronwallValue_le_exp_initial_logFactor_mul_exp_antiderivative, normalizedVorticityEnstrophyAt_le_exp_of_logSobolevGradientControlled, normalizedVorticityEnstrophyAt_le_exp_of_logSobolevControl_balance, continuousOn_normalizedVorticityEnstrophyAt_of_timePairingDerivativeAt, BKMLogControlWitnessEnstrophyGronwallDataFromWitness_of_scalarGronwallData_velocitySchwartz, BKMLogControlWitnessEnstrophyGronwallDataFromWitness_of_primitiveDominationData_velocitySchwartz_closedTimePairing, BKMHighNormContinuationFromLogControl_of_scalarGronwallData_velocitySchwartz_and_boundedEnstrophyBridge, and BKMHighNormContinuationFromLogControl_of_primitiveDominationData_velocitySchwartz_closedTimePairing_and_boundedEnstrophyBridge supply the checked pointwise density derivative, standard-equation pairing algebra, transport-cancellation algebra, Schwartz-slice transport cancellation, Schwartz-slice diffusion IBP, the Schwartz curl bridge from velocity slices to vorticity slices, witness-level Schwartz-slice a-priori estimates, enstrophy-balance assembly, stretching-power derivative control, conditional gradient-bound enstrophy growth estimate, logarithmic-envelope enstrophy growth estimate, conventional affine-log enstrophy growth estimate, direct affine-log stretching integral estimate, witness-level a-priori and affine-log enstrophy estimates from either standard-equation or residual-curl inputs, the finite-family hard-core affine-log chain, affine-log normalization, log-envelope coefficient majorization, constant/majorized/supplied-antiderivative scalar Gronwall comparisons, the scalar BKM log-Gronwall bound for F' <= R(t) * (1 + log(exp(1) + F)) * F, its log-Sobolev-control enstrophy instantiation, the route from scalar Gronwall data plus velocity/time-pairing extraction to the high-norm component modulo bounded-enstrophy continuation, and the sharpened route from primitive/sign/domination data plus closed-slab time-pairing to the same high-norm component. BKMContinuation_reduced_to_single_affineLogBiotSavartGronwallLemma proves that after the checked scaffolding, the repaired nonnegative-horizon BKM target follows from the single named analytic hypothesis BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma. PLN STV <s=.972,c=.90>, ITV [.8748,.9648], PROGRESS 93%."
  blocker := "Final two-frontier deliverable: full arbitrary-witness BKM continuation is reduced to exactly BKMSchwartzSliceVorticityToGradientAffineLogEstimate and BKMFiniteEnergyWitnessBKMEnvelopeSchwartzHighNormContinuation. The high-norm frontier reduces Lemma A and supplies the bounded BKMLogSobolevSchwartzHighNormEnvelopeOn used by the affine component; the one-slice singular-integral theorem is the Biot-Savart/log-Sobolev content. The composed reduction is BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma_of_vorticityToGradient_and_bkmEnvelopeSchwartzHighNormContinuation together with BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma.implies_finiteEnergyBKMContinuationTargetOnNonnegHorizons. The lab and audit record for the Biot-Savart frontier is ns-bkm-biot-savart-affine-constant-lab-strong-20260630.json, ns-bkm-biot-savart-reconstruction-lab-strong-20260630.json, and ns-bkm-biot-savart-mathlib-audit-20260630.log. The checked reduction has Lean dependency seal {propext, Classical.choice, Quot.sound}, the zero BKM witness premise proves non-vacuity, (omega.grad)u is handled by the checked stretching/enstrophy layer, and this is not a global regularity claim."
...
theorem currentNavierBKMResidualCurlDifferentialIdentities_node
    ...
    residualCurlExpansionClosedOn ν u T ∧
      ...
      BKMVorticityFiniteTimeWitnessResidualCurlVelocitySchwartzAprioriClosed ∧
...
theorem currentNavierBKMAnalyticComponents_node :
  ...
  exact
    ⟨BKMVorticityStretchingEstimateClosed_proved,
      ...
      BKMLogSobolevAffineReductionClosed_proved,
      BKMLogSobolevGrowthEstimateClosed_proved,
      (BKMContinuation_reduced_to_affineLogHighNorm_after_residualCurl).2.2.2.2.2,
      ...
      fun h =>
        h.implies_finiteEnergyBKMContinuationTargetOnNonnegHorizons
          BKMResidualCurlExpansionDefectVanishes_proved,
      fun ν T => zeroFiniteTimeRegularityWitness_exhibits_BKMContinuationPremise ν T,
