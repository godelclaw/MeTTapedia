...
def navierBKMAnalyticReductionNode : NavierProofNode where
  ...
  blocker := "BKMAnalyticContinuationLemma remains a broad historical reduction. The current public missing surface is exactly two named analytic frontiers over the genuine arbitrary finite-energy ExplicitFiniteTimeRegularityWitness class: the affine log-Sobolev/Biot-Savart frontier BKMLogSobolevAffinePointwiseFromEnvelope, internally pinned to BKMSchwartzSliceVorticityToGradientAffineLogEstimate plus high-norm envelope extraction, and the high-norm/Sobolev continuation frontier BKMFiniteEnergyWitnessBKMEnvelopeSchwartzHighNormContinuation. BKMHighNormContinuationFromLogControl_of_bkmEnvelopeSchwartzHighNormContinuation proves Lemma A from that single frontier, and BKMArbitraryWitnessAffineLogBiotSavartGronwallContinuationLemma_of_affinePointwiseFromEnvelope_and_bkmEnvelopeSchwartzHighNormContinuation records the bundled reduction. The scalar/enstrophy Gronwall declarations remain checked support, not separate public residuals. This is not a global regularity claim."
...
def navierBKMAnalyticComponentsNode : NavierProofNode where
  ...
  blocker := "Full arbitrary-witness BKM continuation is reduced to two named analytic frontiers. The affine frontier is BKMLogSobolevAffinePointwiseFromEnvelope, internally sharpened to a Schwartz high-norm envelope extraction plus the exact one-slice singular-integral theorem BKMSchwartzSliceVorticityToGradientAffineLogEstimate. Lemma A, BKMHighNormContinuationFromLogControl, is reduced by BKMHighNormContinuationFromLogControl_of_bkmEnvelopeSchwartzHighNormContinuation to the single high-norm/Sobolev frontier BKMFiniteEnergyWitnessBKMEnvelopeSchwartzHighNormContinuation: from the standard vorticity equation and a time-integrable BKM vorticity envelope, produce a bounded BKMLogSobolevSchwartzHighNormEnvelopeOn on the closed slab and the existing explicit global-output surface. The checked reduction is axiom-clean at the Lean baseline, the zero BKM witness premise proves non-vacuity, and this is not a global regularity claim."
