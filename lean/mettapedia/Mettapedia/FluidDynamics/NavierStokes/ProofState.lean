...
def navierBKMLogSobolevControlNode : NavierProofNode where
  ...
  blocker := "This closes the downstream interface, sign algebra, affine-to-envelope normalization, the checked implication from BKMSchwartzSliceVorticityToGradientAffineLogEstimate to BKMSchwartzSliceBiotSavartAffineLogPointwiseEstimate, and the checked implication from the one-slice theorem plus the high-norm/Sobolev frontier to BKMLogSobolevAffinePointwiseFromEnvelope. The analytic gap is still real: prove BKMSchwartzSliceVorticityToGradientAffineLogEstimate; the targeted mathlib audit found Fourier multiplier, Schwartz/Sobolev membership, and Sobolev inequality tools, but no named Biot-Savart/Riesz-transform/Calderon-Zygmund/BMO/Littlewood-Paley endpoint layer."
...
def navierBKMAnalyticComponentsNode : NavierProofNode where
  ...
  blocker := "Full arbitrary-witness BKM continuation is reduced to two named analytic frontiers: BKMSchwartzSliceVorticityToGradientAffineLogEstimate and BKMFiniteEnergyWitnessBKMEnvelopeSchwartzHighNormContinuation. The high-norm frontier reduces Lemma A and supplies the bounded BKMLogSobolevSchwartzHighNormEnvelopeOn used by the affine component; the one-slice singular-integral theorem is the remaining Biot-Savart/log-Sobolev content. The checked reduction is axiom-clean at the Lean baseline, the zero BKM witness premise proves non-vacuity, and this is not a global regularity claim."
