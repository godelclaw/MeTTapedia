...
/-!
# A joint spatial diffusion envelope

The coordinate responses are summed before integrating. Only continuous
finite-difference upper envelopes and the measurable full rate occur in
integrals; no measurability of a selected eigenvector is needed.
-/

set_option autoImplicit false
noncomputable section

namespace Mettapedia.FluidDynamics.NavierStokes.PancakeLaplacianDiffusion
...
theorem linearRate_sum {ι : Type*} (I : Finset ι) (S : SymmetricStrain) (delta : ℝ) (w : R3)
    (A : ι → R3 →L[ℝ] R3) (b : ι → R3) :
    linearRate S delta w (∑ i ∈ I, A i) (∑ i ∈ I, b i) = ∑ i ∈ I, linearRate S delta w (A i) (b i) := by
  classical
  induction I using Finset.induction_on with
  | empty => simp [linearRate, remainderAnisotropy]
  | @insert i I hi ih => simp only [Finset.sum_insert hi, linearRate_add, ih]

def centralUpper (S : T3 → SymmetricStrain) (w : T3 → R3) (delta h : ℝ) (s x : T3) : ℝ :=
  (h ^ 2)⁻¹ * (shiftEnergy S w delta s x + shiftEnergy S w delta (-s) x +
    (8 / delta) * (shiftCost S w s x + shiftCost S w (-s) x))

theorem linearRate_central_le_upper (S : T3 → SymmetricStrain) (w : T3 → R3)
    (delta h : ℝ) (s x : T3) (hd : 0 < delta) :
    linearRate (S x) delta (w x) (centralDirection (fun y ↦ (S y).1) s h x)
      (centralDirection w s h x) ≤ centralUpper S w delta h s x := by
  rw [linearRate_centralDirection]
  apply mul_le_mul_of_nonneg_left ?_ (inv_nonneg.mpr (sq_nonneg h))
  have hp := shiftRate_paid S w delta s x hd
  have hm := shiftRate_paid S w delta (-s) x hd
...
theorem continuous_centralUpper (S : T3 → SymmetricStrain) (w : T3 → R3)
    (delta h : ℝ) (s : T3) (hS : Continuous S) (hw : Continuous w) :
    Continuous (centralUpper S w delta h s) :=
  (((continuous_shiftEnergy S w delta s hS hw).add
    (continuous_shiftEnergy S w delta (-s) hS hw)).add
    (((continuous_shiftCost S w s hS hw).add
      (continuous_shiftCost S w (-s) hS hw)).const_mul (8 / delta))).const_mul _

theorem integral_centralUpper_le (S : T3 → SymmetricStrain) (w : T3 → R3)
    (delta L h : ℝ) (s : T3) (hd : 0 < delta) (hL : 0 ≤ L) (hh : h ≠ 0)
    (hS : Continuous S) (hw : Continuous w)
    (hp : ∀ x, ‖(S (x + s)).1 - (S x).1‖ ≤ L * |h|)
    (hm : ∀ x, ‖(S (x + -s)).1 - (S x).1‖ ≤ L * |h|) :
    (∫ x : T3, centralUpper S w delta h s x) ≤
      (16 / delta) * L ^ 2 * (∫ x : T3, ‖w x‖ ^ 2) := by
  let E := fun x ↦ shiftEnergy S w delta s x + shiftEnergy S w delta (-s) x
  let C := fun x ↦ shiftCost S w s x + shiftCost S w (-s) x
  ...
  have hC : Integrable C := ((continuous_shiftCost S w s hS hw).add
    (continuous_shiftCost S w (-s) hS hw)).integrable_of_hasCompactSupport
      (HasCompactSupport.of_compactSpace _)
  have hW : Integrable (fun x ↦ ‖w x‖ ^ 2) :=
  ...
  have hcost := integral_mono hC (hW.const_mul (2 * L ^ 2 * h ^ 2)) (fun x ↦ by
    have h1 := shiftCost_le S w L h s x hL (hp x)
    have h2 := shiftCost_le S w L h (-s) x hL (hm x)
    dsimp only [C]
    linarith)
  ...
  calc
    _ ≤ (h ^ 2)⁻¹ * ((8 / delta) * (2 * L ^ 2 * h ^ 2 * (∫ x : T3, ‖w x‖ ^ 2))) := by
      gcongr
...
theorem integral_joint_rate_le_at_increment {ι : Type*} (I : Finset ι)
    ...
    (F B : T3 → ℝ) (delta nu h : ℝ) (L eS ew : ι → ℝ) (s : ι → T3)
    (hd : 0 < delta) (hnu : 0 ≤ nu) (hL : ∀ i ∈ I, 0 ≤ L i) (hh : h ≠ 0)
  ...
  have hU : ∀ i, Integrable (U i) := fun i ↦
    (continuous_centralUpper S w delta h (s i) hS hw).integrable_of_hasCompactSupport
      (HasCompactSupport.of_compactSpace _)
  have hUs : Integrable (fun x ↦ ∑ i ∈ I, U i x) :=
    (continuous_finsetSum I (fun i _ ↦ continuous_centralUpper S w delta h (s i) hS hw)).integrable_of_hasCompactSupport
      (HasCompactSupport.of_compactSpace _)
  ...
  have hp : ∀ᵐ x : T3, F x ≤ nu * (∑ i ∈ I, U i x) + B' x := by
    ...
    have hs := Finset.sum_le_sum (s := I) (fun i hi ↦ show linearRate (S x) delta (w x) (A i x) (b i x) ≤
      ...
      have hu := linearRate_central_le_upper S w delta h (s i) x hd
  ...
  have hc := Finset.sum_le_sum (s := I) (fun i hi ↦
    integral_centralUpper_le S w delta (L i) h (s i) hd (hL i hi) hh hS hw (hplus i hi) (hminus i hi))
  have hc' : (∑ i ∈ I, ∫ x : T3, U i x) ≤ (16 / delta) * (∑ i ∈ I, L i ^ 2) * (∫ x : T3, W x) := by
...
theorem integral_joint_rate_le_of_uniform_approx {ι : Type*} (I : Finset ι)
    (S : T3 → SymmetricStrain) (w : T3 → R3)
    (A : ι → T3 → R3 →L[ℝ] R3) (b : ι → T3 → R3)
    (F B : T3 → ℝ) (delta nu : ℝ) (L : ι → ℝ) (shift : ι → ℝ → T3) (eS ew : ι → ℝ → ℝ)
    (hd : 0 < delta) (hnu : 0 ≤ nu) (hL : ∀ i ∈ I, 0 ≤ L i)
    (hS : Continuous S) (hw : Continuous w) (hF : Integrable F) (hB : Integrable B)
    (hpoint : ∀ᵐ x : T3, F x ≤ nu * linearRate (S x) delta (w x)
      (∑ i ∈ I, A i x) (∑ i ∈ I, b i x) + B x)
    (hAp : ∀ h ≠ 0, ∀ i ∈ I, ∀ x,
  ...
  have ht := (((tendsto_const_nhds (x :=
    (16 * nu / delta) * (∑ i ∈ I, L i ^ 2) * (∫ x : T3, ‖w x‖ ^ 2) + ∫ x : T3, B x)).add
  ...
  have ht' : Tendsto (fun h ↦
      (16 * nu / delta) * (∑ i ∈ I, L i ^ 2) * (∫ x : T3, ‖w x‖ ^ 2) + (∫ x : T3, B x) +
      2 * nu * (∑ i ∈ I, eS i h) * (∫ x : T3, ‖w x‖ ^ 2) +
      2 * nu * (∑ i ∈ I, ew i h) * (∫ x : T3, ‖regularizedGap (S x) delta (w x)‖))
      (𝓝[≠] (0 : ℝ))
      (𝓝 ((16 * nu / delta) * (∑ i ∈ I, L i ^ 2) * (∫ x : T3, ‖w x‖ ^ 2) + ∫ x : T3, B x)) := by
    simpa only [mul_zero, zero_mul, add_zero] using ht
  apply ge_of_tendsto ht'
  filter_upwards [self_mem_nhdsWithin] with h hh
  exact integral_joint_rate_le_at_increment I S w A b F B delta nu h L (fun i ↦ eS i h)
    (fun i ↦ ew i h) (fun i ↦ shift i h) hd hnu hL hh hS hw hF hB hpoint
    (hAp h hh) (hbp h hh) (hplus h hh) (hminus h hh)

end Mettapedia.FluidDynamics.NavierStokes.PancakeLaplacianDiffusion
