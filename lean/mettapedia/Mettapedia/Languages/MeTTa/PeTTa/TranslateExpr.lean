...
The lowering is deliberately specified through PeTTa's own operational model.
...
/-- **Deduplication contract** (pure list-level, no Prolog):
    ...
    The exact dedup function (first-
...
/-- **Semantic correctness of PeTTa's unique fix**:
    ...
    Only membership preservation is needed for this result. -/
