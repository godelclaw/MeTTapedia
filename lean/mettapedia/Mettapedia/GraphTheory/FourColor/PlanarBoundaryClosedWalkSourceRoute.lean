...
/-- Same-embedding route endpoint from the repaired finite boundary split carried directly by the
source's annulus boundary data. -/
theorem
    theorem49BoundaryRootNonemptySynthesis_of_closedWalkAnnulusBoundarySource_and_canonicalWitnessChoice_and_sourceBoundaryDataInducedCrossComponentChordFree_direct
    {G : SimpleGraph V}
    [Fintype G.edgeSet] [FiniteDimensional F2 (G.edgeSet → Color)]
    {emb : PlaneEmbeddingWithBoundary G}
    (source : PlanarBoundaryClosedWalkAnnulusBoundarySource emb)
