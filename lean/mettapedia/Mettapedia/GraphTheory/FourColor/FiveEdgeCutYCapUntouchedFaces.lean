...
theorem capTargetReturn_internal_val
  ...
  simp only [Equiv.Perm.mul_apply,
    extendBoundaryPerm_apply_internal RS data.keep data.yBoundaryCycle dart,
...
/-- Against the facial boundary order, the Y-capped shore has exactly the
untouched ambient faces plus its four boundary-closing faces. -/
theorem card_cap_faceCycleLengths_eq_four_add_ambient
  ...
  calc
    (GoertzelV24CurvatureScope.faceCycleLengths
        data.capRotationSystem).card =
        4 + Fintype.card data.CapNonBoundaryOrbit :=
      data.card_cap_faceCycleLengths_eq_four_add_nonHit hsucc
    _ = 4 + Fintype.card (AmbientRetainedFaceOrbit RS data.keep) := by
