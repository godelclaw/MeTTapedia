import Mettapedia.GraphTheory.FourColor.FiveEdgeCutPentagonCapFaces
...
theorem capTargetReturn_internal_val
  ...
  simp only [Equiv.Perm.mul_apply,
    extendBoundaryPerm_apply_internal RS data.keep data.pentBoundaryCycle dart,
...
/-- Against the facial boundary order, the pentagon-capped shore has the
untouched ambient faces, its five sectors, and the inner pentagon. -/
theorem card_cap_faceCycleLengths_eq_six_add_ambient
    (hsucc : GoertzelV24RetainedRegionBoundaryOrder.retainedRegionBoundarySuccessor
        RS data.keep =
      data.crossingBoundaryDartEquiv.permCongr (finRotate 5)⁻¹) :
    (GoertzelV24CurvatureScope.faceCycleLengths
      data.capRotationSystem).card =
      6 + Fintype.card (AmbientRetainedFaceOrbit RS data.keep) := by
  calc
    (GoertzelV24CurvatureScope.faceCycleLengths
        data.capRotationSystem).card =
        6 + Fintype.card data.CapNonBoundaryOrbit :=
      data.card_cap_faceCycleLengths_eq_six_add_nonHit hsucc
    _ = 6 + Fintype.card (AmbientRetainedFaceOrbit RS data.keep) := by
      rw [data.card_capNonBoundaryOrbit_eq_ambientRetainedFaceOrbit]
