...
import Mettapedia.GraphTheory.FourColor.RungPair3Shape

/-! No long `RungPair3` chain in a least counterexample, from the tube-free lemma. -/
...
/-- **No long `RungPair3` chain in a least counterexample.** -/
theorem le_of_tube (minimal : GraphBackedVertexMinimalTaitCounterexample rotation)
    ...
    n + 1 ≤ Nat.factorial 10 * (Nat.factorial 10 * (6 * 10 + 1)) * 2 ^ (4 ^ 10) :=
