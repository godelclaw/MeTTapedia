...
(`rightSet_appendRing`).  A supplied stabilisation check gives stationarity for every inner tangle.
Concrete width-specific decision proofs are not imported here.
-/
