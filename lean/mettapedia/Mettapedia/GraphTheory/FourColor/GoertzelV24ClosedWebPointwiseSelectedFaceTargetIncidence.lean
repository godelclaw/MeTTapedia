import Mettapedia.GraphTheory.FourColor.GoertzelV24ClosedWebPointwiseSelectedFaceTargetCode
...
namespace GoertzelV24ClosedWebPointwiseSelectedFaceTargetIncidence
...
uniform incidence. -/
noncomputable def pointwiseSelectedSourceLocalLayerSerialCellRebaseUniformFaceTargetEntryAt
    (graphData : Data G)
    ...
    Option (SourceLocalLayerSerialCellRebaseUniformFaceTargetEntry
      (pointwiseSelectedSourceLocalLayerSerialCellRebaseFaceInteractionCarrierAt web.toFormation corridor
        hinterior offset hnext).card) := by
  classical
  ...
  if hpresent : ExteriorIncidencePresent graph targetDartAt Prod.fst
    ...
    if houtsideUniform : outside ∈ uniform then
      ...
      have htargetUniform : targetDartAt incidence.1 ∈ uniform :=
        pointwiseSelectedSourceLocalLayerSerialCellRebase_activeFaceTarget_mem_uniform graphData
          minimal caps coloring web corridor hinterior offset hnext
...
theorem pointwiseSelectedSourceLocalLayerSerialCellRebaseUniformFaceTargetEntryAt_isSome_iff
    (graphData : Data G)
    (minimal : GraphBackedVertexMinimalTaitCounterexample graphData)
    (caps : OrientedFacialPentagonCapPair graphData)
    (coloring :
    ...
    (pointwiseSelectedSourceLocalLayerSerialCellRebaseUniformFaceTargetEntryAt graphData minimal
      caps coloring web corridor hinterior offset hnext incidence).isSome ↔
      let graph := pointwiseSelectedSourceLocalLayerBoundaryRebaseSuccessorFaceGraphAt web.toFormation corridor
        hinterior offset hnext
  ...
  classical
  unfold pointwiseSelectedSourceLocalLayerSerialCellRebaseUniformFaceTargetEntryAt
  dsimp only
...
theorem pointwiseSelectedSourceLocalLayerSerialCellRebaseUniformFaceTargetEntryAt_left
    (graphData : Data G)
    (minimal : GraphBackedVertexMinimalTaitCounterexample graphData)
    (caps : OrientedFacialPentagonCapPair graphData)
    (coloring :
    ...
    (hentry :
      pointwiseSelectedSourceLocalLayerSerialCellRebaseUniformFaceTargetEntryAt graphData minimal
        caps coloring web corridor hinterior offset hnext incidence =
    ...
    pointwiseSelectedSourceLocalLayerSerialCellRebaseUniformFaceDartAt web.toFormation corridor hinterior offset
      ...
      ∀ targetSlot,
        pointwiseSelectedSourceLocalLayerSerialCellRebaseUniformFaceTargetSourceAt web.toFormation corridor
          hinterior offset hnext targetSlot ≠ some uniformSlot := by
  classical
  dsimp only
  unfold pointwiseSelectedSourceLocalLayerSerialCellRebaseUniformFaceTargetEntryAt at hentry
      ...
      · intro targetSlot hsource
        have hdart :=
          pointwiseSelectedSourceLocalLayerSerialCellRebaseUniformFaceTargetSourceAt_dart_eq
            web.toFormation corridor hinterior offset hnext targetSlot
              (carrierCoordinate
                (pointwiseSelectedSourceLocalLayerSerialCellRebaseFaceInteractionCarrierAt
                  web.toFormation corridor hinterior offset hnext)
                ⟨faceInterfaceIncidenceVertex web.annular.RS
                    (fun slot : Fin
                      (pointwiseSelectedSourceLocalLayerSerialFaceTransitionCarrierAt web.toFormation corridor
                        hinterior (sourceLocalLayerNextOffset offset hnext)).card =>
                      ((carrierCoordinate
                        (pointwiseSelectedSourceLocalLayerSerialFaceTransitionCarrierAt web.toFormation corridor
                          hinterior (sourceLocalLayerNextOffset offset hnext))).symm
                            slot).1)
                    incidence,
                  houtsideUniform⟩)
              hsource
        exact hpresent.1 targetSlot (by
          simpa [pointwiseSelectedSourceLocalLayerSerialCellRebaseUniformFaceDartAt] using
            hdart)
    · cases hentry
  · cases hentry
...
theorem pointwiseSelectedSourceLocalLayerSerialCellRebaseUniformFaceTargetEntryAt_right
    (graphData : Data G)
    (minimal : GraphBackedVertexMinimalTaitCounterexample graphData)
    (caps : OrientedFacialPentagonCapPair graphData)
    (coloring :
  ...
  · split at hentry <;> rename_i houtsideUniform
    ...
    · simp only [Option.some.injEq, Sum.inr.injEq] at hentry
      ...
      have hbase : uniformDartAt
        ...
        simp [uniformDartAt,
          pointwiseSelectedSourceLocalLayerSerialCellRebaseUniformFaceDartAt, uniform]
      have hvertex :
          faceInterfaceIncidenceVertex web.annular.RS uniformDartAt
              (carrierCoordinate uniform
                ⟨targetDartAt incidence.1,
...
end GoertzelV24ClosedWebPointwiseSelectedFaceTargetIncidence
