...
/-! `RungThick4` is a slab shape: two edge-darts per vertex and a connected interior. -/

namespace Mettapedia.GraphTheory.FourColor.TubeSlab.RungThick4

open Presented Pres

theorem two_darts : ∀ v : Fin 10, ∃ x x' : SlabDart (P.toTangle valid),
...
theorem up_8 : Relation.ReflTransGen R 0 8 :=
  up_7.trans (Relation.ReflTransGen.single (P.adj valid 7))
theorem down_8 : Relation.ReflTransGen R 8 0 :=
  (Relation.ReflTransGen.single (P.adj' valid 7)).trans down_7
theorem up_9 : Relation.ReflTransGen R 0 9 :=
  up_8.trans (Relation.ReflTransGen.single (P.adj valid 8))
theorem down_9 : Relation.ReflTransGen R 9 0 :=
  (Relation.ReflTransGen.single (P.adj' valid 8)).trans down_8

theorem up (v : Fin 10) : Relation.ReflTransGen R 0 v := by
  fin_cases v
  · exact up_0
  · exact up_1
  · exact up_2
...
theorem down (v : Fin 10) : Relation.ReflTransGen R v 0 := by
  fin_cases v
  · exact down_0
  · exact down_1
  · exact down_2
...
theorem connected (u v : Fin 10) : Relation.ReflTransGen R u v := (down u).trans (up v)
