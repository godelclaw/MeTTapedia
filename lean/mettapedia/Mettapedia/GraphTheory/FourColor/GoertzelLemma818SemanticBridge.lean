...
theorem chainFiberRootClosureStep_symmetric_of_connected
    {word : List GoertzelLemma818FrontierMode.TauOrient}
    {key : List GoertzelLemma814.LColor}
    (hconn : Connected (chainFiberRootClosureStep word key)) :
    StepSymmetric (chainFiberRootClosureStep word key) := by
  intro x y hxy
  exact chainFiberRootClosureReach_mem_of_mem hxy (hconn y x)

theorem chainFiberRootClosureParentRowsSymmetricRootedConnectedCertificate_of_connected_nonempty
    {word : List GoertzelLemma818FrontierMode.TauOrient}
    {key : List GoertzelLemma814.LColor}
    (hconn : Connected (chainFiberRootClosureStep word key))
    (hne : Nonempty (ChainFiberPoint word key)) :
    ∃ totalDecidableEq : DecidableEq (ChainFiberPoint word key),
      Nonempty
        (@ParentRowsSymmetricRootedConnectedCertificate
          (ChainFiberPoint word key)
          totalDecidableEq
          (chainFiberRootClosureStep word key)) := by
  classical
  let totalDecidableEq : DecidableEq (ChainFiberPoint word key) :=
    Classical.decEq _
  letI := totalDecidableEq
  have hRootMem :
      chainFiberRootState word key ∈ concreteChainFiber word key := by
    rcases hne with ⟨point⟩
    cases hfiber : concreteChainFiber word key with
    | nil =>
        have hpoint : point.1 ∈ ([] : List (List GoertzelLemma814.TauState)) := by
          simpa [hfiber] using point.2
        cases hpoint
    | cons root rest =>
        simp [chainFiberRootState, GoertzelLemma814.listGetD, hfiber]
  let rootPoint : ChainFiberPoint word key :=
    ⟨chainFiberRootState word key, hRootMem⟩
  let nodes : List (ChainFiberPoint word key) :=
    (concreteChainFiber word key).attach.map fun row =>
      (⟨row.1, row.2⟩ : ChainFiberPoint word key)
  have hCovers : ∀ x : ChainFiberPoint word key, x ∈ nodes := by
    intro x
    unfold nodes
    refine List.mem_map.mpr ?_
    refine ⟨⟨x.1, x.2⟩, List.mem_attach _ _, ?_⟩
    exact Subtype.ext rfl
  have hRootClosure :
      chainFiberRootState word key ∈ GoertzelLemma814.closeChainFiber
        (frontierWordToChainWord word)
        (concreteChainFiber word key)
        (concreteChainFiber word key).length
        [chainFiberRootState word key] :=
    GoertzelLemma814.closeChainFiber_mem_of_seen
      ...
      (by simp)
  have hStepToRoot :
      ∀ x : ChainFiberPoint word key,
        chainFiberRootClosureStep word key x rootPoint := by
    intro _x
    simpa [chainFiberRootClosureStep, rootPoint] using hRootClosure
  exact
    ⟨totalDecidableEq,
      ⟨rootStarParentRowsSymmetricRootedConnectedCertificate
        nodes rootPoint (hCovers rootPoint) hCovers
        (chainFiberRootClosureStep_symmetric_of_connected hconn)
        hStepToRoot⟩⟩
...
theorem closeChainComponent_add_eq_close_close
    (orients : List GoertzelLemma814.TauOrient)
    (states : List GoertzelLemma814.TauState)
    (a c : GoertzelLemma814.LColor)
    (m n : Nat)
    (seen : List GoertzelLemma814.ChainEdge) :
    GoertzelLemma814.closeChainComponent orients states a c (m + n) seen =
      GoertzelLemma814.closeChainComponent orients states a c n
        (GoertzelLemma814.closeChainComponent orients states a c m seen) := by
  induction m generalizing seen with
  | zero =>
      simp [GoertzelLemma814.closeChainComponent]
  | succ m ih =>
      rw [Nat.succ_add]
      simp only [GoertzelLemma814.closeChainComponent]
      exact ih (GoertzelLemma814.appendFresh seen
        (GoertzelLemma814.nextChainComponentLayer orients states a c seen))

theorem closeChainComponent_eq_self_of_nextLayer_empty
    (orients : List GoertzelLemma814.TauOrient)
    (states : List GoertzelLemma814.TauState)
    (a c : GoertzelLemma814.LColor)
    (seen : List GoertzelLemma814.ChainEdge)
    (hnext :
      GoertzelLemma814.nextChainComponentLayer orients states a c seen = []) :
    ∀ n,
      GoertzelLemma814.closeChainComponent orients states a c n seen =
        seen := by
  intro n
  induction n with
  | zero =>
      simp [GoertzelLemma814.closeChainComponent]
  | succ _ ih =>
      simp [GoertzelLemma814.closeChainComponent, hnext,
        GoertzelLemma814.appendFresh]
      exact ih

theorem concreteChainFiberAppendLocalSingletonComponent_shift_close_eq_aux
    (word : List GoertzelLemma818FrontierMode.TauOrient)
    (orient : GoertzelLemma818FrontierMode.TauOrient)
    (pref target : List GoertzelLemma814.TauState)
    (lastX currentLast : GoertzelLemma814.TauState)
    ...
    ∀ n seen,
      ...
      GoertzelLemma814.closeChainComponent
          (frontierWordToChainWord (word ++ [orient]))
          (pref ++ [currentLast]) move.a move.c n
          (concreteChainFiberAppendShiftComponent word seen) =
        concreteChainFiberAppendShiftComponent word
          (GoertzelLemma814.closeChainComponent
            (frontierWordToChainWord [orient]) [currentLast]
            move.a move.c n seen) := by
  let localOrients := frontierWordToChainWord [orient]
  let globalOrients := frontierWordToChainWord (word ++ [orient])
  intro n
  induction n with
  ...
  | succ n ih =>
      ...
      have hnext :
          GoertzelLemma814.nextChainComponentLayer globalOrients
              (pref ++ [currentLast]) move.a move.c
              (concreteChainFiberAppendShiftComponent word seen) =
            concreteChainFiberAppendShiftComponent word localLayer := by
        simpa [localOrients, globalOrients, localLayer] using
          concreteChainFiberAppendLocalSingletonComponent_shift_nextLayer_eq
            word orient pref target lastX currentLast move seen hne hprefLen
            hcompatibleX htrace hseed hspecified hseen
      have hseenNext :
          ∀ edge, edge ∈ GoertzelLemma814.appendFresh seen localLayer →
            edge ∈ GoertzelLemma814.chainComponent localOrients [currentLast]
              move.a move.c move.seed := by
        intro edge hedge
        rcases GoertzelLemma814.mem_appendFresh_source_or_mem seen localLayer
            hedge with
          hseenMem | hlayer
        · exact hseen edge hseenMem
        · exact
            concreteChainFiberAppendLocalSingletonComponent_nextLayer_mem_component
              orient currentLast move seen hseed
              (by simpa [localOrients] using hseen)
              (by simpa [localOrients, localLayer] using hlayer)
      simp only [GoertzelLemma814.closeChainComponent]
      rw [hnext]
      rw [← concreteChainFiberAppendShiftComponent_appendFresh]
      exact ih (GoertzelLemma814.appendFresh seen localLayer) hseenNext

/--
Reverse/no-ingress component equality for the singleton append lift.
...
The reverse direction shows that the appended global component cannot
...
theorem concreteChainFiberAppendRelativeSingletonShiftedComponentClosed_ok :
    concreteChainFiberAppendRelativeSingletonShiftedComponentClosed := by
  intro word orient hne _hcert key _hkey pref lastX hpref _hlastX
    ...
    _htarget hcurrentEq htargetEq move _hpair hseed hspecified
  subst current
  subst target
  let localOrients := frontierWordToChainWord [orient]
  let globalOrients := frontierWordToChainWord (word ++ [orient])
  ...
  have hprefLen : pref.length = word.length := by
    ...
    simpa [frontierWordToChainWord] using hlen
  have hcompatibleLocal : GoertzelLemma814.compatibleAdjacent
      (GoertzelLemma814.tauOrientAt
        (frontierWordToChainWord (word ++ [orient])) (word.length - 1))
      (frontierOrientToChain orient)
      (GoertzelLemma814.chainStateAt pref (word.length - 1))
      lastX = true := by
    simpa [tauOrientAt_frontierWordToChainWord_append_length word orient]
      using hcompatibleX
  have hcurrentTrace :
      concreteChainFiberAppendLastInputTrace orient currentLast =
        concreteChainFiberAppendLastInputTrace orient lastX :=
  ...
  change (!localComponent.isEmpty &&
      GoertzelLemma814.chainComponentAvoidsInputs localOrients localComponent &&
      GoertzelLemma814.chainAgreesWithSwitch localOrients [currentLast]
        [targetLast] localComponent move.a move.c) = true at hspecifiedParts
  simp only [Bool.and_eq_true] at hspecifiedParts
  rcases hspecifiedParts with ⟨⟨hLocalNonempty, _hLocalAvoid⟩, _hLocalAgree⟩
  have hlocalPair :
      GoertzelLemma814.chainEdgeInPair [currentLast] move.a move.c
    ...
    · have hpairFalse := GoertzelLemma814.bool_false_of_not_true hpair
      ...
      rw [hempty] at hLocalNonempty
      cases hLocalNonempty
  have hseedMemLocal : move.seed ∈ localComponent :=
    GoertzelLemma814.chainComponent_seed_mem_of_nonempty
      localOrients [currentLast] move
      (by simpa [localOrients, localComponent] using hLocalNonempty)
  have hseenSeed :
      ∀ edge, edge ∈ [move.seed] → edge ∈ localComponent := by
    intro edge hedge
    simp at hedge
    subst edge
    exact hseedMemLocal
  have hcloseGlobalEq :=
    concreteChainFiberAppendLocalSingletonComponent_shift_close_eq_aux
      word orient pref [targetLast] lastX currentLast move hne hprefLen
      hcompatibleLocal hcurrentTrace hseed hspecified globalLen [move.seed]
      hseenSeed
  have hseedOcc : move.seed.occ = 0 :=
    GoertzelLemma814.chainEdges_singleton_seed_occ_zero
      (frontierOrientToChain orient) move.seed
  ...
  have hglobalPair :
    ...
    simpa [concreteChainFiberAppendShiftMove, concreteChainFiberAppendShiftEdge]
      using hpairEq.trans hlocalPair
  have hlocalComponentEq :
      localComponent =
        GoertzelLemma814.closeChainComponent localOrients [currentLast]
          move.a move.c localLen [move.seed] := by
    simp [localComponent, GoertzelLemma814.chainComponent, localLen,
      localOrients, hlocalPair]
  have hnextEmpty :
      GoertzelLemma814.nextChainComponentLayer localOrients [currentLast]
        move.a move.c localComponent = [] := by
    apply List.eq_nil_iff_forall_not_mem.2
    intro edge hedge
    exact GoertzelLemma814.chainComponent_nextLayer_empty
      localOrients [currentLast] move.a move.c move.seed
      (List.contains_iff_mem.mp (by simpa [localOrients] using hseed))
      (by simpa [localOrients, localComponent] using hedge)
  have hlenLeRaw :
      (GoertzelLemma814.chainEdges (frontierWordToChainWord [orient])).length ≤
  ...
  have hlenLe : localLen ≤ globalLen := by
    simpa [localLen, globalLen, localOrients, globalOrients] using hlenLeRaw
  have hlocalCloseGlobal :
      GoertzelLemma814.closeChainComponent localOrients [currentLast]
          move.a move.c globalLen [move.seed] = localComponent := by
    have hnat : localLen + (globalLen - localLen) = globalLen :=
      Nat.add_sub_of_le hlenLe
    calc
      GoertzelLemma814.closeChainComponent localOrients [currentLast]
          move.a move.c globalLen [move.seed]
          = GoertzelLemma814.closeChainComponent localOrients [currentLast]
              move.a move.c (localLen + (globalLen - localLen)) [move.seed] := by
              rw [hnat]
      _ = GoertzelLemma814.closeChainComponent localOrients [currentLast]
              move.a move.c (globalLen - localLen)
              (GoertzelLemma814.closeChainComponent localOrients [currentLast]
                move.a move.c localLen [move.seed]) := by
              rw [closeChainComponent_add_eq_close_close]
      _ = GoertzelLemma814.closeChainComponent localOrients [currentLast]
              move.a move.c (globalLen - localLen) localComponent := by
              rw [← hlocalComponentEq]
      _ = localComponent :=
              closeChainComponent_eq_self_of_nextLayer_empty
                localOrients [currentLast] move.a move.c localComponent
                hnextEmpty (globalLen - localLen)
  have hshiftSeed :
      concreteChainFiberAppendShiftEdge word move.seed =
        (concreteChainFiberAppendShiftMove word move).seed := by
    simp [concreteChainFiberAppendShiftEdge,
      concreteChainFiberAppendShiftMove]
  have hcloseGlobalShiftSeed :
      GoertzelLemma814.closeChainComponent globalOrients (pref ++ [currentLast])
          move.a move.c globalLen
          [(concreteChainFiberAppendShiftMove word move).seed] =
        concreteChainFiberAppendShiftComponent word
          (GoertzelLemma814.closeChainComponent localOrients [currentLast]
            move.a move.c globalLen [move.seed]) := by
    simpa [globalOrients, localOrients,
      concreteChainFiberAppendShiftComponent, hshiftSeed] using hcloseGlobalEq
  calc
    GoertzelLemma814.chainComponent globalOrients (pref ++ [currentLast])
        move.a move.c (concreteChainFiberAppendShiftMove word move).seed
        = GoertzelLemma814.closeChainComponent globalOrients
            (pref ++ [currentLast]) move.a move.c globalLen
            [(concreteChainFiberAppendShiftMove word move).seed] := by
            simp [GoertzelLemma814.chainComponent, globalLen, globalOrients,
              hglobalPair]
    _ = concreteChainFiberAppendShiftComponent word
          (GoertzelLemma814.closeChainComponent localOrients [currentLast]
            move.a move.c globalLen [move.seed]) := hcloseGlobalShiftSeed
    _ = concreteChainFiberAppendShiftComponent word localComponent := by
          rw [hlocalCloseGlobal]
    _ = concreteChainFiberAppendShiftComponent word
          (GoertzelLemma814.chainComponent localOrients [currentLast]
            move.a move.c move.seed) := rfl
...
/--
Superseded as the live Gate-2 remainder by
`concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixCertifiedParentRowsRemainderClosed`.

This legacy sufficient hypothesis asks for structural fields for every
supplied parent-row base-data record.  The current route only needs the paired
base-data/structural witness, and the certified-parent-rows remainder below
removes every triple whose appended root-closure and projection selector have
...
def concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed :
    Prop :=
  ∀ (word : List GoertzelLemma818FrontierMode.TauOrient)
    (orient : GoertzelLemma818FrontierMode.TauOrient),
    1 < word.length →
...
def concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSectionClosed :
    Prop :=
  ∀ (word : List GoertzelLemma818FrontierMode.TauOrient)
    (orient : GoertzelLemma818FrontierMode.TauOrient),
    1 < word.length →
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSectionClosed_of_projection_surjective
    (hSurjective :
  ...
  intro word orient hlen hcert key hkey root rest hfiber
  rcases hSurjective word orient hlen hcert key hkey root rest hfiber with
    ⟨proj, hproj⟩
  exact
    ⟨proj, fun b => Classical.choose (hproj b),
      fun b => Classical.choose_spec (hproj b)⟩

theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed_of_projection_section
    (hSection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSectionClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed := by
  intro word orient hlen hcert key hkey root rest hfiber
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed_of_projection_surjective
    (hSurjective :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed := by
  intro word orient hlen hcert key hkey root rest hfiber
  rcases hSurjective word orient hlen hcert key hkey root rest hfiber with
    ⟨proj, hproj⟩
  ...
  have hprefixRows : ∀ b : ChainFiberPoint word key, b ∈ prefixRows := by
    ...
    · apply Subtype.ext
      rfl
  let selected : ChainFiberPoint word key →
      ChainFiberPoint (word ++ [orient]) key :=
    fun b => Classical.choose (hproj b)
  let preimageRows : List (ChainFiberPoint (word ++ [orient]) key) :=
    prefixRows.map selected
  have hselectedMem :
      ∀ b : ChainFiberPoint word key,
        selected b ∈ preimageRows := by
    intro b
    exact List.mem_map.mpr ⟨b, hprefixRows b, rfl⟩
  let rowOf : ChainFiberPoint word key → Fin preimageRows.length :=
    fun b => Classical.choose (List.get_of_mem (hselectedMem b))
  refine ⟨proj, preimageRows, rowOf, ?_⟩
  intro b
  have hget :
      preimageRows.get (rowOf b) = selected b :=
    Classical.choose_spec (List.get_of_mem (hselectedMem b))
  rw [hget]
  exact Classical.choose_spec (hproj b)

theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed_of_projection_section
    (hSection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSectionClosed) :
  ...
  concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed_of_projection_surjective
    (concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed_of_projection_section
      hSection)

theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsConnectedClosed_of_appended_connected_and_projection_finite_section
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
    (hSection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsConnectedClosed := by
  intro word orient hlen hcert key hkey root rest hfiber
  rcases hSection word orient hlen hcert key hkey root rest hfiber with
    ⟨_proj, preimageRows, rowOf, _hrow⟩
  let prefixRoot : ChainFiberPoint word key :=
    ⟨root, by simp [hfiber]⟩
  let appendedWitness : ChainFiberPoint (word ++ [orient]) key :=
    preimageRows.get (rowOf prefixRoot)
  exact
    chainFiberRootClosureParentRowsSymmetricRootedConnectedCertificate_of_connected_nonempty
      (hConnected word orient hlen hcert key hkey root rest hfiber)
      ⟨appendedWitness⟩

theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsConnectedClosed_of_appended_connected_and_projection_section
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
    (hSection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSectionClosed) :
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsConnectedClosed_of_appended_connected_and_projection_surjective
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
    (hSurjective :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed) :
...
def concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsProjectionFiniteSectionClosed :
    Prop :=
  ∀ (word : List GoertzelLemma818FrontierMode.TauOrient)
    (orient : GoertzelLemma818FrontierMode.TauOrient),
    1 < word.length →
...
def representativeTargetAppendParentRowsProjectionFiniteSectionClosed : Prop :=
  ∀ target : RepresentativeSemanticTarget,
    Nonempty
...
theorem representativeTargetAppendParentRowsProjectionFiniteSectionClosed_of_target_cases
    {cases : List AppendedParentRowsProjectionFiniteSectionTargetCase}
    (hcover : AppendedParentRowsProjectionFiniteSectionTargetCasesCover cases) :
    representativeTargetAppendParentRowsProjectionFiniteSectionClosed := by
  intro target hcert key hkey root rest hfiber
  exact
    appendedParentRowsProjectionFiniteSectionBody_of_target_cases
      hcover
      (targetFrontierWord_eq_targetAppendPrefix_append target)
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsConnectedClosed_of_appended_parent_rows_projection_finite_section
    ...
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsConnectedClosed := by
  intro word orient hlen hcert key hkey root rest hfiber
  exact (hCombined word orient hlen hcert key hkey root rest hfiber).1

theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed_of_appended_parent_rows_projection_finite_section
    (hCombined :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsProjectionFiniteSectionClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed := by
  intro word orient hlen hcert key hkey root rest hfiber
  exact (hCombined word orient hlen hcert key hkey root rest hfiber).2

theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsProjectionFiniteSectionClosed_of_appended_parent_rows_connected_and_projection_surjective
    (hRows :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsConnectedClosed)
    (hSurjective :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed) :
  ...
  intro word orient hlen hcert key hkey root rest hfiber
  exact
    ⟨hRows word orient hlen hcert key hkey root rest hfiber,
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed_of_projection_surjective
        hSurjective word orient hlen hcert key hkey root rest hfiber⟩

theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsProjectionFiniteSectionClosed_of_appended_parent_rows_connected_and_projection_section
    (hRows :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsConnectedClosed)
    (hSection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSectionClosed) :
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsProjectionFiniteSectionClosed_of_appended_connected_and_projection_finite_section
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
    (hSection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed) :
  ...
  exact
    ⟨concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsConnectedClosed_of_appended_connected_and_projection_finite_section
        hConnected hSection word orient hlen hcert key hkey root rest hfiber,
      hSection word orient hlen hcert key hkey root rest hfiber⟩

theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsProjectionFiniteSectionClosed_of_appended_connected_and_projection_section
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
    (hSection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSectionClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsProjectionFiniteSectionClosed :=
  concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsProjectionFiniteSectionClosed_of_appended_connected_and_projection_finite_section
    hConnected
    (concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed_of_projection_section
      hSection)
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed_of_projection_finite_section
    (hSection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed := by
  intro word orient hlen hcert key hkey root rest hfiber
  rcases hSection word orient hlen hcert key hkey root rest hfiber with
    ⟨proj, preimageRows, rowOf, hrow⟩
  exact ⟨proj, fun b => ⟨preimageRows.get (rowOf b), hrow b⟩⟩

theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed_of_appended_connected_and_projection_surjective
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
    (hSurjective :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed := by
  intro word orient hlen hcert key hkey root rest hfiber
  rcases hSurjective word orient hlen hcert key hkey root rest hfiber with
    ⟨proj, hproj⟩
  exact
    ⟨proj, hConnected word orient hlen hcert key hkey root rest hfiber,
      hproj⟩

theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed_of_appended_connected_and_projection_finite_section
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
    (hSection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed :=
  concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed_of_appended_connected_and_projection_surjective
    hConnected
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed_of_appended_parent_rows_projection_finite_section
    (hCombined :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsProjectionFiniteSectionClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed :=
  concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed_of_appended_parent_rows_connected_and_projection_finite_section
    (concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsConnectedClosed_of_appended_parent_rows_projection_finite_section
      hCombined)
    (concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed_of_appended_parent_rows_projection_finite_section
      hCombined)
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed_of_surjective_projection
  ...
  rcases hProjection word orient hlen hcert key hkey root rest hfiber with
    ⟨_proj, hConnected, _hSurjective⟩
  exact hConnected

theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed_of_surjective_projection
    (hProjection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed := by
  intro word orient hlen hcert key hkey root rest hfiber
  rcases hProjection word orient hlen hcert key hkey root rest hfiber with
    ⟨proj, _hConnected, hSurjective⟩
  exact ⟨proj, hSurjective⟩

theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed_iff_appended_connected_and_projection_surjective :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed ↔
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed ∧
        concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed :=
  ⟨fun hProjection =>
      ⟨concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed_of_surjective_projection
          hProjection,
        concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed_of_surjective_projection
          hProjection⟩,
    fun hSplit =>
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed_of_appended_connected_and_projection_surjective
        hSplit.1 hSplit.2⟩
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixConnectedLiftImageClosed_of_appended_connected_and_projection_surjective
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
    (hSurjective :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed) :
  ...
  concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixConnectedLiftImageClosed_of_surjective_projection
    (concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed_of_appended_connected_and_projection_surjective
      hConnected hSurjective)

theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixConnectedLiftImageClosed_of_appended_connected_and_projection_finite_section
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
    (hSection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed) :
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixConnectedLiftImageClosed_of_appended_parent_rows_projection_finite_section
    (hCombined :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsProjectionFiniteSectionClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixConnectedLiftImageClosed :=
  concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixConnectedLiftImageClosed_of_surjective_projection
    (concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed_of_appended_parent_rows_projection_finite_section
      hCombined)
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixBaseDataStructuralClosed_of_surjective_projection
    (hProjection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixBaseDataStructuralClosed :=
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixBaseDataStructuralClosed_of_appended_connected_and_projection_surjective
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
    (hSurjective :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionSurjectiveClosed) :
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixBaseDataStructuralClosed_of_appended_connected_and_projection_finite_section
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
    (hSection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixProjectionFiniteSectionClosed) :
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixBaseDataStructuralClosed_of_appended_parent_rows_projection_finite_section
    (hCombined :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsProjectionFiniteSectionClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixBaseDataStructuralClosed :=
  concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixBaseDataStructuralClosed_of_surjective_projection
...
structure ChainFiberAppendCertifiedParentRowsTriple
    (word : List GoertzelLemma818FrontierMode.TauOrient)
    (orient : GoertzelLemma818FrontierMode.TauOrient)
    (key : List GoertzelLemma814.LColor) : Prop where
  lengthGtOne : 1 < word.length
  prefixCertificate : Nonempty (ChainWordConcreteFibrationCertificate word)
  keyMem : key ∈ GoertzelLemma814.colorAssignments4
  appendedConnected :
...
theorem ChainFiberAppendCertifiedParentRowsTriple.baseDataStructural
    {word : List GoertzelLemma818FrontierMode.TauOrient}
    {orient : GoertzelLemma818FrontierMode.TauOrient}
    {key : List GoertzelLemma814.LColor}
    (cert : ChainFiberAppendCertifiedParentRowsTriple word orient key) :
    Nonempty
      (Sigma (fun data :
        ChainFiberAppendQuotientFibrationParentRowsBaseData
          word orient key =>
  ...
  | nil =>
      rcases
          concreteChainFiberAppendQuotientFibrationParentRowsConnectedLiftImageWitness_of_empty_prefix_fiber
            (word := word) (orient := orient) (key := key) hne hfiber with
        ⟨data, proj, hTotal, hImage⟩
      exact ⟨⟨data,
        ChainFiberAppendQuotientFibrationParentRowsStructuralFields.ofTotalConnectedAndLiftImage
          proj hTotal hImage⟩⟩
  | cons root rest =>
      ...
      rcases
          concreteChainFiberAppendQuotientFibrationParentRowsConnectedLiftImageWitness_of_prefix_root_star_surjective
            (word := word) (orient := orient) (key := key)
            cert.prefixCertificate cert.keyMem hfiber
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixBaseDataStructuralClosed_of_certified_parent_rows_remainder
    (hRemainder :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixCertifiedParentRowsRemainderClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixBaseDataStructuralClosed := by
  intro word orient hlen hcert key hkey
  by_cases hTriple :
      ChainFiberAppendCertifiedParentRowsTriple word orient key
  · exact hTriple.baseDataStructural
  · exact hRemainder word orient hlen hcert key hkey hTriple
...
theorem concreteChainFiberAppendQuotientFibrationParentRowsFieldsClosed_of_non_singleton_certified_parent_rows_remainder
    (hRemainder :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixCertifiedParentRowsRemainderClosed) :
    concreteChainFiberAppendQuotientFibrationParentRowsFieldsClosed :=
  concreteChainFiberAppendQuotientFibrationParentRowsFieldsClosed_of_non_singleton_base_data_structural
    (concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixBaseDataStructuralClosed_of_certified_parent_rows_remainder
      hRemainder)
...
theorem concreteChainFiberFibrationNonemptyTransferClosed_of_append_quotient_parent_rows_certified_parent_rows_remainder
    (hRemainder :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixCertifiedParentRowsRemainderClosed) :
    concreteChainFiberFibrationNonemptyTransferClosed :=
  concreteChainFiberFibrationNonemptyTransferClosed_of_append_quotient_parent_rows
...
theorem concreteChainAuditFibrationNonemptyTransferClosed_of_append_quotient_parent_rows_certified_parent_rows_remainder
    (hRemainder :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixCertifiedParentRowsRemainderClosed) :
    concreteChainAuditFibrationNonemptyTransferClosed :=
  concreteChainAuditFibrationNonemptyTransferClosed_of_append_quotient_parent_rows
...
theorem chainAuditForFrontierWord_ok_of_append_quotient_parent_rows_certified_parent_rows_remainder
    (hSeed : concreteChainWordFibrationSingletonSeeds)
    (hRemainder :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixCertifiedParentRowsRemainderClosed)
    (word : List GoertzelLemma818FrontierMode.TauOrient) :
...
theorem semanticFrontierStateSufficientForChain_of_append_quotient_parent_rows_certified_parent_rows_remainder
    {targetAudit : RepresentativeSemanticTarget → Bool}
    (hRemainder :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixCertifiedParentRowsRemainderClosed) :
    semanticFrontierStateSufficientForChain targetAudit :=
...
theorem semanticFrontierStateSufficientForChain_of_append_quotient_parent_rows_surjective_projection
    {targetAudit : RepresentativeSemanticTarget → Bool}
    (hProjection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed) :
    semanticFrontierStateSufficientForChain targetAudit :=
...
theorem semanticFrontierStateSufficientForChain_of_append_quotient_parent_rows_appended_connected_and_projection_surjective
    {targetAudit : RepresentativeSemanticTarget → Bool}
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
    (hSurjective :
...
theorem semanticFrontierStateSufficientForChain_of_append_quotient_parent_rows_appended_connected_and_projection_finite_section
    {targetAudit : RepresentativeSemanticTarget → Bool}
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
    (hSection :
...
theorem semanticFrontierStateSufficientForChain_of_append_quotient_parent_rows_appended_parent_rows_projection_finite_section
    {targetAudit : RepresentativeSemanticTarget → Bool}
    (hCombined :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsProjectionFiniteSectionClosed) :
    semanticFrontierStateSufficientForChain targetAudit :=
...
theorem chainAuditForFrontierWord_ok_of_targets_and_append_quotient_parent_rows_certified_parent_rows_remainder
    {targetAudit : RepresentativeSemanticTarget → Bool}
    (hTarget : ∀ t, targetAudit t = true)
    (hRemainder :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixCertifiedParentRowsRemainderClosed)
...
theorem chainAuditForFrontierWord_ok_of_targets_and_append_quotient_parent_rows_surjective_projection
    {targetAudit : RepresentativeSemanticTarget → Bool}
    (hTarget : ∀ t, targetAudit t = true)
    (hProjection :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixSurjectiveProjectionClosed)
...
theorem chainAuditForFrontierWord_ok_of_targets_and_append_quotient_parent_rows_appended_connected_and_projection_surjective
    {targetAudit : RepresentativeSemanticTarget → Bool}
    (hTarget : ∀ t, targetAudit t = true)
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
  ...
  chainAuditForFrontierWord_ok_of_targets_and_frontier_state_sufficiency
    hTarget
    (semanticFrontierStateSufficientForChain_of_append_quotient_parent_rows_appended_connected_and_projection_surjective
      hConnected hSurjective)
    word

theorem chainAuditForFrontierWord_ok_of_targets_and_append_quotient_parent_rows_appended_connected_and_projection_finite_section
    {targetAudit : RepresentativeSemanticTarget → Bool}
    (hTarget : ∀ t, targetAudit t = true)
    (hConnected :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedConnectedClosed)
...
theorem chainAuditForFrontierWord_ok_of_targets_and_append_quotient_parent_rows_appended_parent_rows_projection_finite_section
    {targetAudit : RepresentativeSemanticTarget → Bool}
    (hTarget : ∀ t, targetAudit t = true)
    (hCombined :
      concreteChainFiberAppendQuotientFibrationParentRowsNonSingletonPrefixAppendedParentRowsProjectionFiniteSectionClosed)
  ...
  chainAuditForFrontierWord_ok_of_targets_and_frontier_state_sufficiency
    hTarget
    (semanticFrontierStateSufficientForChain_of_append_quotient_parent_rows_appended_parent_rows_projection_finite_section
      hCombined)
    word
