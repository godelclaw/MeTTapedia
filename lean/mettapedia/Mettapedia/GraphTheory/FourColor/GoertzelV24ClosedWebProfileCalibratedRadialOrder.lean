...
`6164740`, and use `closedWebCutProfileCount widthBound` as the requested
laminar depth.  Thus a closed web with more than `12329480` interior vertices
constructs an actual radial witness having one of the two source coordinates:

* a block of `6164740` consecutive radial cuts, each with actual chord load at
...
theorem profileCalibratedBlockLength_eq :
    profileCalibratedBlockLength = 6164740 := by
...
theorem separatedProfileRadialVertexThreshold_eq :
    separatedProfileRadialVertexThreshold = 12329480 := by
