...
structure BoundedSerialBoundaryRebaseLetterCode where
  ...
  tracked : BoundedCarrierGraphFamilyCode 14 2
...
noncomputable def sourceCorridorSerialBoundaryRebaseLetterCodeAt
  ...
  tracked := sourceCorridorSerialBoundaryRebaseTrackedCodeAt realization
...
/-- The two named successor crossings remain distinct in the bundled tracked
carrier. -/
theorem sourceCorridorSerialBoundaryRebaseLetterCodeAt_point_injective
    {source : SourceTrail G}
    {embedded : source.AnnularEmbedding} {blockLength : Nat}
    (realization : BoundaryCleanCorridorRealization embedded blockLength)
    (hcubic : embedded.cellulation.rotation.toRotationSystem.IsCubic)
    ...
    Function.Injective
      (sourceCorridorSerialBoundaryRebaseLetterCodeAt realization hcubic
        hrotation htwoSided hunique offset hnext color hcolor).tracked.point := by
  exact sourceCorridorSerialBoundaryRebaseTrackedCodeAt_point_injective
