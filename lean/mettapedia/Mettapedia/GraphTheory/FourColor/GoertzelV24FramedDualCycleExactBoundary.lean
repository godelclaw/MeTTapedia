...
/-- The connected-side conclusion holds for a prescribed deletion component,
not merely for an existentially selected one.  This oriented form is needed
when source geometry fixes a distinguished root (for example, a container
cycle root) and the retained side must be chosen relative to it. -/
theorem component_exactBoundary_connectedSides_of_dualCycle
    (data : Data G)
    (htwoSided : OrbitFacesTwoSided data.toRotationSystem)
    (hdual : (interiorDualGraph
      (orbitFaceBoundary data.toRotationSystem)
  ...
  have hsaturation
    ...
    have hnonempty :
      ...
      · simpa [heq] using hanchorNonempty
      · exact
          GoertzelV24FiniteDeletionCyclicCut.componentCrossingEdges_nonempty_of_distinct
            hconnected removed other anchor heq
    have hsaturated :=
      crossingEdgeFinset_eq_dualWalkCrossingEdges_of_isCycle_of_subset
        data htwoSided hunique walk hcycle
          (fun vertex => vertex ∈ other.supp) hsubset hnonempty
  ...
  have hremovedNonempty : removed.Nonempty := by
    rw [← hboundary]
    by_cases heq : component = anchor
    · simpa [heq] using hanchorNonempty
    · exact
        GoertzelV24FiniteDeletionCyclicCut.componentCrossingEdges_nonempty_of_distinct
          hconnected removed component anchor heq
  exact ⟨hboundary,
    GoertzelV24FiniteDeletionCyclicCut.connected_induce_component
      removed component,
    GoertzelV24FiniteDeletionCyclicCut.induce_complement_connected_of_component_boundary_saturation
