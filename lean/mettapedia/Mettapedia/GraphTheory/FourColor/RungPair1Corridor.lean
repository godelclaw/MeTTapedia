...
import Mettapedia.GraphTheory.FourColor.RungPair1Shape

/-! No long `RungPair1` chain in a least counterexample, from the tube-free lemma. -/
...
/-- **No long `RungPair1` chain in a least counterexample.** -/
theorem le_of_tube (minimal : GraphBackedVertexMinimalTaitCounterexample rotation)
    ...
    n + 1 ≤ Nat.factorial 6 * (Nat.factorial 6 * (6 * 6 + 1)) * 2 ^ (4 ^ 6) :=
