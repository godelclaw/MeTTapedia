...
/-! # Supplied spherical reduction preserving all frozen prescriptions

The contour/cotree alternative supplies enough marked cuts for the refined
joint state. The selected seam is structural, strictly smaller, and preserves
each named-dart prescription. Ordinary zero Count is not a premise. The
ordered-mesh branch is left explicit, not silently ruled out.
-/

namespace Mettapedia.GraphTheory.FourColor.SphericalFrozenReduction
...
open MarkedCotreeSeam FrozenSeamState
...
theorem exists_joint_seam_of_nodes (rotation : Data G)
    (ambient : BridgelessSphericalCubicMapData rotation.toRotationSystem)
    (w : ℕ) (marks : Finset V) (mark : M → G.Dart)
    (nodes : Fin (FrozenSeamState.stateBound w (Fintype.card M) + 1) →
    ...
    ∃ pair : SeamPair rotation marks mark w, pair.JointEq := by
  let n := FrozenSeamState.stateBound w (Fintype.card M)
  let literal := fun i => (nodes i).toLiteralOfSpherical rotation ambient
  let state := fun i => FrozenSeamState.boundedOfShore rotation (literal i).shore
    (literal i).innerOuter mark w (literal i).widthBound
  have hcard : Fintype.card (FrozenSeamState.Bounded.{u} M w) < Fintype.card (Fin (n + 1)) := by
    rw [Fintype.card_fin]
    exact lt_of_le_of_lt (FrozenSeamState.card_bounded_le (M := M) w) (Nat.lt_succ_self n)
  ...
  obtain ⟨width, outerWidth, innerWidth, hs⟩ := FrozenSeamState.boundedOfShore_eq_elim
  ...
  let pair : SeamPair rotation marks mark w := {
    ...
    state_eq := congrArg Prod.fst hs }
  exact ⟨pair, congrArg Prod.snd hs⟩

def contourLength (a b vertices darts : ℕ) : ℕ :=
  (6 * SphericalMeshFreeSeam.contourWidth a b + 1) *
    ((vertices + 1) * FrozenSeamState.stateBound (SphericalMeshFreeSeam.contourWidth a b) darts) + 1
...
def sizeBound (a b vertices darts : ℕ) : ℕ :=
  markedCotreeVertexBound (radiusBound a b vertices darts) vertices
    (FrozenSeamState.stateBound (2 * radiusBound a b vertices darts + 1) darts)

/-- Both geometric branches construct joint-compatible cuts. No chain,
radius bound, prescription equivalence or matching is supplied. -/
theorem exists_joint_seam_of_no_orderedMesh (rotation : Data G)
    (ambient : BridgelessSphericalCubicMapData rotation.toRotationSystem)
  ...
  let n := FrozenSeamState.stateBound c (Fintype.card M)
  ...
  · obtain ⟨nodes, hnest, hm, hstar⟩ := exists_marked_nodes_of_large_card rotation ambient htwo
      root h (FrozenSeamState.stateBound (2 * h + 1) (Fintype.card M)) marks hradius hlarge
    exact ⟨2 * h + 1, le_max_right _ _,
      exists_joint_seam_of_nodes rotation ambient _ marks mark nodes hnest hm hstar⟩
  · obtain ⟨far, hfar⟩ := not_forall.mp hradius
    ...
    exact ⟨c, le_max_left _ _, exists_joint_seam_of_nodes rotation ambient c marks mark nodes hnest hm hstar⟩

/-- Every sufficiently large ambient map either contains the actual mesh,
or has a strictly smaller structural map accepting exactly the same named
colour prescriptions. The marked vertex set is computed from the names. -/
theorem exists_orderedMesh_or_frozen_replacement (rotation : Data G)
    (ambient : BridgelessSphericalCubicMapData rotation.toRotationSystem)
    (htwo : OrbitFacesTwoSided rotation.toRotationSystem)
  ...
  · exact Or.inl hm
  · obtain ⟨w, hw, pair, hjoint⟩ := exists_joint_seam_of_no_orderedMesh rotation ambient htwo a b ha
      (Finset.univ.image (fun m => (mark m).fst)) mark hm hlarge
    ...
    exact Or.inr ⟨w, hw, pair, ⟨pair.structuralData ambient htwo⟩, pair.candidate_card_lt ambient,
      pair.liftedMark hmarks, pair.liftedMark_name hmarks, pair.accepts_iff hjoint hmarks⟩

end
end Mettapedia.GraphTheory.FourColor.SphericalFrozenReduction
