...
Algebraic-F2 table delta: the latest report-level residual scheduler fork
removes the raw forced-all handoff from the promoted scheduler state.  The
executable CAP5 report now returns either a primitive portal/cycle checker gap
or the terminal processed-coverage no-evader/synthesis branch, an already
closed synthesis branch, or a promoted residual-edge obstruction.  Under
`¬ Theorem49BoundaryRootSynthesis`, Lean eliminates both success branches and
returns only the primitive checker gap or the genuine selected-boundary-zero
evader with residual-worklist membership, immutable processed-control failure,
the one-edge promoted control update, and strict residual decrease.  Current
Algebraic-F2 estimate is PLN STV `<0.88,0.95>`, ITV `[0.84,0.89]`, progress
`99%`.
...
- Projected-generator edge-40/color-1M lab rerun archived locally as
  ...
  Algebraic-F2 estimate is PLN STV `<0.88,0.95>`, ITV `[0.84,0.89]`,
  progress `99%`.
