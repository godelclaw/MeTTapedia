...
- Move 2: derive canonical remaining-interior trace-control from the uniform
  CAP5 shell data and close the terminal processed-coverage no-evader
  certificate, or accept the trace-control failure theorem's genuine
  vertex-Kirchhoff/path-xor evader with its concrete nonzero trace edge.
