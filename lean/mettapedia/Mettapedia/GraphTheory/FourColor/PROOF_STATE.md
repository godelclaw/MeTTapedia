...
|---|---|---:|---:|---:|
| Whole CAP5/F2 route verdict | `CAP5RouteVerdict.lean` now packages the route into the single Lean theorem `cap5F2Route_irreducibleBarrier`, plus the closed-frontier no-gap form `cap5F2Route_irreducibleBarrier_of_noGap`.  The verdict is an irreducible barrier, not a 4CT claim and not a positive-canary sufficient condition: under the standard cyclic-five finite F2 route data, Lean returns either a concrete primitive portal/cycle checker gap from the executable CAP5 report, or the closed-frontier statement that absence of the two concrete target/off-target F2 evaders is exactly equivalent to theorem-4.9 synthesis plus full selected-boundary-zero classifier control.  The unified emitted-kernel/remaining-map obstruction is now named as `CAP5F2UnifiedKernelMapEvader`; under `¬ CAP5PrimitiveCheckerGap`, Lean proves no such evader iff the route is closed, and existence of such an evader iff the closed route endpoint fails.  In that same closed-frontier branch, the CAP5/Jordan geometric separator repair is impossible by the already formalized cyclic-cut obstruction.  The remaining open crux is therefore precisely the actual-side primitive checker frontier plus the single named unified no-evader predicate; adding more scheduler, rank, or report forks would not change the route decision. | `<0.97,0.95>` | `[0.92,0.97]` | 100% verdict layer |
| Route coloring payoff | `CAP5RoutePayoff.lean` extracts the route payoff as a real coloring statement without reopening the unified-evader barrier.  Lean proves `cap5RouteClosedColoringPayoff_of_routeClosed`: a closed CAP5/F2 route certificate yields `Theorem49BoundaryRootSynthesis emb C₀` and `G.EdgeColorable 4`.  This `EdgeColorable 4` is genuine line-graph `Colorable 4`, obtained from the root `G.EdgeColoring Color`; it is not a Tait/nonzero-color theorem by itself.  The stronger route-to-Tait upgrade is named exactly as `CAP5RouteClosedRootTaitBridge cert := CAP5F2RouteClosed cert → IsTaitEdgeColoring G C₀`, and `taitColorable_of_routeClosedRootTaitBridge` consumes that bridge.  The finite pre-RREF input is now named as `CAP5FiniteNoGapRouteInput`: executable checker closure, the forced-edge classifier, and all emitted/remaining red-blue witnesses build a coherent `CAP5F2RouteCertificate`.  The concrete wheel graph already has the proven coloring canary `WheelCAP5RoutePayoff.wheelColoringCanary`; the exact finite route-closed wheel canary remains the witness obligation `WheelCAP5RoutePayoff.WheelCAP5FiniteRouteClosedCanary`, built from `CAP5FiniteRouteClosedWitness`. | `<0.89,0.93>` | `[0.83,0.94]` | no-gap input bundle green; finite route-closed canary open |
...
Route-payoff finite-input delta: `CAP5RoutePayoff.lean` now proves
...
oracle, and `CAP5FiniteRouteClosedWitness.ofNoGapRouteInput` packages it once
`CAP5F2RouteClosed` is proved.  No concrete actual-side instance or checked
unified RREF/no-evader certificate is claimed by this delta.
...
- Route payoff extraction:
  `CAP5RoutePayoff.lean` adds the closed-route coloring payoff
  `cap5RouteClosedColoringPayoff_of_routeClosed`, the exact Tait bridge
  obligation `CAP5RouteClosedRootTaitBridge`, the finite witness bundle
  `CAP5FiniteRouteClosedWitness`, and the wheel canary names
  `WheelCAP5RoutePayoff.wheelColoringCanary` and
  `WheelCAP5RoutePayoff.WheelCAP5FiniteRouteClosedCanary`.  The payoff is a
  real line-graph `EdgeColorable 4` theorem from the root `Color` edge-coloring;
  route closure still does not imply Tait colorability unless the named bridge
  proves the root coloring is nonzero on every edge.  Focused verification:
...
- Move 2: decide `CAP5F2NoUnifiedKernelMapEvader` for the actual no-gap route
  ...
  same as deciding `CAP5F2RouteClosed`; if it fails, Lean returns a genuine
