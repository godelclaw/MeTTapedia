...
`CAP5PrimitiveCheckerGap`, `CAP5F2NoTargetOffTargetEvader`, and
`CAP5F2RouteClosed`, and the top-level theorem
`cap5F2Route_irreducibleBarrier`.  The theorem is the route verdict: after the
standard cyclic-five finite F2 route data is fixed, the route is not a
goes-through theorem with a positive coloring canary.  It is reduced exactly
to the primitive checker frontier plus the target/off-target no-evader
predicate, while the CAP5/Jordan geometric separator repair is blocked in the
closed-frontier branch.  Current whole-route verdict estimate is PLN STV
