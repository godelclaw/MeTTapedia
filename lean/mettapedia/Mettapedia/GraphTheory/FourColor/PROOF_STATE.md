...
Algebraic-F2 table delta: the latest initial residual scheduler fork removes
the remaining scheduler-state handoff from the executable report.  The
processed-control invariant is now discharged at `processed = ∅`, so the CAP5
report itself starts the promoted scheduler: primitive checker gap, empty
initial worklist no-evader/synthesis, already-closed synthesis, or a promoted
first residual-edge obstruction.  Under `¬ Theorem49BoundaryRootSynthesis`,
Lean eliminates both success branches and returns only the primitive checker
gap or the genuine selected-boundary-zero first residual evader with immutable
one-edge processed-control failure, the promoted one-edge control update, and
strict residual decrease.  Current Algebraic-F2 estimate is PLN STV
`<0.89,0.95>`, ITV `[0.85,0.90]`, progress `99%`.
...
- New initial residual scheduler theorem surface:
  ...
  residual decrease.  Current Algebraic-F2 estimate is PLN STV `<0.89,0.95>`,
  ITV `[0.85,0.90]`, progress `99%`.  The lab was rerun as
...
- Projected-generator edge-40/color-1M lab rerun archived locally as
  ...
  Algebraic-F2 estimate is PLN STV `<0.89,0.95>`, ITV `[0.85,0.90]`,
  progress `99%`.
