...
`concreteChainFiberAppendQuotientFibrationPathRowsFieldsClosed` also uses
...
proved `Nodup`.  `ttFiber32PrefixStatesList_eq_concreteChainFiber` and
...
matching selected `TTT` appended fiber points.  The local selector now also has
`ttFiber32AppendProjection`, `ttFiber32AppendProjection_section`, and
`ttFiber32AppendProjection_surjective`, which explicitly package the local
right-inverse projection/section surface for the same instance.  This is still
...
`GoertzelLemma818MirrorTripleLightSlices.lean` now scales that pattern to the
...
then checks the first three 16-row chunks of block 5, and
`GoertzelLemma818MirrorTripleLargeFiber36Block5ComponentCertificate.lean`
completes block 5 with an interval-membership row checker for chunk 3.  The
large fiber now has 320 checked rows across five 64-row blocks.  These modules
recheck row moves, parent-map agreement, and parent convergence under the
actual mirror-triple word.  This is still not the full `MMM` target audit: the
large fixed-input fibers for `mode05` remain mostly open, and the finite
automaton closure/consequence theorem is still open.
...
certified, the fifth large `[b,r,b,r]` fiber certified, and the sixth large
