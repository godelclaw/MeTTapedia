...
Algebraic-F2 table delta: the latest report-closed emitted-rank fork proves
that the executable CAP5 report itself separates the remaining cases.  If the
primitive missing-evidence frontier is nonempty, Lean returns the portal/cycle
checker gap.  If the frontier is empty, cyclic five-edge-connectivity supplies
the forced-all handoff and the lab-facing finite F2 emitted-edge pairing kernel
is trivial iff theorem-4.9 synthesis plus full selected-boundary-zero
classifier control holds.  The remaining shell obligation is exactly to prove
...
- New report-closed emitted-rank theorem surface:
  ...
  `99%`.
- New report-closed trace-evader theorem surface:
  ...
  boundary-zero evader with nonzero canonical remaining-family pairing.
...
- Move 2: derive canonical remaining-interior trace-control, equivalently the
  emitted-edge kernel certificate, from the uniform CAP5 shell data and close
  the terminal processed-coverage no-evader certificate; otherwise accept the
  trace-control/emitted-kernel failure theorem's genuine
  vertex-Kirchhoff/path-xor evader with its concrete nonzero trace edge and
  canonical red/blue family pairing/map/kernel obstruction.
  Immutable processing of a selected residual edge remains formally ruled out
  unless the route promotes new certified control information.
