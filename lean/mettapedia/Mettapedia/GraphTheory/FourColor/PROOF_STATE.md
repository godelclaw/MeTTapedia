...
Algebraic-F2 table delta: the latest emitted-rank exact oracle proves that,
under forced-all, the lab-facing finite F2 emitted-edge pairing kernel is
trivial iff theorem-4.9 synthesis plus full selected-boundary-zero classifier
control holds.  The remaining shell obligation is exactly to prove that kernel
certificate from uniform exact CAP5 data, or keep the unified evader as the
obstruction.
