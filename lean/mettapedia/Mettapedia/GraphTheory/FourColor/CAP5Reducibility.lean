...
/-- CAP5-facing holds-vs-counterexample classifier.  Given portal-crossing and two-sided-cycle
data, the finite separator generator has exactly the two operational outcomes needed downstream:
either realization data for the proposed CAP5 cyclic cut, or an explicit outside crossing edge
with the one-edge portal-avoiding walk across it. -/
theorem CAP5ExceptionalAnnulusBoundaryEdgeSupportCandidate.cyclicEdgeCutRealizationData_or_oneEdge_walk_avoiding_portalBoundaryEdges_of_portal_crosses
    {V : Type*} {G : SimpleGraph V} [DecidableEq G.edgeSet]
    {boundaryEdge : Fin 5 → G.edgeSet}
    (candidate : CAP5ExceptionalAnnulusBoundaryEdgeSupportCandidate boundaryEdge)
    (side : V → Prop)
    ...
    (hinside_cycle : HasCycleOnSide G side)
    (houtside_cycle : HasCycleOnSide G (fun v => ¬ side v)) :
    Nonempty (candidate.CyclicEdgeCutRealizationData (G := G)) ∨
  ...
  · exact Or.inl
      ⟨candidate.cyclicEdgeCutRealizationData_of_cyclicSeparatorCandidate
        side hportal_crosses hinside_cycle houtside_cycle hrealizes⟩
...
/-- Portal-language constructor from the counterexample-free separator form.  This is the direct
Sublemma 6.8 target: the named separator portals cross the chosen side, no opposite-side walk can
avoid all named portals, and both sides contain cycles. -/
def CAP5ExceptionalAnnulusBoundaryEdgeSupportCandidate.cyclicEdgeCutRealizationData_of_portal_no_counterexample_walk
    {V : Type*} {G : SimpleGraph V} [DecidableEq G.edgeSet]
    {boundaryEdge : Fin 5 → G.edgeSet}
    (candidate : CAP5ExceptionalAnnulusBoundaryEdgeSupportCandidate boundaryEdge)
    (side : V → Prop)
    ...
    (houtside_cycle : HasCycleOnSide G (fun v => ¬ side v)) :
    candidate.CyclicEdgeCutRealizationData (G := G) :=
  CyclicEdgeCutRealization.of_no_walk_avoiding_edgeCut side
    (by
      intro e heSupport
      rcases (candidate.mem_edgeSupport_iff_exists_portal e).1 heSupport with
        ⟨i, hi, hboundary⟩
    ...
    (by
      intro hcounter
      rcases hcounter with ⟨u, v, p, hu, hv, havoidSupport⟩
      exact hno_counterexample ⟨u, v, p, hu, hv, by
        intro i hi hiEdges
...
planar/Jordan layer must rule out before the Sublemma 6.8 walk-separator route can close. -/
theorem CAP5ExceptionalAnnulusBoundaryEdgeSupportCandidate.exists_walk_avoiding_portalBoundaryEdges_of_noncandidate_crosses
    {V : Type*} {G : SimpleGraph V} [DecidableEq G.edgeSet]
    {boundaryEdge : Fin 5 → G.edgeSet}
    (candidate : CAP5ExceptionalAnnulusBoundaryEdgeSupportCandidate boundaryEdge)
    (side : V → Prop) {e : G.edgeSet}
    (hnot : e ∉ candidate.edgeSupport)
    (hcross : EdgeCrossesVertexSide G side e) :
    ∃ u v : V, ∃ p : G.Walk u v,
      side u ∧ ¬ side v ∧
        ∀ i : Fin 5, i ∈ candidate.portalCandidate.portalSet →
...
/-- Direct refutation of the no-outside-crossing CAP5 separator target.  Under cyclic
five-edge-connectivity, portal crossings, and cycles on both sides, the proposed CAP5 support cannot
be the whole side boundary: some non-support edge must cross the same side. -/
theorem CAP5ExceptionalAnnulusBoundaryEdgeSupportCandidate.false_of_cyclicallyFiveEdgeConnected_of_no_crossing_edge_outside_portalSupport
    {V : Type*} {G : SimpleGraph V} [DecidableEq G.edgeSet]
    {boundaryEdge : Fin 5 → G.edgeSet}
    (candidate : CAP5ExceptionalAnnulusBoundaryEdgeSupportCandidate boundaryEdge)
    (side : V → Prop)
    ...
    False :=
  hno_crossing_outside
    (candidate.exists_crossing_edge_outside_portalSupport_of_cyclicallyFiveEdgeConnected_of_portal_crosses
      side hcyclic hportal_crosses hinside_cycle houtside_cycle)

/-- Same-side realization obstruction for the CAP5 Sublemma-6.8 separator route.  The finite
candidate support cannot realize the proposed side in a cyclically five-edge-connected graph once
the portals cross that side and both sides contain cycles. -/
theorem CAP5ExceptionalAnnulusBoundaryEdgeSupportCandidate.not_cyclicEdgeCutRealizationData_on_side_of_cyclicallyFiveEdgeConnected_of_portal_crosses
    {V : Type*} {G : SimpleGraph V} [DecidableEq G.edgeSet]
    {boundaryEdge : Fin 5 → G.edgeSet}
    (candidate : CAP5ExceptionalAnnulusBoundaryEdgeSupportCandidate boundaryEdge)
    (side : V → Prop)
    ...
    ¬ ∃ realization : candidate.CyclicEdgeCutRealizationData (G := G),
      realization.side = side := by
  rcases candidate.exists_crossing_edge_outside_portalSupport_of_cyclicallyFiveEdgeConnected_of_portal_crosses
      side hcyclic hportal_crosses hinside_cycle houtside_cycle with
    ⟨e, heOutside, hcross⟩
  exact candidate.not_cyclicEdgeCutRealizationData_on_side_of_crossing_outside
    side heOutside hcross
