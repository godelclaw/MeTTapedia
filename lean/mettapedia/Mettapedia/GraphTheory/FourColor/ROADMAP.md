...
target is partially discharged on its light fibers and the first six 64-row
blocks of the first large fiber, but the remaining 128 rows of that large
...
the rows beyond 383; the block2-through-block5 fresh pass shows that 16-row chunks with
...
uncertified rows beyond 383 in the first `TTT`/`mode09` large fiber or with an
...
| `GoertzelLemma818TripleTauLargeFiber4Data.lean` / `GoertzelLemma818TripleTauLargeFiber4Block0ComponentCertificate.lean` / `GoertzelLemma818TripleTauLargeFiber4Block1ComponentCertificate.lean` / `GoertzelLemma818TripleTauLargeFiber4Block2ComponentCertificate.lean` / `GoertzelLemma818TripleTauLargeFiber4Block3ComponentCertificate.lean` / `GoertzelLemma818TripleTauLargeFiber4Block4ComponentCertificate.lean` / `GoertzelLemma818TripleTauLargeFiber4Block5ComponentCertificate.lean` | First large-fiber decomposition for `TTT`/`mode09`: records the parent map for the `[r,r,b,b]` size-512 fiber and checks the first 384 source rows by explicit component rows, parent-map agreement, and parent-depth convergence; the parent map is split into 64-entry offset matchers under the same public interface, and blocks 1 through 5 are split into four 16-row chunks, with blocks 2 through 5 using explicit bounded parent paths; the other 128 rows of this fiber, the other eleven large `TTT` fibers, and the other thirteen representative target modes remain open |
