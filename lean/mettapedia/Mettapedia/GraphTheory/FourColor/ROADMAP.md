...
the large fixed-input fibers remain open.
...
the large fixed-input fibers and automaton closure remain open.
...
| `GoertzelLemma818MirrorTripleComponentSmoke.lean` / `GoertzelLemma818MirrorTripleLightSlices.lean` | Partial semantic certificate for the `MMM`/`mode05` representative target: the smoke theorem `mmmFiber9ComponentCertificateAudit_ok` checks key `[r,b,r,r]`, and `mmmLightComponentCertificateAudit_ok` checks the full mirror-triple light surface, including 69 light fixed-input fibers, 45 empty fibers, 24 nonempty size-16 fibers, 384 light states, and reversed-key coverage; the large fixed-input fibers remain open, so this is not a full target audit and not the all-chain lift |
