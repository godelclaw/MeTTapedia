...
nonempty orientation words.  Do not resume monolithic `decide` or open-ended
...
| `GoertzelLemma818FrontierMode.lean` | Lab-facing finite profile-mode DFA table for the Lemma 8.18/8.19 repair: 20 mode constructors, profile hashes, representative words, two orientations, 40 total transitions, axiom-free `tableClosedCheck_ok`, and the abstract word-fold theorem `wordMode_inTable`; this is table closure/induction only, not yet the all-chain `LKR_in` consequence |
