...
completed third, fourth, fifth, sixth, seventh, and eighth size-512 large fibers.  The other four large `TTT`
...
set is the four still-open large `TTT`/`mode09` fibers plus
...
certificates, `mode09` has its light `TTT` fibers plus the first eight large
fixed-input fibers certified, and the remaining work is the other four large
...
`mode09`, the light-fiber audit, the eight completed large-fiber audits, and
the current exact state-count split: 4,480 certified `TTT` states and 2,048
states remaining in four named large fixed-input fibers.  This is the right
