...
checks the first three 64-row blocks of the next large fiber `[r,p,r,p]`, so the
current explicit target-surface coverage is 2,112 states with 4,416 states
...
| `GoertzelLemma818TripleTauLargeFiber20Data.lean` / `GoertzelLemma818TripleTauLargeFiber20Block0ComponentCertificate.lean` / `GoertzelLemma818TripleTauLargeFiber20Block1ComponentCertificate.lean` / `GoertzelLemma818TripleTauLargeFiber20Block2ComponentCertificate.lean` | First three 64-row blocks of the fourth large `TTT`/`mode09` fixed-input fiber `[r,p,r,p]`: records the size-512 fiber data and checks rows 0 through 191 by explicit component rows, parent-map agreement, and bounded parent paths to root `4640`; this is a block certificate, not the full fourth large fiber |
| `GoertzelLemma818TripleTauTargetProgress.lean` | Partial semantic progress aggregator for the `TTT`/`mode09` representative target: `tttPartialTargetCertificateAudit_ok` verifies the `mode09` semantic witness, the light-fiber audit, the three completed large-fiber audits, the first three 64-row `[r,p,r,p]` blocks, and the exact count split of 2,112 certified states versus 4,416 states still open in the large-fiber layer; this is not the full `TTT` audit and not the all-chain lift |
