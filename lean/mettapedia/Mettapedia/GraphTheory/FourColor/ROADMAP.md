...
completed third, fourth, fifth, sixth, and seventh size-512 large fibers.  The other five large `TTT`
...
set is the five still-open large `TTT`/`mode09` fibers plus
...
certificates, `mode09` has its light `TTT` fibers plus the first seven large
fixed-input fibers certified, and the remaining work is the other five large
...
`mode09`, the light-fiber audit, the seven completed large-fiber audits, and
the current exact state-count split: 3,968 certified `TTT` states and 2,560
states remaining in five named large fixed-input fibers.  This is the right
