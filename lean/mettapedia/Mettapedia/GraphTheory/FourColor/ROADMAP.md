...
completed third, fourth, fifth, and sixth size-512 large fibers.  The other six large `TTT`
...
set is the six still-open large `TTT`/`mode09` fibers plus
...
certificates, `mode09` has its light `TTT` fibers plus the first six large
fixed-input fibers certified, and the remaining work is the other six large
...
`mode09`, the light-fiber audit, the six completed large-fiber audits, and
the current exact state-count split: 3,456 certified `TTT` states and 3,072
states remaining in six named large fixed-input fibers.  This is the right
