...
completed third, fourth, and fifth size-512 large fibers.  The other seven large `TTT`
...
set is the eight still-open large `TTT`/`mode09` fibers plus
...
certificates, `mode09` has its light `TTT` fibers plus the first five large
fixed-input fibers certified, and the remaining work is the other seven large
...
`mode09`, the light-fiber audit, the five completed large-fiber audits, and
the current exact state-count split: 2,944 certified `TTT` states and 3,584
states remaining in seven named large fixed-input fibers.  This is the right
