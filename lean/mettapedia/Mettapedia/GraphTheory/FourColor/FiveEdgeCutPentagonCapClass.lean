import Mettapedia.GraphTheory.FourColor.FiveEdgeCutPentagonCapEuler
import Mettapedia.GraphTheory.FourColor.FiveEdgeCutPentagonCapBridge
import Mettapedia.GraphTheory.FourColor.GoertzelV24TwoEdgeCutMinimality

/-!
# The pentagon-capped shore is in the cap-stable class, and minimality colours it

With the structural ladder assembled — spherical Euler data, primal
connectivity, cyclic rotations, bridge-freeness — the pentagon-capped
shore lies in the cap-stable class.  When the deleted side has at least
six vertices the replacement is strictly smaller, so a graph-backed
vertex-minimal Tait counterexample colours it.
-/
...
/-- Six deleted vertices make the pentagon-capped carrier strictly
smaller. -/
theorem card_capVertex_lt
    (hdeleted :
      6 ≤ (Finset.univ.filter fun v => ¬ data.keep v).card) :
    Fintype.card data.CapVertex < Fintype.card V := by
  classical
  have hcap := data.card_capVertex
  have hsplit :=
    Finset.filter_card_add_filter_neg_card_eq_card
      (s := (Finset.univ : Finset V)) (p := data.keep)
...
/-- The pentagon-capped shore of a graph-backed spherical bond lies in the
...
def capBridgelessSphericalCubicMapData
    ...
    (hfree : EdgeBridgeFree rotation.toRotationSystem) :
  ...
  edgeBridgeFree :=
    capData.capRotationSystem_edgeBridgeFree hfree hretainedAmbient

/-- Vertex minimality colours the pentagon-capped shore. -/
theorem capTaitColorable_of_vertexMinimal
    ...
    (hretainedAmbient :
      (retainedAmbientPrimalGraph rotation.toRotationSystem
        capData.keep).Connected)
    (hdeleted :
      6 ≤ (Finset.univ.filter fun v => ¬ capData.keep v).card) :
  ...
  apply minimal.smallerColorable capData.capRotationSystem
    (capData.card_capVertex_lt hdeleted)
  exact capData.capBridgelessSphericalCubicMapData rotation deleted
    ...
    hdeletedConnected hkeep hretainedAmbient minimal.edgeBridgeFree
