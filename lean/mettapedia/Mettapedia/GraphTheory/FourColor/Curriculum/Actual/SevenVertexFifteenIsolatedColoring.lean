...
theorem colorable_four_of_isolated_compl_vertex_and_compl_support_cycle_covering_all_vertices
  ...
  have hcolor_a5 : color a5 = 2 := by simp [color, ha5u, h50, h51, h52, h53]
...
theorem colorable_four_of_isolated_compl_vertex_and_compl_support_cycle_components_card_three_three
  ...
  have hcover_c {x : Gᶜ.support} (hx : x ∈ c.supp) : x = c0s ∨ x = c1s ∨ x = c2s := by
    ...
    · exact Or.inl <| by
        calc
          ...
          _ = pc.getVert pc.length := by simp [hlenc]
  ...
  have hcover_d {x : Gᶜ.support} (hx : x ∈ d.supp) : x = d0s ∨ x = d1s ∨ x = d2s := by
    ...
    · exact Or.inl <| by
        calc
          ...
          _ = pd.getVert pd.length := by simp [hlend]
  ...
  have hc0mem : c0s ∈ c.supp := by
    have : c0s ∈ pc.toSubgraph.verts := by simp [c0s]
    exact hpcverts ▸ this
  ...
  have hd0mem : d0s ∈ d.supp := by
    have : d0s ∈ pd.toSubgraph.verts := by simp [d0s]
    exact hpdverts ▸ this
...
/-- An isolated six-vertex complement support that is regular of degree `2` is explicitly
`4`-colorable. -/
theorem colorable_four_of_isolated_compl_support_branch_of_card_eq_seven
...
/-- The isolated-support branch is impossible for incidence-critical non-`4`-colorable
seven-vertex graphs. -/
theorem IsIncidenceCriticalNonColorable.not_isolated_compl_support_branch_of_card_eq_seven
  ...
  exact h.not_colorable
    (colorable_four_of_isolated_compl_support_branch_of_card_eq_seven (G := G) hcard hiso)
...
theorem IsIncidenceCriticalNonColorable.fifteen_edge_compl_case_split_reduced_of_card_eq_seven
  ...
  · exact False.elim
      ((h.not_isolated_compl_support_branch_of_card_eq_seven hcard) hiso)
  · exact hrest
