...
@[simp] theorem SimpleGraph.compl_compl (G : SimpleGraph V) [DecidableRel G.Adj] : Gᶜᶜ = G := by
  ext a b
  constructor
  · intro h
...
theorem colorable_three_of_compl_path_covering_all_vertices_of_card_eq_six
  ...
  have hcolor_a5 : color a5 = 2 := by simp [color, h50, h51, h52, h53]
  ...
  have hcover_d {y : V} (hy : y ∈ d.supp) : y = b0 ∨ y = b1 ∨ y = b2 := by
    ...
    · exact Or.inl <| by
        calc
          ...
          _ = q.getVert q.length := by simp [hlenq]
