...
theorem colorable_four_of_compl_cycle_components_card_three_four
  ...
  have bucket0 {x : V} (hx : color x = 0) : x ∈ c.supp := by
    ...
    · rcases hcover x with hxc' | rfl | rfl | rfl | rfl
      · contradiction
      · simp [hcolor_d0] at hx
      · simp [hcolor_d1] at hx
      · simp [hcolor_d2] at hx
      · simp [hcolor_d3] at hx
  have bucket1 {x : V} (hx : color x = 1) : x = d0 ∨ x = d1 := by
    rcases hcover x with hxc | rfl | rfl | rfl | rfl
    · simp [hcolor_c hxc] at hx
    · exact Or.inl rfl
    · exact Or.inr rfl
    · simp [hcolor_d2] at hx
    · simp [hcolor_d3] at hx
      ...
      · by_cases hxd23 : x = d2 ∨ x = d3
        ...
        · have hcx : color x = 3 := by
          ...
          have : False := by
            ...
            · simp [hcolor_c hxmem] at hcx
            · simp [hcolor_d0] at hcx
            · simp [hcolor_d1] at hcx
            · simp [hcolor_d2] at hcx
            · simp [hcolor_d3] at hcx
