  ...
  have hltc : c.supp.ncard < (Set.univ : Set V).ncard := by
    ...
    exact hnot
      (connected_of_connectedComponent_ncard_eq_card (G := G) c
        (by simp [hsupp, Set.ncard_univ, Nat.card_eq_fintype_card]))
...
/-- Any raw isolated-pair complement support case realizes the support as a single `5`-cycle.

Source: generic realization helper for the first exact complement branch of the live seven-vertex
frontier; criticality/cardinality/clique-freeness are used upstream to produce `hcase`. -/
theorem exists_distinct_two_compl_degree_eq_zero_and_compl_support_cycle_of_support_case
    (hcase :
      ∃ u v : V, u ≠ v ∧
        Gᶜ.degree u = 0 ∧ Gᶜ.degree v = 0 ∧
