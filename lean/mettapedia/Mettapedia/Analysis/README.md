...
`UnitTorusGradientProjection.lean` constructs the orthogonal projection
onto the closed `L²` space generated by actual continuous periodic
gradients. It preserves every gradient pairing, satisfies contraction
and an exact Pythagorean identity, and annihilates continuously
differentiable divergence-free tensors. Subtracting any tensor with
zero gradient work leaves the projection unchanged; the projected norm
