...
theorem integrable_twoPointDerivativeCost_of_continuous (B w : X → ℝ)
    (dB dP dw : X → ℝ) (hB : Continuous B) (hw : Continuous w)
    (hdB : Integrable (fun x ↦ dB x ^ 2) ν) (hdP : Integrable (fun x ↦ dP x ^ 2) ν)
    (hdw : Integrable (fun x ↦ dw x ^ 2) ν) (H : C(X × X, ℝ)) (tau : ℝ)
...
theorem integrable_twoPointDerivativeCost (B w : X → ℝ) (P : X → E)
  ...
  exact integrable_twoPointDerivativeCost_of_continuous B w dB dP dw hB hw hdB hdP hdw
    ⟨_, hH⟩ tau M hM a b ha hb
