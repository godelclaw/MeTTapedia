...
open Amplitude Equiv

/-! ### Selecting among perfect matchings -/

variable {V C : Type*}

/-- A colouring selects one matching at each vertex and is compatible when the
selected matching takes the vertex to another vertex with the same selection. -/
def MixColouring (σ : C → Equiv.Perm V) (c : V → C) : Prop :=
  ∀ x : V, c (σ (c x) x) = c x

/-- A compatible pointwise selection from perfect matchings is itself a perfect
matching. -/
theorem perm_of_mixColouring [Fintype V] [DecidableEq V]
    (σ : C → Equiv.Perm V) (hσ : ∀ k, σ k ∈ pairings V)
    (c : V → C) (hc : MixColouring σ c) :
    ∃ τ : Equiv.Perm V, τ ∈ pairings V ∧ ∀ x : V, τ x = σ (c x) x := by
  classical
  have hinv : ∀ (k : C) (x : V), σ k (σ k x) = x :=
    fun k x => (mem_pairings_iff.mp (hσ k)).1 x
  have hnf : ∀ (k : C) (x : V), σ k x ≠ x :=
...
/-- If the union of two indexed matchings has two vertices in different
components, choosing one matching on one component and the other elsewhere
gives a nonconstant compatible selection. -/
theorem mixColouring_of_disconnected (σ : C → Equiv.Perm V) {k l : C} (hkl : k ≠ l)
    {x₀ y₀ : V}
    (hsep : ¬ Relation.EqvGen (fun x y : V => y = σ k x ∨ y = σ l x) y₀ x₀) :
    ∃ c : V → C, MixColouring σ c ∧ ¬ (∀ x y : V, c x = c y) := by
  classical
  set R : V → V → Prop := fun x y => y = σ k x ∨ y = σ l x with hR
  set c : V → C := fun x => if Relation.EqvGen R x x₀ then k else l with hc
  have hstep : ∀ (x : V) (j : C), (j = k ∨ j = l) →
...
lemma cross_eq_two_iff_false (_hm : 1 < m) (i j k : ZMod m) :
