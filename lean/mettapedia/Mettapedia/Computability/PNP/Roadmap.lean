...
def currentPNPRoadmap : List PNPRoadmapEntry :=
  [ { stage := .sourceMap
    ...
    { stage := .v13RealRungOneQRowBound
      ...
      itvLowerPercent := 92
      itvUpperPercent := 100
      obligation := "Assemble the explicit q-row success bound: the proved kernel-flip surcharge plus the single pinned span-counting obligation V13RealLinearAdaptiveRowSpanCountingBound imply success at most 1/2 + 2^q/2^m." },
...
theorem currentPNPRoadmap_records_v13_real_rung_one_qrow_bound :
    ({ stage := PNPRoadmapStage.v13RealRungOneQRowBound
       ...
       itvUpperPercent := 100
       obligation :=
        "Assemble the explicit q-row success bound: the proved kernel-flip surcharge plus the single pinned span-counting obligation V13RealLinearAdaptiveRowSpanCountingBound imply success at most 1/2 + 2^q/2^m." } :
      PNPRoadmapEntry) ∈ currentPNPRoadmap := by
  simp [currentPNPRoadmap]
