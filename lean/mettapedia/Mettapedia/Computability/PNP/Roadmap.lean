...
def currentPNPRoadmap : List PNPRoadmapEntry :=
  [ { stage := .sourceMap
    ...
    { stage := .v13RealRungOneQRowBound
      ...
      itvLowerPercent := 98
      itvUpperPercent := 100
      obligation := "Assemble the explicit q-row success bound for the uniform certified-invertible sampler: the proved kernel-flip surcharge, row-combination space bound, block-iff-generate characterization, blocked/generated mass equality, closed zero-row plus high-budget cases, and target-row chooser mass equality identifies q = 1 exactly with uniform target-row occurrence mass. The proved two-dimensional GL2(F2) count bound turns the full-map branch-interface obstruction unconditional. The active theorem surface is the causal row-query interface: zero-row and high-budget cases are closed, and the all-q theorem now follows from the fixed-map fiber coefficient-counting pin V13RealLinearUniformCausalLowPositiveFiberCoefficientCountingBound, which sums over sampled maps to the coefficient pin V13RealLinearUniformCausalLowPositiveCoefficientCountingBound and then implies the division-free completion pin V13RealLinearUniformCausalLowPositiveCompletionCountingBound." },
