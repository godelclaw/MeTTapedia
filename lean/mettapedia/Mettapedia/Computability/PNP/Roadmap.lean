...
def currentPNPRoadmap : List PNPRoadmapEntry :=
  [ { stage := .sourceMap
    ...
    { stage := .v13RealRungOneQRowBound
      ...
      itvLowerPercent := 98
      itvUpperPercent := 100
      obligation := "Assemble the explicit q-row success bound for the uniform certified-invertible sampler: the proved kernel-flip surcharge, row-combination space bound, block-iff-generate characterization, blocked/generated mass equality, closed zero-row plus high-budget cases, and target-row chooser mass equality identifies q = 1 exactly with uniform target-row occurrence mass. The proved two-dimensional GL2(F2) count bound turns the full-map branch-interface obstruction unconditional. The active theorem surface is the causal row-query interface: zero-row and high-budget cases are closed, and the all-q theorem follows from the coefficient-counting pin V13RealLinearUniformCausalLowPositiveCoefficientCountingBound, which implies the division-free completion pin V13RealLinearUniformCausalLowPositiveCompletionCountingBound. The naive pointwise fixed-map fiber strengthening V13RealLinearUniformCausalLowPositiveFiberCoefficientCountingBound is formally refuted in the two-dimensional identity case. Constant one-row causal observers now reduce to the fixed-row occurrence pin V13RealLinearUniformFixedTargetRowOccurrenceCountingBound; coordinate-swap equivalence proves this is equivalent to one representative fixed row/target count in each nonempty dimension, and every fixed row and target coordinate in m = 2 is closed by fixed-row occurrence counting. The remaining count must prove the representative GL occurrence bound, generalize this averaged occurrence accounting, or refine to a transcript-local injection that accounts for hidden-witness multiplicity." },
